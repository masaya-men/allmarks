  ...
  return (
    <div className={vertical ? styles.iframeWrap9x16 : styles.iframeWrap16x9}>
      <iframe
        ...
        // Every tier loops (Tier 1 ambient preview AND Tier 3 / Lightbox
        // playback-through): short clips restart instead of freezing on a
        // final frame. YouTube only loops a single video when `playlist=<id>`
        // is also present (loop=1 alone is a no-op for one video).
        src={`https://www.youtube.com/embed/${videoId}?autoplay=1&enablejsapi=1&loop=1&playlist=${videoId}${muted === true ? '&mute=1&controls=0&modestbranding=1' : ''}`}
        title={title}
