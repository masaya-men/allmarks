...
**プロセスメモ**: 拡張の floating-button/PiP は実機/PiP 上の挙動を自動検証できないため、CSS 隔離レンダで見た目を確認 + ユーザー実機テストで確定の二段構え。スクショ生成は Playwright で本番 origin の IndexedDB に直接シード注入する方式(`DB_NAME='booklage-db'` v16、`type:'website'`+`thumbnail`+`ogpStatus:'fetched'`、密度は localStorage `booklage:card-width-px`/`card-gap-px`、背景ワードマークは board-config `bgTypoEnabled`)。

**次セッション**: ① ユーザーが拡張ストアに提出(デベロッパー登録 約¥800・一度きり、`dist/` の zip + store-assets を使用)→ 公開後 `EXTENSION_STORE_URL` 投入 → 再デプロイ。② 別タスク=右下・言語切替の見た目磨き込み(`🌐`絵文字→オンブランド、リストをガラス+フェード、a11y)。③ 残り公開前 TODO(オンボーディング/操作動画/テーマ1つ/モバイル最適化/バックアップ表出し/公式X)。詳細は [CURRENT_GOAL.md](./CURRENT_GOAL.md)。
