...
**s166 追記（実機フィードバック1件・本番反映済・commit `d02b922`）**：ユーザー報告「コラージュでテキストカードを一定より小さくすると、いきなり何倍にも縮む」。systematic-debugging で根本原因を特定＝共有 `ResizeHandle` の `dom = |dx| ≥ |dyW| ? dx : dyW`（符号保持の支配軸）＋ `dyW = totalDy·signY·aspect`（縦移動をアスペクト倍増幅）。2軸が grow/shrink で食い違う方向にドラッグすると、支配軸が反対符号値へ切り替わる瞬間に幅が**不連続ジャンプ**（数値シミュ実測＝placeholder aspect 1.25 でも ~200px ジャンプ／幅広カードは縦40pxで MIN 崩落）。純math `lib/board/resize-math.ts` に `resizeWidthFromPointer`（2モデル：`'dominant'`＝盤面デフォルト byte-identical・テストで固定／`'projection'`＝対角への連続投影）を切り出し、CollageCanvas は `resizeModel="projection"` を渡す。投影は「掴んで対角に引く」自然操作では dominant と数値一致（感触不変）、off-axis ではジャンプ消失。TDD 9テスト（連続性 assert 含む）。tsc0 / vitest 2035 / build OK。**盤面のリサイズは意図的に無変更**（デフォルト dominant のまま・ユーザーが満足しているため literal scope を尊重）。ジェスチャは setPointerCapture で Playwright 不可＝純関数テスト＋実機目視で確認。
