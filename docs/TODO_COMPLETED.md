...
## セッション 134 (2026-06-25) — paper-atelier ブラッシュアップ継続（角の真因修正・本物インク染み・タグ=マステ 他 6本）

### 進め方: 5本の並列調査(Workflow)で事実を固めてから実装
角の不具合で前セッションの私の説明（コントラスト/影の話）が**誤り**だったため、思い込みを避けて Workflow で5エージェント並列調査（A角バグのブラウザ再現+修正案注入検証 / Bカード台紙 / Cメディア contain / Dパララックス強度 / E染み素材）。各々ファイル行つきの検証済み所見を返し、それを統合して実装。

### shipped（全て本番 allmarks.app、commit+push 済、default byte-identical）
1. **角が y=0 でだけ四角くなる不具合の真因特定＋修正**: 原因は `.canvas::before/::after`（上下の暗い影帯, position:absolute z-index:80）。角丸(14px)overflow:hidden の `.canvas` の上辺にこの positioned 子孫が乗ると、Chromium が y=0 のとき**パネル自身の背景の角丸クリップを破棄**（スクロールすると再描画で直る＝最上段だけ四角）。サブエージェントが候補修正をブラウザに注入して「四角→丸」を実測（clip-path/isolation/contain/mask は効かず、影帯 display:none と will-change:transform on .canvas のみ有効）。`.canvas`への will-change は全画面 Lightbox（fixed backdrop/stage）を壊すので不採用。**紙テーマで影帯を display:none**（`BoardRoot.module.css`、paper-scoped。暗い影帯はクリーム羊皮紙に元々不要）。
2. **インク染み＝ユーザーの本物素材に差し替え**: 自作の手続き生成(turbulence/円群)は「雲」っぽく、ユーザーの本物（飛び散った墨の脚つき）の方が良い。マスターシート `12_29_56-Photoroom.png` 節4 TEXTURE-ACCENT から sharp で切り出し（単発/二連/反転）→ `ink-splat-1/2/3.png` を上書き、生成スクリプト+テスト削除。元素材は ~60px と低解像だが、にじんだ背景染みには soft が正解。
3. **染み小型化＋ループ配置**: width 30-76（旧180-380、user「1/8でいい」）、リング/飾りも縮小。`board-decor.ts` を固定 1500px タイルを繰り返す方式に（交互に x反転+回転でリピート感を隠す）→ 下までスクロールしても途切れない（旧 MAX_ITEMS=90 で tall board は下が空だった）。
4. **カードの英語ワードスタンプ削除**: `paper-decorations.ts` の word stamp（ARCHIVE等）は付くとタグと誤認されるので常時 null（rng 列は温存し他装飾は不変）。散布層の accents からもワードスタンプ除外。
5. **タグ＝マステに手書き文字（テーマ追従）**: `TagIndicatorStrip.tsx` を `useIsPaperTheme` で分岐。紙テーマでは washi-tape PNG 上に Yomogi/Caveat 手書きでタグ名（id 由来で安定変種+傾き）、写真コーナーより上(z90)に出して可読。default は従来の白モノスペース pill。
6. **（前々回分も本番化）メディア contain + 台紙可視化**: `.paperPhoto img` cover→contain（切り取らず縮小）、窓背景 `--paper-window-bg`、台紙プールに card-mat-aged 追加。

検証: tsc0 / vitest 1790・1790 / build OK。メモリ index(MEMORY.md) 22.7→17.9KB に圧縮、stale 4件除去。

### user フィードバック / 確認
- 染み・パララックス・washiタグ **OK**。
- **次の最優先バグ**: washiタグのマステが「+ TAG」ボタン（CardsLayer.tsx:1317-1341, top:8 left:8 z40）に被って操作不能（TagIndicatorStrip paper container top:4 left:12 z90 が覆う）。
- 他（D=台紙バリエーション / F=パネル背景羊皮紙化＝もれなくテーマ追従）は私の判断で進めてよい。

### 残（次セッション = 135）
①washi×+TAG 重なりバグ修正 → ②D カード台紙バリエーション追加 → ③F パネル背景の羊皮紙化（TUNE/SETTINGS/言語/タグ追加＋別ルートの保存ページ）。詳細 [CURRENT_GOAL.md](CURRENT_GOAL.md)。

---

## セッション 135 (2026-06-25) — paper-atelier ブラッシュアップ継続（バグ1＋台紙2＋パネル羊皮紙化5面）本番反映

3タスクを優先順どおり実装・全て `allmarks.app` 反映済。tsc0 / vitest1790 / build OK。**default(黒+音波)は byte-identical**（全変更が paper-scoped、各 CSS は git numstat で deleted=0 の純追記を確認）。

### ① washiタグ × +TAG 重なりバグ修正（最優先）
- 真因を実コードで裏取り: paper の `TagIndicatorStrip` コンテナが **z90**（装飾overlay z11 の上に出すためだけの値が、全インタラクティブchrome=+TAG z40/popover z70 より上になっていた）＋ top:4 で +TAG(top:8) に第1テープが被りクリックを奪取。
- 修正（[TagIndicatorStrip.tsx](../components/board/TagIndicatorStrip.tsx) の `isPaper` 分岐のみ）: **z90→15**（装飾z11 の上・全chrome の下）＋ **top:4→32**（+TAG の下へ）。z を下げたことで「単に下げるだけだと起きる popover(z70) との新衝突」も同時に防止。
- Playwright 実測検証: +TAG 中心の `elementFromPoint` が +TAG ボタンを返す（テープでない）。commit `(washi fix)` → push。

### ② カード台紙バリエーション追加（D）
- 既存プールは無地クリーム3＋古紙1＝変化に乏しい。マスターシート(`12_29_56-Photoroom.png`, 透明背景=alpha)の節2(CARD&PANEL)から **連結成分ラベリングで24 piece を自動検出** → 罫線紙(i12) と 方眼紙(i11) を trim+upscale(~3.3x) で切り出し → `card-mat-lined` / `card-mat-grid` として登録、ImageCard プールに追加（[ImageCard.tsx:227](../components/board/cards/ImageCard.tsx#L227)）。
- **重要な技術的発見**: `.paperCard` は `background-size: cover`。カードのアスペクト比は写真ごとに変わるので、**枠付き/クリップボード等の強い矩形フレーム素材は cover で切れて壊れる**（deckle-edge と同じ）。→ cover で生きる**無地テクスチャ系のみ**プールに入れた。Playwright で grid/lined を強制描画し写真窓の保持を確認。commit。
- **user 相談事項として持ち越し**: 「囲った全部」のうち**枠付き・クリップボード**は cover 制約で別扱いが必要。user が今日生成した 16:08 の**古紙フレームセット(1)**（高解像度・専用）は、(a) 専用「枠付きカード」モード(`background-size:100% 100%` か枠PNGオーバーレイ) か (b) パネル枠の装飾、どちらで使うか要相談。※16:08 バッチは5テーマ分のフレーム（古紙/SF白メカ/暗緑端末/celestial紺金/リキッドグラス）＝将来テーマの素材準備。

### ③ パネル背景の羊皮紙化（F）— もれなくテーマ追従
- session133 は文字だけ墨化。今回は**パネル背景そのもの**を羊皮紙に。globals.css paper ブロックに**共有トークン**追加（`--paper-panel-surface #f1e7cf` / `-texture`=parchment-bg-plain / `-border` / `-hairline` / `-shadow` / `-btn-bg`）。各パネルの `.module.css` に `:global(html[data-theme-id='paper-atelier'])` で背景=羊皮紙＋温色hairline＋温色shadow＋blur無し＋墨文字。白ハードコード文字→墨、静音ボーダーボタン→温色面。機能アクセント温存（orange ADD-TO-CHROME / green quick-tag / green "already-added" chip）。
- 対象: **TUNE**(TuneTrigger+TunePresetColumn) / **SETTINGS**(ExtensionEntry+ThemePicker) / **言語**(LanguageSwitcher) / **タグ追加pop**(TagAddPopover)。CSS 変換は4並列サブエージェント(sonnet)に委譲、私が設計トークン・/save・全検証を担当。
- **別ルート /save のテーマ伝達**: /save は (app)/ 配下でないので board の pre-paint テーマスクリプトを継承しない。[app/save/layout.tsx](../app/save/layout.tsx) に同等スクリプト注入＋テーマ連動 bg（default黒/paper羊皮紙）。[SaveToast.module.css](../components/bookmarklet/SaveToast.module.css) に paper ルール（stage=羊皮紙＋墨チェック/ブランド/ラベル）。
- Playwright 実測検証（paper, dpr2）: SETTINGS/TUNE/言語 とも computed bg=`rgb(241,231,207)`＋墨文字、+TAGpopover 入力=墨on半透明、/save は `data-theme-id` 伝播＋stage羊皮紙。視覚も4パネル+/save 全て確認。commit `(parchment panels)` → push → 本番デプロイ（master direct upload）→ board/save/root 200 スモーク。

**user 宿題（最優先）**: allmarks.app ハードリロード → Paper Atelier で実機目視: ①washiタグ下の+TAG が押せるか ②罫線/方眼の新台紙の見え（cover+大写真なので余白＋caption帯に控えめに出る） ③TUNE/SETTINGS/言語/タグ追加pop/保存窓 の羊皮紙パネルの色味・可読性。④枠付きカード/クリップボードと 16:08 フレームセットの使い道を決める。

---

## セッション 136 (2026-06-26) — Paper/テーマ系の実機フィードバック多数反映（4デプロイ）

ユーザーの実機目視フィードバックを1つずつ反映。全て本番 `allmarks.app` 反映済み。tsc0 / vitest1791 / build OK。default(Sound Wave=黒+音波)は byte-identical 維持。

### ① テーマ追従漏れクラスタ（FilterPill / 拡張strip / PiP）
- **FilterPill 右上ドロップダウン**: 暗い黒箱(`rgb(8,8,10)`+白+mono)→ 羊皮紙紙片(`rgb(241,231,207)`+墨)。TagAddPopover の `--paper-panel-*` トークンをミラー。pill も残存していたアイボリー text-stroke(globals が paper で 0 にリセット忘れ)を除去＋11px/400 に揃え(隣の TUNE/SETTINGS と所帯統一)。実機指摘でメニュー本文も mono→serif(Fraunces)化。
- **拡張のフローティングタグ strip**: 暗いままだった真因 = `app/save-iframe` は拡張の offscreen 内で **allmarks.app を third-party 埋め込み**するので Chrome のストレージ分離で **localStorage が別パーティション(空)**。pre-paint localStorage スクリプトが効かない。→ **IDB `board-config` からテーマを読む**よう `SaveIframeClient.buildSavePayload` を修正(タグと同じ共有 IDB なので確実)。memory `reference_save_iframe_storage_partition` に記録。
- **PiP**: `lib/board/pip-window.ts` が PiP ドキュメントに `data-theme-id` をスタンプしていなかった → 複製済 paper CSS が効かず暗かった。属性スタンプ＋非default時の stage bg をテーマ色に＋PiP CSS 4枚(`PipCompanion/PipStack/PipEmptyState/PipCard`)に paper override＋`PasteSaveFeedback` に実 themeId を渡す。user 確認OK。

### ② テーマ名 英語統一＋DEFAULT バッジ
全15ロケール JSON を英語に統一: **Sound Wave**(既定・旧 Dotted Notebook=音波が主役)／**Grid**(旧 Grid Paper)／**Paper Atelier**。`ThemePicker` で既定テーマのみ **DEFAULT** バッジ(他は FREE)。

### ③ Grid テーマ刷新（ビューポート固定層）
- user 指摘「左右で枠の見切れ方が違う＝とてつもない違和感」。真因 = グリッドが `ThemeLayer`(contentWidth≈1963、横パン)に乗っていて `background-position:center` が**コンテンツ幅基準**で中央化 → 盤面(canvas 1393)とズレて非対称。
- 修正: グリッドを **ビューポート固定層**(`.gridLines`、`InteractionLayer` 直下に inset:0、`BoardRoot` で `themeId==='grid-paper'` 時のみ描画)に移設 → canvas 中央=viewport中央で対称(canvas端で左右16.5px)。`.gridPaper`(panned ThemeLayer)は透明化。
- パララックス: `background-position-y = -(usePaperParallax factor 0.5)` で 0.5x ドリフト(motion/reduced gating)。「張り付き」解消。memory `reference_theme_bg_viewport_vs_content`。
- 計測の教訓: IDB board-config の直接 seed はタイミングでテーマ切替らないことがある → playwright は `theme-button-<id>` を UI クリックで確実切替。

### 次セッション(137)に持ち越し
④SETTINGS 再設計(方向B=テーマ別枠、user が構成提示済)／⑤SHARE のテーマ化(Plan3・バイラル核、背景色しか変わっていない)／②拡張strip 実機検証／⑥マステ・ピン配置／⑦チュートリアル PiP 紹介アニメ／⑧枠付きカード(16:08フレーム)の使い道。

---

## セッション 138 (2026-06-29) — ⑤SHARE のテーマ化 設計完了（スクショ方式に確定）＋ テーマパネル外側クリック修正

### テーマパネル 外側クリックで閉じる（出荷）
session137 の右ドック非ブロッキング テーマパネル(`ThemeModal`)が × / Esc でしか閉じなかった。`document` の pointerdown を監視し、パネル外(背面の盤面・SETTINGS ドロワー等)の押下で閉じるよう追加(FilterPill の作法に統一)。非ブロッキング設計(overlay は pointer-events:none で盤面に透過)を壊さないため document リスナ + パネル containment 判定が正解。CUSTOMIZE のカラーピッカーはネイティブ `<input type=color>`(OS別窓)なので誤爆しない。`ThemeModal.test.tsx`(本コンポ初テスト)で 外側=閉じる/内側=維持/閉時=非購読 をピン留め。tsc0 / vitest1802 / build OK → allmarks.app 反映済。

### ⑤ SHARE のテーマ化 — ブレスト→調査→スパイク→spec→plan（実装は次セッション）
user 指摘「共有は背景色しか変わらず台紙/装飾/パターンが乗らない＝バイラルが生まれない」。session137 の patternLayer + 色カスタム(theme-customization.ts)を共有に乗せるのが目的。

- **調査(並列サブエージェント7体)で事実確定**: 共有は3面(送信プレビュー ShareMirror / OG画像 capture-mirror=手描きcanvas・色ベタ書き / 受信ページ SharedBoard=本物部品再利用だが data-theme を div に付けるだけで空振り)、いずれもテーマ非対応。送信は `BoardRoot:1786` で `DEFAULT_THEME_ID` 固定、ShareDataV2 に custom の箱なし。
- **user の素人考えが核心**: 「プレビューは作り直さず複製でいい / 画像はただのスクショでいい」→ 記録(IDEAS.md 2026-05-06「シェア画面=ボード縮小版」)とも一致。clickability の心配は杞憂(SNS はリンクが飛ばす役、画像は見せる役、画像は本来クリック不要)。
- **dom-to-image が過去捨てられた真因を履歴で確認**: `09f6e46`/`b69e2e3`/`73c18c7` = 本物の盤面(最大300枚+動画iframe)を丸ごと処理して 5GB+ 爆発 + 埋め込み動画の幽霊自動再生、しかもライブラリがフィルタ前に全画像を先読みするため画面外除外も効かず。→「**見えてる枠だけ**撮る」なら全要因回避、と判明(user の仮説が正しかった)。
- **Satori(業界標準サーバー描画)を feasibility 調査→見送り**: Cloudflare Pages Functions で動くが無料枠の CPU(10ms)/容量(3MB compressed)上限に当たりやすく安定運用は有料 Workers(~¥800/月)＝¥0 設計と衝突。クライアント Satori も候補に挙げたが、結局 user の「スクショ」案が最善と判断。
- **スパイクで実証(2026-06-29)**: 本物テーマCSS + 本物の2.3MB紙PNGで「見えてる枠を dom-to-image でPNG化」を実測 → Grid 47ms/553KB・Paper 321ms/1134KB、落ちない/taintしない、**Paper(紙テクスチャ+セリフ)まで完璧再現**(Satori が苦手な所をスクショは苦もなく出した)。唯一の発見=格子の横線が薄く落ちる(CSSグラデ2枚重ねの既知クセ)→**単層SVG化**で対策(紙PNGが完璧だったのと同じ理屈)。成果物 scratchpad/frame-{grid,paper}-{DTI,REF}.png。
- **spec + plan を commit/push**: 方式=見えてる themed プレビューを dom-to-image でスクショ→JPEG→今と同じく R2 保存(¥0・サーバー無変更)。custom を ShareDataV2 に載せ、プレビュー/OG画像/受信ページ の3面をテーマ化。default は live 盤面 byte-identical、dom-to-image 失敗時は従来canvasへ fallback。plan は6タスク(全TDD・全コード入り)。

### 次セッション(139)
**⑤の実装をサブエージェント駆動で Task 1 から**(user 合意)。詳細は plan 参照。残り: ⑥マステ/ピン ⑦PiP紹介 ⑧枠付きカード / follow-up 明色BOARDヘッダー色。

---

## セッション 139 (2026-06-29) — ⑤SHARE のテーマ化 全6タスク実装・出荷 (サブエージェント駆動)

セッション138 で完成した spec/plan を **superpowers:subagent-driven-development** で実行。各タスク = fresh implementer subagent (TDD) → review-package → task reviewer (spec + quality) → fix loop。全タスク完了後に opus で whole-branch 最終レビュー。feature ブランチ `share-theming-screenshot` → master no-ff マージ (`510b7bf`)。

### 実装した 6 タスク

1. **`ShareDataV2` に `custom` フィールド** (`483650d`): `ShareCustomization`(縁/盤面/パターン色+柄+密度+Title色 = 既存 `ResolvedThemeCustomization` と同形) を型 + Zod + sanitize(壊れた custom は payload 全体を落とさず drop) + `board-to-share` に配線。
2. **BoardRoot が live themeId+custom を送信** (`01eaaf1`): `buildShareData` の `DEFAULT_THEME_ID` placeholder を撤去し `themeId` + `resolvedCustom ?? undefined` を送信、useCallback deps 拡張。
3. **単層パターン SVG ヘルパ** (`7d7da1a`): `patternSvgDataUri()` — grid/dots/diagonal/crosshatch の継ぎ目なしタイル SVG data-uri。dom-to-image が積層 CSS gradient の片方向を落とす問題を回避(spike で実証済)。
4. **受信ページ `/s/` のテーマ適用** (`d140c75`): `<html data-theme-id>` effect(hooks 順守・cleanup あり) + pattern テーマは patternLayer 描画 / Paper は globals.css カスケード。
5. **ShareMirror プレビューのテーマ化** (`0ba8ea1`): inline style で縁/盤面/パターン/Paper parchment/title色を塗る(.module.css 無編集)。SenderShareModal + BoardRoot に themeId+custom 配線。波及で SharedBoard 再シェアも受信テーマを引き継ぐ。
6. **dom-to-image で OG画像生成** (`534d60a` + 修正 `e191a39`): `render-share-image.ts`(`jpegUnderTarget` 純関数 + dom-to-image 遅延 import、失敗時 null)。SenderShareModal に hidden visible-only capture node(≈10-20枚のみ=メモリ安全)。null 時は従来 `captureMirrorToWebP` へ fallback(共有は絶対壊れない)。

### 最終レビュー(opus)が発見した統合バグ + 修正

per-task レビューでは見えなかった統合不具合: **受信 patternLayer に position 指定が無く Grid が不可視**(in-flow で height 0)。3面のうち receiver だけ壊れていた。BoardRoot の実証済み patternLayer と同形に修正(`position:absolute/inset:0/zIndex:THEME_BG/pointerEvents:none`、`eab7962`)+ SharedBoard.test に position/inset 退行ガード追加。

### 検証

tsc clean / vitest **1813緑** / build OK(assert-share-template OK)。**live allmarks.app で実測**: Grid 受信ページの patternLayer = position:absolute, height **583.23px**(修正前 0), backgroundImage = SVG グリッド → スクショで視覚確認。Paper 受信は `[data-pattern]` 無し = 正しい(work テーマは html[data-theme-id] parchment カスケード)。default(Sound Wave) byte-identical、¥0(サーバー無変更)。
...
## セッション 145 (2026-07-01) — (C) 生計まわりの設計フェーズを完全クローズ（設計・計画のみ／コード変更なし）

**このセッションは実装ゼロ・設計と調査に専念。有料テーマ解錠機構（K3）の設計 spec ＋ 実装計画を確定し、外部連携の宿題フラグを事実調査で解決した。機微情報は全て `docs/private/`（gitignored・commit しない）に記録。tracked 側は中立表現のみ。**

### やったこと
- **宿題フラグ（唯一の未知＝外部連携）を事実調査で解決**：会員確認の外部連携について、日本＆海外の支援系プラットフォームを3方向＋9サービス個別で徹底調査（web）。「どのプラットフォームにも"会員に固有キーを自動配布する仕組み"も"外部から会員資格を自動確認する公式連携"も無い」ことが判明 → **キー配布は自作の"合言葉リンク方式"に一本化**する結論に至り、未知を消した。
- **K3 解錠機構の設計 spec を確定**：署名付きライセンスキー（Ed25519・公開鍵でオフライン検証）＋極小 Worker/KV（発券＋発動台数カウント）＋クライアント配線（既存の `EMPTY_LICENSES` を本物のライセンス集合へ差し替え・SETTINGS 解錠UI・端末ID）。**5台キャップ／サーバー障害はフェイルオープン／ブクマ非接触／¥0インフラ／default byte-identical** を不変条件に。
- **実装計画（工程表・10タスク TDD）を確定**：キー形式→鍵ペア生成→端末ID→ライセンス保存→発動 Worker（5キャップ）→発券 Worker→解錠アクション→`EMPTY_LICENSES` 差し替え→SETTINGS 解錠UI→デプロイ配線＋実機検証。各タスクに実コード・テスト・コミットまで記述済で着工可能な状態。
- **既存土台の確認**：`ThemeMeta.tier`/`isThemeUnlocked`/`resolveThemeId`（locked→default フォールバック）/ピッカーの鍵表示は s131〜で既に実装・テスト済。K3 は「本物のライセンス集合を差し込む」残作業だと確定。

### 記録先（すべて docs/private・gitignored）
- `2026-07-01-support-platform-research.md`（プラットフォーム徹底調査の統合）
- `2026-07-01-k3-unlock-design.md`（K3 設計 spec）
- `2026-07-01-k3-unlock-plan.md`（K3 実装計画・10タスク）
- 既存 `2026-07-01-launch-plan-design.md` に宿題フラグの調査結果を追記（解決済に更新）
...
## セッション 150 (2026-07-02) — カード内ボタンのホバー反応 + ドラッグ中grabbingカーソル + テキストカードscrollbar誤ライトボックス修正 → ブランチ master マージ

前セッション149のボード磨きブランチ `fix/board-cursor-and-paper-meter` に3件を追加実装し、全て `allmarks.app` 反映・ユーザー実機OK 後、`--no-ff` で master にマージ（ローカル/リモートのブランチ削除）。本番は既にブランチ内容を反映済のため再デプロイ不要。

### ① カード内ボタンのホバー反応（▶ 風「拡大＋明るく」）
- お手本 = `MediaTypeIndicator` の ▶（hover で 22→34px grow）。文字/グリフには控えめ倍率で移植。共通 `transition: ... cubic-bezier(0.16,1,0.3,1)`、育つ方向は角から内側（transform-origin）。
- ×（削除）/↺（リセット）= [CardCornerActions.module.css](../components/board/CardCornerActions.module.css) の `:hover` に `scale(1.2)` + transform-origin（右上/左下）。
- ＋TAG = [CardsLayer.tsx](../components/board/CardsLayer.tsx) の inline button に `className={styles.addTagButton}` + inline transition に transform/filter 追加、[CardsLayer.module.css](../components/board/CardsLayer.module.css) の `.addTagButton:hover { scale(1.15); brightness(1.15) }`。
- タグpill = 新規 [TagIndicatorStrip.module.css](../components/board/TagIndicatorStrip.module.css)。paper の傾きを inline transform → `--pill-tilt` CSS変数に逃がし、`.pill:hover` で `rotate(var) scale(1.12)` 合成。default=brightness、paper=暗インクなので drop-shadow で浮かせる。
- **default 盤面 静止時 byte-identical**（追加は全部 `:hover` 限定、class は rest で transition のみ）。敵対レビュー2観点＝指摘ゼロ。

### ② ドラッグ中の grabbing カーソル
- 症状：カードをドラッグ移動中もカーソルが指（pointer）のまま。
- 根本原因：並べ替えドラッグは**ラッパー**に `setPointerCapture`。grabbing を出す `.cardNode:active`（子）は **capture 中は効かない**（カーソルは capture 対象/ヒットテスト側で決まる）。
- 修正：`dragState` 非null中だけ `<html data-card-dragging>` を立て、[globals.css](../app/globals.css) の `html[data-card-dragging] * { cursor: grabbing !important }` で強制。静止時は無効＝byte-identical。ビルド出力にも規則残存を確認。

### ③ テキストカードの scrollbar 押下で誤ってライトボックスが開く
- 根本原因（**実ブラウザで実証**）：`.titleScroll`（[PlaceholderCard](../components/board/cards/PlaceholderCard.tsx)、`data-card-scroll="true"`）の scrollbar 押下も pointerdown がラッパーに届き `setPointerCapture` 成功 → ①native スクロール強奪（>5px で並べ替え化）②静止クリックが「カードのクリック」判定 → ライトボックス。
- 修正：純関数 `pressLandsOnCardScrollbar`（[use-card-reorder-drag.ts](../components/board/use-card-reorder-drag.ts)）で scrollbar 帯の押下を検出し、`handleCardPointerDown` が capture 前に return（native に委譲、board pan は stopPropagation で抑制）。**classic scrollbar（実 Windows Chrome=10px、headed 実測）は厳密な gutter 幾何でパディングは従来どおり開く／overlay（自動非表示・幅0）は容器 target フォールバック**のハイブリッド。TDD（失敗テスト先行）+ Playwright 実マウスで両モード検証。テスト9件追加（vitest 1858→1867）。
- 手法メモ：Playwright の `page.mouse`（CDP=trusted 入力）は `setPointerCapture` が効く（= synthetic dispatchEvent との違い）。board のドラッグも実マウスなら検証可能と判明。headless は overlay/headed は classic とレンダリングが違う点に注意。

### マージ
- `--no-ff` で master へ（9コミット＝149の磨き5＋docs1＋150の3）。tsc0 / vitest1867 / build OK を merge 後にも再確認。origin/master push、ローカル+リモートの feature ブランチ削除。

---

## セッション 150続き (2026-07-02) — 細かい気づき nit バッチ（EMPTY TRASH 赤 / 拡張タグ窓見切れ）+ 空ボード青モーダル調査

ユーザー実機メモ7件を分類・全て TODO に記録し、実施2件を `fix/board-small-nits` で出荷 → `--no-ff` で master マージ（ローカル/リモート削除）。tsc0 / vitest1867 / build OK。

### ✅ N-17 EMPTY TRASH ボタンを赤 danger 表示
- TRASH ビューかつ削除済みがあるときだけ出る「EMPTY TRASH」（＝ゴミ箱を完全削除する破壊操作）が他ボタンと同じ見た目で目立たなかった。
- `ChromeButton` に `data-variant?: 'danger'` を追加し、[ChromeButton.module.css](../components/board/ChromeButton.module.css) の `.btn[data-variant='danger']`（赤・hover で明赤・paper は濃赤インク）で着色。[BoardRoot.tsx](../components/board/BoardRoot.tsx) の EMPTY TRASH に付与。他ボタンは variant 無し＝byte-identical。**本番反映・ユーザー確認OK**。

### ✅ N-18 拡張クイックタグ窓の右カラム見切れ
- 原因（実装確認）：`extension/floating-button.css` の `.allmarks-tagstrip__drawer` が `grid-template-columns: 1fr 1fr`（各≒104px）＋ chip `white-space:nowrap`＋`overflow-y:auto`（横も clip）で "read later"/"ui inspo" 等が切れていた。ホストページ上の狭幅小窓（max 224px）なので2列が根本的に無理。
- 修正：drawer を **flex 1列**（full 幅）に。各タグが全幅を使えて見切れ解消。`text-overflow: ellipsis` を極端長タグの安全網に、開時の下端フェード（`::after`・`pointer-events:none`＝chip クリック透過）でスクロール示唆（生スクロールバーは従来どおり非表示＝ハウスルール準拠）。
- 検証：実 CSS を読み込んだ独立再現（Playwright headed）で **19タグ=19行の1列・通常タグ見切れ0・右端はみ出し0・…は超長テスト1件のみ**を実測＋目視。JS は content script のため `node --check` 実施（構文OK）。
- 配布：`extension/manifest.json` v0.1.21 → **0.1.22**、`pnpm package:extension` で `dist/booklage-extension-0.1.22.zip` 生成。**2026-07-02 Chrome ウェブストアに更新提出済（審査待ち）**。承認で既存ユーザーに自動更新。

### ⏹ N-14 Lightbox 中のボードモーション = 対応不要
- 調査（Explore）：カード浮遊/PlaceholderCard スライドショー巡回/Tier1 動画再生は全て `ambientOn = motionEnabled && !sourceCardId && !reduceMotion && !isScrolling` で **Lightbox 開くと停止済**。常時動くのは ScrollMeter だけ（Lightbox 中の「何枚目か」index 表示を兼務）。ユーザー判断で現状維持。

### 🅿 N-16 空ボードの青モーダル = スマホ限定・保留
- `EmptyStateWelcome` が旧・青グレートークン（`--color-bg-elevated` #222233 / `--color-text-secondary` #a0a0b8）を使用。ダーク raw-alpha に修正（backdrop-filter はボードの transform/will-change 層で inert＝memory `reference_board_backdrop_filter_inert` 準拠で不使用）。
- **重要な実測**：デスクトップ（headed 実Chrome相当・headless 両方）ではこのパネルは**背景の巨大ワードマークに occlude されて不可視**（z-12 なのに paint 逆転＝ボードの合成レイヤ由来）。→ ユーザーが見た青モーダルは**スマホ**。スマホ未対応につき保留。色修正は残置（将来スマホ対応時に効く／デスクトップ実害なし）。

### 手法メモ（再利用可）
- 拡張のホストページ注入UIは、実 CSS を scratchpad にコピー→独立 HTML から相対 link で読み、Playwright で実測するのが確実（file:// の日本語パス %エンコードは壊れやすい）。
- 空ボード検証は fresh context で IDB `settings` に `{key:'onboarding-completed',completed:true}` を put → reload で empty-state を出せる。

---

## セッション 152 (2026-07-02) — N-19「カードのサイズ/並び順を default に戻す」出荷（master `a7be63d` マージ・allmarks.app 反映済）

**ユーザー要望**: 600件も使っていると下の方のカードを小さくしたまま忘れる → 「default の大きさに戻すボタン」が欲しい。加えて「並び順を新しい順にいつでも戻せるか？」。

**調査で判明（ゼロから設計ではない）**:
- 個別サイズリセット（カード隅 ↺・`hasCustomWidth` 時のみ表示）と一括リセットロジック `resetAllCustomWidths` は既存。だが一括の UI は**セッション41でユーザー自身が「ResetAll 廃止（Ctrl+Z で代替）」と決めて外した**経緯（[TODO_COMPLETED.md:2139]）。今回は跨セッションで Ctrl+Z が効かないため再導入。
- 並び順は `repairOrderIndexIfNeeded`（savedAt DESC 振り直し）が既存だが**初回一度きり**（手動ドラッグ順の上書き防止のフラグガード）。→ フラグ非依存の新関数に切り出せば「何度でも実行可」にできると確認。
- 位置は保存しない（skyline 自動レイアウト）＝「位置を戻す」は実質「並び順を新しい順に戻す」以外の意味を持たない。

**設計（brainstorm → spec → plan → 承認）**: SETTINGS ドロワー（`ExtensionEntry`）に新グループ **LAYOUT** を追加し2ボタン。①**RESET CARD SIZES**（リサイズ済み枚数表示・0枚で無効）②**SORT: NEWEST FIRST**。どちらも**その場2タップ確認**（1回目「TAP AGAIN TO CONFIRM」→3秒以内に2回目で実行・排他・ドロワー閉/タイムアウトで解除）＋実行後トースト。EXPORT バックアップが同ドロワーの保険。**確認方式はモーダルではなく2タップ**（ドロワーのミニマルな雰囲気優先・ユーザー選択）。正本 [spec](superpowers/specs/2026-07-02-board-reset-layout-design.md) / [plan](superpowers/plans/2026-07-02-board-reset-layout.md)。

**実装（サブエージェント駆動 5タスク・各タスク2段レビュー＋opus 全ブランチレビュー Ready to merge）**:
- **T1** `lib/storage/indexeddb.ts`: 純ヘルパ `computeNewestFirstOrder`（savedAt DESC・id ASC・最新=最大 orderIndex）＋ `resortByNewestFirst(db)`（フラグ非依存・差分レコードのみ put・`{updated}` 返す・tx scope 'bookmarks' のみ＝migration store に触れない）。**`repairOrderIndexIfNeeded` は byte-identical で無変更**（比較ロジック数行の重複はユーザー承認済の安全側判断＝フェイルセーフなマイグレーション経路を触らない）。新テスト5。
- **T2** i18n 15言語に `board.settings.{layoutGroup,resetCardSizes,sortNewestFirst,tapAgainToConfirm,sortNewestNote,resetSizesDone,sortNewestDone}`。UI ラベル4種は英語 verbatim、注意文/トースト3種は各言語訳、`resetSizesDone` は `{n}` 保持。パリティテスト新設。
- **T3** `use-board-data.ts`: hook に `resortNewestFirst(): Promise<number>`（`resortByNewestFirst` → `reload()` で即反映。reload は orderIndex DESC 整列）。
- **T4** `ExtensionEntry.tsx`＋`.module.css`＋test: LAYOUT グループ＋2タップ確認 state machine（discriminated union `'sizes'|'sort'|null`）＋props3＋CSS 純追記（`.panelCta:disabled`/`[data-confirming]`/`.layoutNote`）。新テスト5。
- **T5** `BoardRoot.tsx`: hook 分割代入に2関数＋`customWidthCount` memo＋2ハンドラ（トーストは `t('...resetSizesDone').replace('{n}', ...)`）＋`ExtensionEntry` に props 受け渡し。

**検証**: tsc0 / **vitest 1887 pass 0 fail**（channel.test.ts フレーキー無し）/ pnpm build OK（`out/` 生成・share OG アンカー assert OK）。**default 盤面 byte-identical**（全変更 SETTINGS ドロワー内＝盤面 DOM 不変）。個別 ↺・TUNE ↺・マイグレーション無変更。opus 全ブランチレビューで tx 分離・reload DESC 経路・二段確認の race-free・データ損失なし（A=フラグのみ / B=orderIndex のみ）を直接検証し **Ready to merge**。`--no-ff` で master マージ・ブランチ削除・push・デプロイ。ユーザー実機で **N-19 動作OK**。

**続き（同セッション・フォローアップ修正）= SETTINGS ドロワーのスクロールフェードが下端に固定されないバグ修正**（master `365ddd4`・allmarks.app 反映済）。LAYOUT 追加でドロワーが初めてスクロール状態になり発覚。**根本原因**（systematic-debugging + Playwright 実測で確定）＝`.scrollFade` が `position:absolute; bottom:0` だが、ドロワー自身が `display:flex; flex-direction:column` のスクロールコンテナのため、フェードがコンテンツに固定されて一緒にスクロール（viewport 上端へ drift＝下端ヒントとして機能せず、特定項目＝ユーザー環境ではチュートリアル行に黒帯が貼り付いて遅れて消えるように見えた）。sticky+負マージンは flex で height 0 に潰れ不採用 → **インナースクロール方式**採用：内側 `.drawerScroll`（flex:1 1 auto / min-height:0 / overflow-y:auto）が実スクロールを担い、ドロワー本体は非スクロールの器として固定 → 絶対配置フェードが可視下端に常駐。Playwright 実測で「全スクロール位置で fade 下端==ドロワー下端／下に内容がある間だけ opacity1／最下部で opacity0」を dev＋本番の両方で確認。tsc0 / vitest1887 / build OK。2ファイル（ExtensionEntry.tsx / .module.css）のみ。

## セッション 155 (2026-07-03) — N-05 LP ナビ格納演出を作り直して出荷（session 154 は全損だった）

**発見（重大）**: session 154 の「実装済み・ブランチ `feat/lp-nav-dock`・commit `88178ff`」は**リポジトリのどこにも存在しなかった**。ツール破損は handoff commit だけでなく**実装の書き込み・commit 全部が偽保存**（fsck の dangling・reflog・origin・worktree・デスクトップ複製フォルダまで捜索して痕跡ゼロ）。実際にディスクに届いていたのは spec / TODO.md 追記 / 計測メモ feat4.txt の3つのみ。feat4.txt の計測（targetRect:null）から、当時の「横移動しない」バグ自体がヘッダー改修の偽保存が原因だった可能性が高い。

**さらに spec の「実コード確認」も偽読みだらけと判明**して全事実を再検証: SubpageHero は存在しない（各 `*Content.tsx` が個別 kicker 持ち）／SiteHeader に active 判定は無い／**15言語英語固定は誤り（en/ja のみ英語、13言語はローカライズ済）**／FAQ kicker="About" バグは実在しない。訂正一覧は plan の対照表参照。

**作り直し（このセッションで完遂・master `b0d81a6` マージ済・allmarks.app 反映済）**:
- **`lib/scroll/nav-dock-math.ts`（+test 14）**: 進捗・状態遷移（armed/traveling/docked）・適用判定の純関数。**範囲＋ラッチ式**なので Lenis 慣性で 1 フレームに大きく飛んでもすり抜けない（当初計画の「範囲判定」を最初から内蔵）。
- **`NavDockTraveler.tsx/.module.css`（新規）**: position:fixed の word が実 DOM rect（kicker とナビ ghost スロット）を毎フレーム追従。①ガラス帯で1文字ずつ乗り上がり（28ms/字）＋モーフ（12→14px 等 `--mp` 補間）→②460ms `cubic-bezier(.34,1.42,.64,1)` で右へダッシュ→③バウンド着地。上スクロールで可逆。
- **配線**: SiteHeader（active スロット2枚持ち＝通常ラベル + ghost、`html[data-nav-dock]` 属性ゲートで CSS 切替）/ MarketingShell（`useSmoothScroll`=Lenis + traveler マウント）/ 5ページ kicker に `data-nav-dock-anchor` / landing-tokens.css に anchor 隠しゲート。**属性は traveler が mount 後にのみ書く＝SSR/prerender・JS無効・reduced-motion は従来表示そのまま**。
- **安全ゲート（`isDockEligible`）**: reduced-motion オフ（ユーザー確定: OS設定尊重で演出全オフ）／≤960px オフ（スロット非表示幅）／kicker≠ナビ語（13ローカライズ言語）は自動オフ。
- **実測**: dev で検証シート 9/9 PASS（初期/traveling/docked±0px/復帰/慣性大ジャンプ耐性/reduced-motion/ja=有効/ko=自動オフ/トップLP非接触）＋残り4ページ armed 確認＋FAQ 着地実走。実測で settle タイマーの状態巻き戻しバグを1件発見して修正（`b5095b8`）。**本番 allmarks.app でも docked ずれ 0px・可逆・トップLP非接触を実測 PASS**。
- tsc 0 / vitest **1901** 全緑（dev サーバー並走中に無関係 suite が7件落ちる順序フレークを確認→並走停止で全緑）/ build OK。

**教訓（恒久）**: 書き込み・commit は必ず独立した実出力（ls / git log --stat）で確認する。引き継ぎの「実装済み」は git 上の実在確認までしてから信じる。

## セッション 149 — grab-reaction を master に squash マージ・ブランチ整理（TODO.md から移設）

- **`feat/edge-data-dissolve`（正味差分＝grab-reaction のみ）を master に squash マージ**（却下した縁エフェクト系20コミットは畳んで master 履歴をクリーンに保持）。マージ前に **tsc0 / vitest1858 / build OK** を再確認。本番 `allmarks.app` は既にこの状態を反映済（別デプロイ不要）。マージ後にブランチ削除。正本 spec/plan は `2026-07-02-board-grab-reaction-*`。
- **続き（同セッション）＝ボード磨き＋バグ修正5件をブランチ `fix/board-cursor-and-paper-meter` で出荷・実機OK**: ①テーマ選択モーダルの副題削除 ②カード上カーソル grab→pointer（盤面余白は grab、ドラッグ中は grabbing）③Paper メーター数字を静かな墨の目盛りに（`isRuler` 短絡で scramble/グリッチ停止）④🐛 テーマモーダルが盤面クリックで閉じない修正（capture-phase pointerdown＝grab-wiggle の setPointerCapture がバブルを食っていた）⑤🐛 ボタン直前で手のひらが一瞬出る修正（カード外側ラッパーに cursor:pointer 継承漏れ）。全て tsc0 / vitest1858 / build OK。マージは s150 で実施。

## セッション 151 — N-15 拡張の再起動後・初回保存失敗を根本修正（TODO.md から移設）

- **確定した真因＝offscreen→iframe のコールドスタート race**。保存トリガ → SW → offscreen 文書が `allmarks.app/save-iframe` へ postMessage。だが iframe（SaveIframeClient.tsx）は `message` リスナーを `useEffect` 内＝React ハイドレーション後に張る。`window.postMessage` は未登録の窓宛てだと**破棄（バッファ無し）**なので、再起動直後の初回 post が消える → 8s timeout → dispatch.js が offscreen を閉じて作り直し → 遅延 or 失敗。2回目以降は温まって成功＝「初回だけ失敗」の正体。3視点ワークフローで敵対検証。
- **修正（拡張のみ・web 無変更）**: offscreen.js が envelope を **250ms ごと再送**、iframe が返信（nonce 解決）or 8s 期限まで（新 `extension/lib/offscreen-repost.js` の `startRepostPump`＋純関数 `repostTickDecision`）。保存は nonce 重複排除・add-tag は冪等 → 再送しても二重保存なし。敵対レビューで blocker 0。
- 検証: 新テスト8＋全体 vitest 1875 緑 / tsc0 / offscreen.js は `node --check` 緑。拡張 **v0.1.23** 再パッケージ→Chrome ストア提出済。master `49ed5a9`。memory `reference_extension_offscreen_iframe_ready_race`。
- 副次（低確度・未対応）: コンテキストメニューは `onInstalled` のみ生成で `onStartup` 無し（実害薄）。任意の将来: 保存不能時の自動リトライ保存キュー。

## セッション 153 — N-19 実機OK＋SETTINGSフェード修正出荷 / N-05 ブレスト開始（TODO.md から移設）

- **前半＝N-19 実機確認OK ＋ フォローアップ修正**（master 反映済）：SETTINGS スクロールフェードが下端に固定されず特定項目に黒帯が貼り付くバグを修正（内側 `.drawerScroll` 方式・master `365ddd4`・Playwright 実測で dev＋本番合格）。narrative は s152 エントリ末尾参照。
- **後半＝N-05（LP ヘッダーナビ格納演出）のブレスト開始**。仕組み＋演出3段を確定し、スクロールできるプロトタイプ（`dock-proto-v2.html`）まで作成。確定：対象＝5サブページ、動く要素＝既存 kicker（緑玉＋ページ名）、演出＝①ガラス帯で1文字ずつ乗り→②「しゅっ」と右へ→③少しバウンド。詳細 IDEAS.md N-05。（実装は s154 偽保存→s155 作り直し→s156 ブラッシュアップで完遂）

## セッション 156 (2026-07-03) — N-05 ブラッシュアップ v2+v3（3段直列＋境界マイクロ演出）完遂・マージ・本番反映

**ユーザー実機フィードバックを2巡受けて N-05 を刷新し、`--no-ff` で master マージ（merge `782dcf6`）・`allmarks.app` 反映・実測 PASS でクローズ。LP はいったんここで区切り（ユーザー判断）。**

**v2 = 3段直列の変身（spec §9）**:
- 帯に乗っている間は**本文の姿のまま**（スクロール連動の無段階モーフ廃止）→ ナビの行の高さで止まり**左→右の衣装替えの波**（1文字240ms・30ms間隔＝時間制）→ その後の**横移動はスクロール駆動**（`holdPx:160` とどまり → `dashPx:140` を easeOutBack `dashBack:0.65` でスクラブ・逆走で同じ道を戻る完全可逆）。
- **docked 状態と時間制 zip/return を廃止**。帯上の位置は anchorTop の純関数（`dashProgress`/`dashEase`）。帯を離れる帰還は横ズレ0の垂直移動のみ＝**「斜めの軌跡」を根治**。
- **着地形はナビ実体と完全一致**: 実測し直しで s155 の「uppercase + ls0.06em」が誤りと判明 → **"Features" 混在ケース・-0.005em・14px w450**。衣装替え時に per-char で `text-transform:none` も掛け替え。`SiteHeader .dockSlot` の誤上書きを削除して navLink 継承に。**既知の許容差**: per-char span 化でカーニングが文字境界で効かず幅+1.88px（実物と同時表示されないため不可視・記録済）。
- 寸法モーフ transition は `.txt` に移設（word の inline transition は left/top 専用＝競合解消）。

**v3 = 境界のマイクロ演出4点（spec §10・ユーザー全案採用）**:
- ①**乗り上がりのスクロール駆動化**: 従来の「実 kicker が消え文字が湧く」を廃止し、引き継ぎ瞬間は完全同姿（がたっの原理的根治）→ 帯に入るほど左から順に跳ねる（`bandClimbProgress`+`charHopArc`、per-char `--hop` を毎フレーム書き CSS が `translateY(calc(var(--hop)*-3px))` に整形）。**実測: DOM＝純関数の誤差 0.000px**。
- ②**hairline 屈折**: ヘッダー下端 y=64 を光学面として、横断中の文字だけ `--cut` で基層（線下）/`::after`（線上・+0.5px シフト・0.85）に clip 分割。静止観察可。
- ③**玉のノック**: 下向きに線へ触れた瞬間だけ squash＋リング一拍（`knockMs:360`・時間制は意図＝衝撃は完結すべき）。
- ④**境界線グロー**: 線のうち語の真上の区間だけアクセント緑で灯る（`crossGlow` 三角ピーク・traveler が兄弟要素で描画）。
- バグ2件を経路特定で修正: settling 中に armed へ抜けると境界演出の後始末が漏れる／キャンセル整定中の morphing 再進入で `data-cancel` 旗が残る（44b07c2）。

**検証**: tsc0 / vitest **1922** 全緑（nav-dock-math 35テスト）/ build OK / dev+本番 Playwright 実測 **13項目 PASS**（継ぎ目0.000px・とどまり凍結・ダッシュ可逆・定着0.00px・フォントパリティ・屈折clip・グロー・ノック・残骸ゼロ・armed復帰）。本番の初回計測FAILはデプロイ直後のキャッシュ切替前に走った race（再実行で全緑・HTML マーカーで確認）。

**教訓**: ①「ヘッダー中央線」等の言葉はユーザーと座標系がズレる＝止まる場所・着地形は実物基準で確認 ②s155 の kicker/ナビ実測値を盲信せず再実測して誤り発見（uppercase/字間）③検証スクリプトの閾値は「なぜその値か」を書く（カーニング+1.9px 等）。

**同セッション終盤＝LP 最下部「黒幕」の既存構造バグを根治**（master `9e7ea1b`・`allmarks.app` 反映済）:
- ユーザー報告「黒い部分の左右に白い矩形」→ systematic-debugging で調査。**過去デプロイ3世代（s153=N-05着手前 / s155 / 当日）で要素スタック・ピクセルとも完全同一**を実測し、今回の変更由来ではなく**以前からの構造バグ**と確定。
- 真因2つ: (a) 黒幕 `.finale`（z120・「ヘッダーごと全画面を黒にする」設計）が `.footer` の横 padding 40px の内側にあり左右の溝で fixed ヘッダーの半透明バー端が露出（サブページ） (b) トップLPは footer が `LandingPage .content`（z-index:1 の stacking context）内のため幕がヘッダー(z100)を**一度も覆えていなかった**（最下部でヘッダーのバー全体が黒の上に浮いていた）。
- 修正: ①`.finale` に `margin: 0 -40px`（全画面化する PC+motion 時のみ。モバイル/reduced-motion の静的ブロックは margin:0 で従来どおり） ②SiteHeader が `[data-footer-finale]` の rect.top<=8 で `data-finale-covered` を立て、CSS（同条件の media query 限定）で `opacity:0 + pointer-events:none` にフェード退場（上へ戻れば復帰・transition 0.25s）。
- 検証: dev+本番の両方で `/` と `/features` の四隅が黒幕で覆われ横スクロールなしを実測 PASS／N-05 回帰7項目 PASS／tsc0 / vitest1922 / build OK。
- **教訓**: stacking context の中の z-index は外の fixed 要素と比較できない（`z=120 > 100` でも context が z1 なら負ける）。「覆う」系の設計は覆う側でなく**退場する側**の実装が文脈に強い。バックグラウンドタスクの exit 0 + 空出力は信じず前面で再実行（偽成功対策の実践）。

---

## セッション 157 (2026-07-03) — 選択的シェア (SELECT CARDS) 出荷・master マージ済・本番反映＋実測 PASS

**ゴール**: 共有は「今の絞り込み・新しい順100枚固定」だけで、545枚ユーザーが共有カードを意図して選べなかった。SHARE モーダルから「選択モード」に入り、選んだカード(最大100枚)だけを共有できるようにする。

**ユーザー相談で確定した仕様**: ①入口は SHARE モーダル内の SELECT CARDS(ヘッダー常設ボタンは作らない)。既存の「押したらすぐ100枚共有」は無変更 ②選択モードは**空っぽから足す**(現行100枚プリセットはしない) ③100枚上限は維持。

**進め方**: brainstorming → spec → plan → **サブエージェント駆動6タスク + 各2段レビュー(実装レビュー＋fix後の再レビュー) + opus 全ブランチレビュー(Ready to merge)** → `--no-ff` マージ → デプロイ → Playwright 本番スモーク。正本 [spec](superpowers/specs/2026-07-03-selective-share-design.md) / [plan](superpowers/plans/2026-07-03-selective-share.md)。

**実装(master merge `1aaeb37`・tsc0 / vitest1942 / build OK・default 盤面 byte-identical・本番反映済)**:
- **Task 1** `lib/share/selection.ts`(純関数12テスト): `toggleSelection`(上限で追加拒否・削除は常に可)/`addAllVisible`(可視集合を盤面順で上限まで)/`selectedInBoardOrder`(盤面順に写像＝クリック順でない)。上限は `SHARE_LIMITS_V2.MAX_CARDS`(マジックナンバー100を書かない)。
- **Task 2** `ShareSelectBar`(下部固定バー・5テスト): `n/100 SELECTED` + SELECT ALL + SHARE(n)(0で無効) + CANCEL + 琥珀「100 MAX」pill。`BOARD_Z_INDEX.SHARE_SELECT_BAR=115`(TOOLBAR:110の上・POPOVER:120の下)。レビュー指摘で **cap pill は実際の bump 時だけ点滅**(prevCycleRef を受領値で初期化＝非0マウントでも点かない)に修正、pill 数字も定数由来化。
- **Task 3** CardsLayer 選択モード: tap=選択トグル(`handleSelectPointerDown`＝receiver ハンドラの意図的ツイン)、選択中は Lightbox/並べ替え/リサイズ/ホバーchrome(×/↺/+TAG/タグpill行/再生トグル)を**全て非マウント**、選択カードに緑✓バッジ+緑アウトライン。レビューで2つのバグを摘出→修正: ①+TAG/タグpill行が未ゲート(spec §2 違反) ②再生トグルは `visible` prop でしか隠れずボタンがクリック可能なまま(`.interactive` の `pointer-events:auto`)→タップが選択を奪って再生切替する実バグ。両方マウント自体をゲートして解消。
- **Task 4** SenderShareModal に SELECT CARDS ボタン(idle 時のみ・3テスト) + `selectionActive` で hint 文言切替(「SELECTED CARDS ONLY…」)。
- **Task 5** BoardRoot 配線(opus レビュー): 選択 state・Esc=CANCEL・`buildShareData` の source/filter 分岐(選択時は `selectedInBoardOrder(items,…)` で全盤面から盤面順・`filter:null`・タグ帯なし)・選択集合の skyline 再計算プレビュー(モーダルローカルスクロール)・下部バー mount。**受け取り側 /s/ は無変更**(既存 v2 ペイロード)。normal path は全分岐が旧コードに一致(byte-identical を代数的に再検証)。
- **Task 5 追加 fix(最終レビュー指摘)**: オンボーディング共有シーンのクローザーが `shareSelectedIds` を破棄していなかった(spec §1「モーダル閉で選択破棄」との不整合・理論上の残留選択)→閉じるときに破棄追加。

**本番実測(allmarks.app・Playwright 12/12 PASS)**: seed-demos 6枚投入→盤面→SHARE→SELECT CARDS→下部バー(0/100・SHARE無効)→カードtap(カウンタ1・緑✓バッジ・SHARE有効)→SELECT ALL(6/6)→SHARE(1)で選択プレビュー再開。カードtapは合成ポインタでも動作(選択ハンドラの tap window が受理・reorder の pointer-capture とは別経路)。console エラーは TikTok/IG oembed の CORS/404(デモ内容の既知制約)のみで機能由来ゼロ。スクショで緑アウトライン+✓バッジ+ガラス調バーの視覚も確認。

**教訓**:
- **rtk git log はマージコミットを隠すことがある**。`--no-ff` 直後 `rtk git log` が branch tip(単一親)を HEAD に見せ「FF された?」と誤認 → 生 `git log --graph` で 2親のマージコミット `1aaeb37` を確認。**マージ検証は生 git を信頼**(偽保存対策の一環)。
- **plan の code snippet を鵜呑みにしない**: Task 2 の cap-pill テスト(タイトルと中身の不一致)・Task 3 の chrome ゲート漏れ(2箇所)・再生トグルの pointer-events カスケードは、いずれも plan のサンプルコードに起因。レビューが摘出して修正。
- **spec §2「全chrome抑止」は列挙を実コードと突き合わせる**: `!receiverMode` ブロックが実際は3つあり、plan は1つしか触れていなかった。

### セッション 157 後半 — ロードマップ整理＋実機フィードバック反映＋オンボ改善 spec/plan（実装は s158 へ）

- **ロードマップ Artifact**: 「やること全体像」を1枚に（今動くもの／本命3つ＝③テーマ・④K3・タグ／収益ローンチ流れ／バグ／アイデア箱）。大画面閲覧用。
- **ユーザー実機フィードバック反映**: **N-15**（拡張の初回保存失敗）＝ユーザー「終わってます」で解決扱い／**拡張は Chrome ストア審査通過**（残＝`EXTENSION_STORE_URL` 投入のみ）。新規バグ3件を backlog 追加＝**N-20** 拡張クイックタグが上だけ2列（原因確定：[floating-button.js:453](../extension/floating-button.js#L453) `tagstripSplit(tags, 2)` が上位2タグを横並びプレビュー行に。N-18 の1列化はドロワーのみ。直すと拡張再提出）／**N-21** オンボ SETTINGS 説明が開いたドロワーに埋もれる／**N-22** オンボに POP OUT 説明シーンが無い。
- **選択的シェアの詰め直し実証**: ユーザー懸念「選んだ位置を記憶してないか」に対し、飛び飛び選択（元は上段/下段に散在）→ 共有 → 実 `/s/` リンクを開き、受け取り側が **3枚とも最上段に左詰め（y=48）で再パック**されることを Playwright で実証。共有ペイロード（[ShareCardV2](../lib/share/types-v2.ts#L32)）に x/y 位置を持たない＝受け取り側が毎回 skyline を上から組み直す設計を確認。
- **オンボ改善 N-21+N-22 を brainstorm→spec→plan**: [spec](superpowers/specs/2026-07-04-onboarding-settings-popout-design.md) / [plan](superpowers/plans/2026-07-04-onboarding-settings-popout.md)（6タスク・TDD・完全コード）。N-21＝`settings` beat に `captionAtBottom`（下中央）。N-22＝`install` の後に `popout` cinema シーン＋新 `PopOutReenactment`（**事実確認**: 実 PiP はカードが**右からグライドイン→中央着地**＝[PipStack.tsx:145](../components/pip/PipStack.tsx#L145) `power4.out`/700ms・常時メーター。「上から落ちる」は当初の私の誤り→ユーザー指摘で修正）＋15言語コピー。**実装はセッション158へ**（セッションが重くなったため安全に持ち越し）。
- **教訓**: 再現アニメは推測でなく実挙動をコードで確認してから設計する（CLAUDE.md 推測抑制原則の実践）。

---

## セッション 158 (2026-07-04) — オンボ改善 N-21+N-22 出荷（サブエージェント駆動・master マージ済・allmarks.app 反映）

**成果**: チュートリアルの2つの穴を塞いで出荷（merge `28931b9`・`--no-ff`・tsc0 / **vitest 1945** / build OK・`allmarks.app` 反映済・default 盤面 byte-identical）。

- **N-21（SETTINGS 説明の埋もれ）解決**: `manage`/`settings` beat の `OnboardingSpotlight` に `captionAtBottom` を1行追加（[OnboardingController.tsx](../components/onboarding/OnboardingController.tsx)）。説明バブルが画面下中央に固定され、右側に開いた SETTINGS ドロワーと重ならず読める。`motion` シーンで実績のある prop の再利用。
- **N-22（POP OUT 説明シーンが無い）解決**: シーン列に **desktop 専用 `popout` cinema シーン**を `install` の後・`manage` の前へ追加（[steps.ts](../lib/onboarding/steps.ts)、`MOBILE_SCENE_IDS` には入れない）。新 `PopOutReenactment`（[.tsx](../components/onboarding/PopOutReenactment.tsx) / [.module.css](../components/onboarding/PopOutReenactment.module.css)）＝**実挙動どおりの純視覚再現**：ミニカードが**右からグライドイン→中央着地**（GSAP `power4.out`/0.7s＝実 PipStack の値）、下に**常時メーター**（`00/00`→`01/01`→`02/02`）。**実 PiP は開かず・PipStack/PipCompanion も import しない**（結合ゼロ）。`prefers-reduced-motion` 尊重（即着地・パルス停止）。コピー `board.onboarding.popout.body` を**15言語**に追加（parity テスト強制）。OnboardingController の cinema 分岐に `popout` ケースを配線（`share` の後・汎用 `OnboardingStage` return の前＝到達性確認済）。
- **進め方**＝`superpowers:subagent-driven-development`。6タスク＝各実装サブエージェント（haiku/sonnet）＋各タスク後の spec+quality レビュー＋最後に **opus 全ブランチレビュー（Ready to merge YES・Critical/Important ゼロ）**。
- **レビューで捕捉した回帰1件（修正済）**: Task 2 の `popout` 挿入で `OnboardingController.test.tsx` の manage walk テストが off-by-one（固定 NEXT クリック列が1回足りず manage の `settings` beat で停止＝残存 NEXT でアサート失敗）。**製品バグではなくテスト更新漏れ**→ walk に popout 分の +1 NEXT を追加（test-only・10/10）。他の walk テストは非影響（share は resume、motion/tag walk は extDemo 手前で停止）。
- **spec 準拠の仕上げ1件**: メーターが `{count>0}` ガードで空窓中（毎ループ約1s）消えていた（計画の例コード由来）が、spec §36/§55 は「常時表示メーター」を要求。ガードを外し**真の常時表示**（空窓中は `00/00`）に（`dc04767`・両レビュアー事前承認・1行）。
- 正本 [spec](superpowers/specs/2026-07-04-onboarding-settings-popout-design.md) / [plan](superpowers/plans/2026-07-04-onboarding-settings-popout.md)。
- **残**: ユーザー実機目視（`allmarks.app` ハードリロード→オンボを頭から。①SETTINGS 説明が下中央で読める②`install` の次に POP OUT が出て右からカードが入る③メーターが `00/00→01/01→02/02`）。**メーター空窓 `00/00` が気になる場合は1行で hidden-when-empty に戻せる**旨をユーザーに案内済。

### セッション 158 後半 — POP OUT 再現シーンを実機フィードバックで全面作り直し（v2・master マージ済・allmarks.app 反映）

ユーザーが v1 を本番実機でテスト → **2つの問題**を発見。**同一機能を作り直して出荷**（merge `ca81341`・`--no-ff`・tsc0 / **vitest1945** / build OK・`allmarks.app` 反映済・default 盤面 byte-identical）。

- **問題①＝詰まり**: POP OUT シーンで **NEXT が押せず盤面クリックがブロックされない**。真因をコードで確定＝オンボ土台 `.root` は `pointer-events:none`、各 cinema シーンは自分の `.stage` で `pointer-events:auto`＋暗幕 `rgba(6,6,6,0.97)` を貼り直す契約（[OnboardingStage.module.css:12-14] 明記）だが、v1 の `PopOutReenactment.module.css .stage` が**両方書き忘れ**→ NEXT が `pointer-events:none` を継承・盤面が透けてクリックを奪う（SKIP だけは `wrap()` 側で生きるので抜けられた）。
- **問題②＝品質**: 空の暗箱＋単語だけ。ユーザー指示＝**拡張チュートリアル（[ExtensionSaveReenactment.tsx]）と同じ「偽ブラウザ＋実LPスクショ＋自動で動く緑カーソルがクリックして見せる」方式に合わせる**。加えて**文言を淡々と他シーンに合わせ**、**タグ付けもデモに入れる**。
- **作り直し内容**（`PopOutReenactment` 3ファイル全面書き換え・Props 不変＝配線/`steps.ts` 無変更）: `.stage` に暗幕＋`pointer-events:auto`（詰まり解消の核）／偽ブラウザ＋実LPスクショ `/onboarding/lp-hero-shot.webp`／右上ナビに `POP OUT` ボタン／緑カーソルが POP OUT を押す（scale-pulse）→ 相棒窓が `back.out` でポップアウト → デモカードが右からグライドイン（`power4.out`/0.7s）＋常時メーター `00/00→01/01→02/02` → カーソルが「+ TAG」を押してタグチップ（design）が緑点灯。`prefers-reduced-motion` は静止終端。実 PiP は開かず import もしない（純視覚再現）。
- **コピー刷新（15言語）**: 「相棒」「どんどん」等を廃し他シーンと同じ淡々トーン＋**タグ付け＋カードクリックで AllMarks へジャンプ**を追記（実装で両機能を確認＝PiP `onOpenTags`／`handleCardClickFromPip` が `window.focus()`＋`booklage:focus-card`）。ja例「POP OUT を押すと、小さなウィンドウが開きます。保存したカードがそこに並び、そのままタグ付けもできます。カードを押すと、AllMarks を開いてそのカードに移動します。」
- **進め方**＝ユーザーと対話で storyboard/コピー/タグbeat を合意 → spec → plan（完全コード）→ サブエージェント駆動2タスク＋各レビュー＋**opus 全ブランチレビュー（Ready to merge YES）**。opus が Task2 の flash triage を訂正（大きい相棒窓が CSS 初期非表示でない＝兄弟より flash が大きい）→ `.pip`/`.card` に `opacity:0` を追加（`eff5fc3`）。
- 正本 [spec](superpowers/specs/2026-07-04-onboarding-popout-reenactment-v2-design.md) / [plan](superpowers/plans/2026-07-04-onboarding-popout-reenactment-v2.md)。
- **教訓**: ①cinema シーンの `.stage` は暗幕＋`pointer-events:auto` が必須契約（新規 reenactment を足すとき最優先で確認）②「兄弟に合わせる」＝兄弟を全部読んでから（[feedback_match_sibling_read_fully]）。v1 は兄弟の暗幕/カーソル/実UI流用を踏襲せず独自実装したのが両問題の根。

### セッション 158 末 — v2 実機フィードバックの追い込み修正4連（全て master 直コミット・allmarks.app 反映）

ユーザーが v2 を実機で見て出た指摘を1つずつ潰した（オンボは実クリック主体で Playwright 不安定＝unit＋ロジック＋ユーザー実機を gate に、1修正=1デプロイ=1確認で進行）:

- **`fb16eb8`（3件同時）**: ①POP OUT のタグチップがカードタイトルと重なる → `.chip` を左下→**左上**（+TAG の反対）。②SETTINGS beat で緑リングがヘッダー上の空箱に浮く（ドロワーが縦長化＝s152 以降で LAYOUT/THEME/HOW-TO/EXTENSION 増設・下スクロール状態で開き先頭の QUICK-TAG トグルがクリップ外へ）→ **forceOpen 時に `drawerScroll.scrollTop=0`＋`overflow:hidden` でトグルを上に固定＆スクロールロック**（[ExtensionEntry.tsx]）。③キャプション（下中央）の右半分がドロワー裏に隠れる → いったん `captionAtBottomLeft`（下・左寄せ）変種を新設。
- **`bf34335`**: ②③のうちキャプションが「トグルから離れすぎ」（ユーザー: 押させたいボタンに寄り添え）→ `captionAtBottomLeft` を廃し **`captionLeftOfHole`（穴の左に寄り添う＝`.bubbleBeside`、右端をトグル左28px手前に固定）** に作り替え。
- **`9305cbd`**: 「リングは必須」（ユーザー）→ 原因＝SETTINGS ドロワーは body portal で **z-index 401**（オンボ overlay=210 より上）＝リングがドロワー裏に隠れていた。**定数 `BOARD_Z_INDEX.ONBOARDING_SPOTLIGHT_RING=410` を追加、`ringAbovePanels` prop でリング `<div>` を body に createPortal（z 410＞401）**＝ドロワーは明るいまま、その中のトグルに緑リングが乗る（dim/caption は 210 のまま）。
- 各回 tsc0 / **vitest1945** / build OK / 関連テスト22 PASS。POP OUT チップ左上＋SETTINGS（リング可視・トグル固定・寄り添いキャプション）が揃った。
- **教訓（追加）**: 後から育った UI（縦長ドロワー・portal z-index）に、古いオンボ演出が追随できていないと崩れる。オンボの spotlight は **①ターゲットを可視域に固定（スクロール/クリップ）②portal パネルより前面にリング（z 順）③説明はターゲットに寄り添える** ように作る。
- **残**: ユーザー最終目視（概ね OK。気になる点あれば次回微調整）。

---

## セッション 159 (2026-07-04) — 拡張 N-20 完遂＋新規タグ作成 / オンボ PopOut ペースト / 高解像度は revert

### 拡張 N-20（クイックタグ「上だけ2列」）を根治し、さらに新規タグ作成まで
- 折りたたみプレビューが上位2タグ横並びだった問題を、「+ add tag」ハンドル1個＋ホバーで開く1列ドロワーに刷新（inline `tagstripSplit` 除去）。フォントをタグchipと一致（小文字/0.08em/85%）。開いたドロワー下部フェードがスクロール末尾で消えない典型ミスを `data-scroll-end`（transitionend+scroll 判定）で修正。commit `31e1092`/`eb2b5c2`。
- ユーザー要望で「+ add tag クリック→その場入力→Enter で新規タグ作成＋付与」を新設。web+拡張の往復 `booklage:add-new-tag`（`dispatchAddNewTag`→offscreen→save-iframe が `applyNewQuickTag` で find-or-create）。floating-button に入力UI（楽観✓チップ・連続入力・Esc/blur で戻る・入力中は自動閉じしない）。commit `958e255`。
- 敵対的レビュー2件（サブエージェント）で実バグ5件を摘出→全修正：①IME 変換確定Enter でタグ化（`isComposing`/keyCode229 でIMEに譲る＝日本語タグ全滅を回避）②重複タグ（save-iframe に nonceガード＋入力側に送信済みSet）③bookmarklet 経由の悪用（bookmark 存在時のみ作成＝`getBookmark` 追加）④keyup/keypress 漏れ⑤終了時ドロワー畳み＋破棄後の共有タイマー保護。tsc0/vitest1959。manifest 0.1.24・zip 生成・ユーザー審査提出。

### オンボ PopOut にペースト保存を追加
- PopOut シーンに「緑カーソルが窓へ→URL チップ＋緑フラッシュ（貼り付け）→カードが右からグライドイン」ビートを追加（案A・尺は伸ばさず）。キャプションを15言語更新（拡張もブクマも要らずリンク貼り付けで保存できることを教える＝無インストール層の最短導線）。reduced-motion/loop-reset 対応・テスト更新。commit `419fb4d`。

### アイデア洗い出し（X-01〜X-25）
- ユーザー「プロとして拡張にあるべき機能は？」に対し、5レンズ（競合パリティ/カードの絵/保存の摩擦/表現差別化/信頼と再訪）で並行洗い出し→MV3・ノーサーバー制約で実現性判定→統合、を workflow で実施。3階層＋「次の3つ」を IDEAS.md に記録。落とし＝動くヒーロー(canvas汚染)/音の波形(DRM)。

### 高解像度化（案X）を試みて revert
- Lightbox の X 写真だけ `name=orig` に上げる案を実装（純関数 `upgradeImageResolution`＋`HiResImage`）→ 表示時に新URLへ差し替えると FLIP 時に画像が未デコードで小さく表示される劣化が出た（ユーザー報告）。`6f4621d` でまるごと revert・本番は既知の良い状態に復帰。教訓＝見た目変更は tsc/vitest 通過≠OK、実機で見てから出す。

### 新規報告（次セッションへ）
- N-23：YouTube 動画→Lightbox で「がくっと小さくなる」。ユーザー「前はなかった」。次セッション(160)で systematic-debugging（新規か既存かを git/実機で検証→根本原因→実機確認して修正）。

---

## セッション 160 (2026-07-04) — N-23 動画Lightbox「がくっと」修正（実機OK）＋拡張 v0.1.24 審査通過（URLは既に配線済）

### N-23：YouTube 動画→Lightbox「がくっと小さくなる」を根治（commit `d05cc48`・本番反映・ユーザー実機「完璧」）

**進め方**＝systematic-debugging を厳守（Iron Law＝根本原因が固まるまで修正しない）。ユーザー明示の順＝①新規か既存かを git+本番で検証 ②根本原因 ③実機確認して修正。

- **Phase 1（原因調査）**: Lightbox の open FLIP を精読。clone（板カードのコピー）は `mediaEl.getBoundingClientRect()`（=`.media` の box rect）へ着地し、handoff で `.media` を可視化する構造。`.media` は `flex:0 1 auto`＋`align-items/justify-content:center` で子（poster box）を縦横 shrink-wrap → **理屈上は写真同様シームレスに着地するはず**で、静的コード読解だけでは症状を再現できなかった（＝憶測を重ねず実測へ）。
- **git 検証**: default（非paper）動画 Lightbox の関連コード（`.media`/`.embedPosterBox`/`.iframeWrap`/FLIP handoff・`deriveThumbnail`・`.media img{contain}`）は s130 台からずっと未変更。唯一の最近の動画関連変更は `4ddb38c`（s144, paper マット＋`[data-paper-window]`）だが VideoThumbCard のみで default 未影響 → **default は長期未変更＝新規リグレッションではない可能性が高い**。実際 [Lightbox.tsx](../components/board/Lightbox.tsx) の onComplete コメントが「YouTube はレターボックス不一致が一瞬見える（既知）」と自認していた。
- **ユーザー再現条件（質問で確定）**: テーマ両方／向き両方／**YouTube 動画だけ**（写真・他カードは滑らか）→ paper マット説・アスペクト固有説を否定、**YouTube 埋め込み特有**に絞り込み。
- **Playwright 実測で真因確定**（板が空・seed 破損・クリック pointer-capture の3障害を、YouTube を1件 IDB seed＋CDP 実マウスクリックで突破）:
  - 板カード＝`maxresdefault`(1280×720・**16:9**)・**object-fit:cover**・可視画像 268×151（枠いっぱい）。
  - Lightbox poster＝`hqdefault`(480×360・**4:3**)・**object-fit:contain**（`.media img{contain}` が `.embedPoster{cover}` を詳細度 0,1,1>0,1,0 で上書き）・box 888×500 に対し**可視画像 666.7×500（左右に黒帯 各約111px）**。
  - ＝clone が 888幅の maxres を枠いっぱいに育てた後、handoff で 4:3 の hqdefault が contain で入り**絵が幅 −25%＋低解像に瞬時に縮む**＝「がくっと小さくなる」。写真が滑らかなのは板↔LB で同画像・同 fit・計測アスペクト一致だから。
- **修正（3ファイル・41行）**:
  1. `YouTubeEmbed.tsx`＝poster を **板と同じ maxres→hq→mq→0 の onError 鎖**に（`posterLevel` state＋`handlePosterError`、`item.thumbnail` は無視して CDN 由来に統一）。板が settle したのと同じ鮮明な 16:9 画像へ解決。
  2. `EmbedShell.tsx`＝`EmbedPosterBox` に任意 `onError` prop を追加（poster img へ配線）。
  3. `Lightbox.module.css`＝`.media img[class*="embedPoster"]{object-fit:cover}` で**元々 cover 意図だった poster を cover に戻す**（詳細度で `.media img{contain}` に負けていた取りこぼしバグ）。`.imageBox` 写真（`.imageBox>img` で contain 明示）は無傷。
  - 結果＝「板の maxres/cover → Lightbox の maxres/cover」で一致 → 横も縦(Shorts)も、default も紙も直る。tsc0／vitest 実質全緑（既知フレーキー channel.test 再実行で緑）／build OK。
  - **ユーザー要望で Playwright 検証は省き、本番デプロイ→ユーザー実機確認**（「完璧でした」）。memory `reference_lightbox_youtube_poster_parity` 記録。**新規リグレッションではなく前から潜在していた既存の不一致**と結論。

### 拡張機能 Chrome ウェブストア v0.1.24 審査通過（ストアURLは既に投入済＝追加作業ゼロ）
...
## セッション 161 (2026-07-04) — ③ バックアップの法的守り A〜D 出荷（本番反映済・opus 全ブランチレビュー READY）

ローカル保存（IndexedDB）のみでサーバーにデータを持たない設計ゆえ、「端末を変えた／消えた」時にユーザーが自己責任でバックアップを取れる**法的・体験的な安全網**を整えた。ユーザー要望＝「法的に安全にしたい・両方（法的＋体験）」。

**進め方**：`superpowers:brainstorming`（範囲合意）→ spec → `superpowers:writing-plans` → `superpowers:subagent-driven-development`（フィーチャーブランチ `feat/backup-legal-safeguard`・7タスク・各タスクに実装haiku/sonnet＋レビューsonnet・最終 opus 全ブランチレビュー「READY TO MERGE」）→ `--no-ff` マージ `bb168f5` → deploy。tsc0 / **vitest1970** / build OK / default 盤面 byte-identical。

**調査で判明した事実**：利用規約（`/terms`、準拠法=日本/東京）は§2「データはブラウザ内のみ」§6「無保証・端末故障/その他の原因の消失に責任負わず」§7「責任の制限」で**免責は既に相当強い**。EXPORT/IMPORT は SETTINGS「SAVING」に配線済み・堅牢（rank3 部分復元防止）。`persist.ts` の `requestPersistentStorage()` は起動時 best-effort 呼び出し済。**欠けていたのは①初回の正直な説明②定期リマインド③規約の「自己バックアップ責任」の一文**。

**実装（A〜D）**：
- **A 利用規約**：§3 本文に「データは端末内のみ・控えを取る責任は利用者・端末変更/ブラウザ消去/故障/更新での消失は復元不可」を15言語追記（新節でなく既存 body 延伸＝`SECTIONS` 配列固定のため低リスク）。**本番 /terms で live 確認済**。文面は「当方」を排し既存規約と同じ register（AllMarks 主語＋受け身＋「利用者」）。
- **B 初回「データの住処」カード** [DataHomeCard.tsx]：オンボ finale 後（既存ユーザー/skip 者は初回ロード）に一度だけ。**ユーザー要望でポエムを排除し、チュートリアルと同じ淡々調**（「データはこの端末の中だけに保存されます。」）。「GOT IT」で `data-home-ack` に了解時刻を記録＝二度と出ない。離脱回避は「短く・明快・一度きり・オンボの延長」で担保。
- **C SETTINGS 表示** [BackupStatus.tsx]：「Last backup: N days ago / never」を SAVING グループに常駐（`BackupStatusView` 純関数＋自己ロード container）。
- **D 定期リマインド** [BackupReminder.tsx]：純関数 `shouldShowBackupReminder`＝**新規15件＋前回控えから30日＋dismissから30日**が揃った時だけ1回。LATER で `backup-nudge-dismissed-at` 記録。EXPORT で `last-backup-at` 更新＝差分0で自然沈黙。→「まめな人には出ない／溜めて忘れる人にだけ、たまに」。
- **共有ヘルパ** [lib/board/export-backup.ts]：`exportBackupFile(db, nowIso)`＝ダンプ→ダウンロード→`recordBackup`。SETTINGS ボタン／リマインド両方が最終バックアップ時刻を記録。BackupButton は旧インライン download を等価移設（既存テスト無改変で緑）。
- **純関数モジュール** [lib/storage/backup-reminder.ts]（13テスト）：`data-home-ack` / `last-backup-at` / `backup-nudge-dismissed-at` の read/write＋判定純関数（時刻注入）。定数 `BACKUP_REMINDER_NEW_THRESHOLD=15` / `BACKUP_REMINDER_DAY_GAP_MS=30日`。

**レビューで摘出・対応**：
- **Critical（T3）**：テストの `vi.fn<[Blob,string],void>()` が vitest4 で 2引数ジェネリック非対応→tsc で落ちる（`rtk vitest` は型検査しないので見逃す）。`vi.fn<Downloader>()` に修正（`814b2bb`）。この誤りは plan のサンプルコード由来。
- **T7 仕様逸脱（検証済み・正しい）**：brief は `items[i].savedAt` を仮定したが `BoardItem`（useBoardData の view 型）は `savedAt` 非保持（raw `BookmarkRecord` のみ）。effect 内 `db.getAll('bookmarks')`＋`!isDeleted`（use-board-data と同じ実削除フィールド）で件数算出に変更＝spec 意味論を正確に再現。
- **最終 opus レビュー Minor（must-fix）**：en の `dataHome.body` が "in Settings"（title case）で他14言語＋実ボタンの大文字 "SETTINGS" と不一致→ "SETTINGS" に修正（`c6d57f4`）。

**同期ビジョンを記録（別セッション送り）**：ユーザーが「サーバー無しで複数端末に持ちたい（課金対象でも可）」と発案。物理的に「両端末から届く共通の置き場所」が要る→(A)手動ファイル(今可)／(B)ユーザー自身のクラウド(Google ドライブ等・運営はデータもコストも持たない・思想維持・**本命**)／(C)運営サーバー(思想放棄)。非同時利用なので P2P 不向き。**案B**中心で専用セッション brainstorm 予定。骨子は `docs/private/IDEAS.md` の `(SYNC)` 節（既存 X-22/X-23/X-25 と紐付け）。今回のバックアップ=同期の第一歩。

**⚠ 公開前ゲート**：バックアップ機能の13言語（en/ja以外）翻訳、特に **Terms 法的条項は Claude のたたき台＝未レビュー**。ワイドローンチ前に**ネイティブ＋弁護士等の法務レビュー**必須（spec 冒頭に明記）。ru の `{days} дн.` は格変化省略。

**先送り（non-goal・記録済み）**：**E＝DB更新前の控え促し**（不可逆スキーマ変更の直前に自動 EXPORT 促し。技術負荷が高いため別セッション。spec §3）。

---

## セッション 162 (2026-07-05) — Mac実機バグ2件＋N-30出荷／★フラット化の方向性を確定

Mac 実機スプリントの一環。友人 Mac(Chrome) と本人シークレットで再現した保存不具合を systematic-debugging で根治し、PopOut のタグUI改善（N-30）まで出荷。さらにフラット化（default テーマの作り直し）の方向性を brainstorm で確定し親 spec に落とした。並行で端末間同期(案B)の1日スパイクをサブエージェントで実施＝緑。

### フルスクリーン保存の改善（N-39＋「シークレットでタグ窓が別タブ」）
- **根本原因＝Chrome 仕様**：macOS の Chrome はフルスクリーン中、`window.open(...,features)` の小窓要求を無視して**別タブ**で開く（[Bugzilla #803675] の同挙動・一次情報で確認）。拡張なし保存は経路B＝`window.open('/save?...','...','width=256,height=256,...')` を無条件実行するため、フルスクリーンだと 256 小窓でなく全画面タブ化＝友人の N-39・本人の「タグ窓が別タブ」の両方を同一原因で説明。**コードのバグではない**（推測でなくコード＋一次情報で確定）。
- **対応**：`/save`（=SaveToast）が自分の `window.inner{Width,Height}` で「タブとして開かれた」（`isOpenedAsTab`）と検知したときだけ分岐（純関数 `planSaveWindow` を拡張）。①PopOut あり=最短で自動クローズ（保存確認は PopOut のカードスライドインが担う） ②PopOut 無し・初回=中央の落ち着いたカードで「フルスクリーン中です／回避法（フルスクリーン解除・PopOut・拡張）」を1回だけ案内（`fullscreen-save-notice` フラグ IDB） ③PopOut 無し・以降=中央に「Saved」→約1.3秒で自動クローズ。フルスクリーン時のみタグ付け省略（あとでボードで）。普通の小窓は従来どおり（タグUIも維持）。
- **15言語化**：`/save` は I18nProvider 外なので `localStorage['allmarks-locale']` を `resolveInitialLocale()` で読み、自己完結 15言語コピー `lib/bookmarklet/save-fullscreen-copy.ts`（en/ja 確定・他13は Claude 初回訳＝ローンチ前レビュー対象）。製品名 PopOut/Chrome は非翻訳、GOT IT は英語維持（DataHomeCard に合わせ）。
- TDD＝`save-window-plan`（15）＋`save-fullscreen-copy`（網羅）＋`SaveToast`（タブ3モード）。tsc0/全1989緑/build OK。Playwright でタブ相当ビューポート・小窓・日本語表示を実測。commit `a3d53ed`/`ccae0f1`・本番反映。

### N-30 PopOut「+ TAG」をカード外へ
- カード左上に重ねていた「+ TAG」（透明文字＝明るいサムネで埋もれ・カード形で位置が動く）を撤去し、**PopOut 窓の上部中央・読みやすいピル**（`PipStack .addTagPill`・半透明暗背景＋細枠）に移設。対象は従来どおりアクティブ（中央）カードのみ。`PipCard` からボタン＋props を撤去、`pip-add-tag-button` testid は維持（PipCompanion フロー無傷）。/pip-tune にも `onOpenTags` を渡してピル表示。brainstorm→spec→TDD→Playwright 実測（上部中央 top10px・カード非重なり）。tsc0/全1993緑。spec [2026-07-05-pip-add-tag-outside-card-design.md]。commit `eab12f1`・本番反映。

### ★フラット化の方向性を確定（親 spec）
- brainstorm で確定：**白フラット・エディトリアルテーマを新default**（LP と呼応）／**現・暗い体験は「音波」テーマとして盤面を byte-identical に温存**（ユーザーが気に入っている）／**テーマが乗る範囲を盤面5項目だけに絞る**（背景/パターン・カード・メーター・カードモーション・明暗）／**メニュー類は全テーマ共通の中立 chrome に固定・例外なし**（paper のメニューも中立化）・大パネル（TUNE/SETTINGS/SHARE/テーマ）は右ドロワー統一、小ポップ（フィルタ/タグ）は据え置き／**カード角丸 ON/OFF ＋ N-35 つまみ**を customization 層へ／N-33 タグ表記はサブ④で確定／**N-27 は切り離し**。
- 機構は確認済みで素直に載る（`theme-registry` に1つ足して `DEFAULT_THEME_ID` 差替え／`theme-customization` に角丸等を追加／`isLightColor` が明るい盤面の chrome 文字反転を既に担保）。**「default 盤面 byte-identical」不変条件は本プロジェクトで意図的に更新**（暗い盤面は音波テーマで温存・メニューは全テーマ中立化）。
- 分解＝①テーマ境界＋メニュー中立化＋右スライド統一（次セッション）→②白フラットテーマ→③カスタマイズ→④音波命名＋タグ表記。親 spec [2026-07-05-flat-theme-and-theme-boundary-design.md]・commit `1212d1f`。

### 端末間同期(案B) 1日スパイク＝緑
- サブエージェント調査（一次情報引用付き）：**Dropbox＝PKCE 公開クライアント・トークン交換もファイルAPIもブラウザ CORS 完結・refresh token 有り＝自動同期可**で最有力。**Google Drive＝GIS トークンモデルでブラウザ完結・`drive.file` は非センシティブ＝重審査不要**だが access token のみ（1時間・手動同期寄り）。サーバーレス／アカウント無しの制約に両者適合＝**案B 成立**。結果は `docs/private/IDEAS.md` (SYNC) に清書。
...
**s166 追記（実機フィードバック1件・本番反映済・commit `d02b922`）**：ユーザー報告「コラージュでテキストカードを一定より小さくすると、いきなり何倍にも縮む」。systematic-debugging で根本原因を特定＝共有 `ResizeHandle` の `dom = |dx| ≥ |dyW| ? dx : dyW`（符号保持の支配軸）＋ `dyW = totalDy·signY·aspect`（縦移動をアスペクト倍増幅）。2軸が grow/shrink で食い違う方向にドラッグすると、支配軸が反対符号値へ切り替わる瞬間に幅が**不連続ジャンプ**（数値シミュ実測＝placeholder aspect 1.25 でも ~200px ジャンプ／幅広カードは縦40pxで MIN 崩落）。純math `lib/board/resize-math.ts` に `resizeWidthFromPointer`（2モデル：`'dominant'`＝デフォルト byte-identical・テストで固定／`'projection'`＝対角への連続投影）を切り出し、CollageCanvas は `resizeModel="projection"` を渡す。投影は「掴んで対角に引く」自然操作では dominant と数値一致（感触不変）、off-axis ではジャンプ消失。TDD 9テスト（連続性 assert 含む）。tsc0 / vitest 2035 / build OK。ジェスチャは setPointerCapture で Playwright 不可＝純関数テスト＋実機目視で確認。commit `d02b922`。

**s166 追記2（実機フィードバック2件・本番反映済・commit `df8637b`・deployment `2a10c466`）**：
1. **盤面のカードリサイズも同じ滑らかさに**（ユーザー「盤面でも同じ問題があるなら直した方がいい」）：`CardsLayer` の `ResizeHandle` にも `resizeModel="projection"` を付与。`resize-math` のデフォルトは `'dominant'` のまま温存。
2. **アレンジ画面がグリッドと違う並びから始まる問題を根治**（ユーザー慧眼「せっかくの盤面の並びを再利用せず再発明していないか」）＝**指摘は正しかった**。`handleEnterArrange` は `seedCollagePositions`（`computeSkylineLayout` を **viewport.w・y=0・左右/上パディング無し**で再パック）していたため、全選択でも盤面グリッドと一致しなかった。→ **WYSIWYG 化**：各選択カードの実 `layout.positions` を、**盤面カード層の生の画面原点**（描画済みカード1枚の `rect.left − layout.x` で実測）でシフトして seed。collage root は viewport 原点なのでピクセル完全一致。盤面は仮想化（`visibleItems.map`）するので、DOM を1枚ずつ測るのでなく **全選択カードを layout で置く**ことで画面外選択も脱落しない。**Playwright 実測＝盤面 vs アレンジのカード毎 maxΔx=maxΔy=maxΔw=0px**（layout＋静的オフセット案では 48px ずれ→実 rect 実測で container/stage オフセットを堅牢に吸収）。`seedCollagePositions` の import は BoardRoot から除去（関数は collage-layout に温存・テストで使用）。tsc0 / vitest2035 / build OK。**副次効果**：初期カードが上部クロム裏に潜る旧 cosmetic も解消（実 y を使うため）。ジェスチャ感触は実機目視。

**s166 追記3（実機フィードバック3件・本番反映済・commit `383f0b91` deploy）**：
...
### セッション167 追記2（実機ブラッシュアップ round 2・本番反映済 deploy d486acd7）

ユーザー実機フィードバック2件を修正（tsc0 / vitest 2051 / build OK・Playwright 2解像度で確認）:
...
## セッション 169 (2026-07-07) — SHARE 手動スクショの仕上げ＋COPY LINK（画像再構成なし）出荷

ユーザーが本番の配置(arrange)画面を見て2点を提起：①「シェア中…」バーがスクロールメーターの下に潜って読めない（実バグ）②スクショを撮って最小操作で共有する業界標準はあるか。相談の結果 **全環境で手動スクショ＋取り込みリンク(URL)併記** に確定（B＝PC自動キャプチャもレプリカ再構成も不採用）。

### 方式決定の根拠（調査で裏取り・うろ覚え禁止）
- **モバイルは画面キャプチャ API 全滅**：caniuse で `getDisplayMedia` は iOS Safari / Chrome Android / Samsung Internet いずれも Not supported。
- **canvas 系ライブラリは全部クロスオリジンで撮れない**：html2canvas / html-to-image / dom-to-image / SnapDOM いずれも CORS 無い他ドメイン画像で汚染＝ブラウザのセキュリティ境界でライブラリ非解決。他ドメイン iframe（生ツイート/動画）は原理的に写らない。
- **PC の `getDisplayMedia` は毎回許可が仕様で必須**（Web 標準・`granted` 権限は永続化不可。企業端末管理を除き一般ユーザーに抑制不可）→ 非エンジニアに不安・離脱の元。
- 結論：手動スクショ＋URL 併記が業界水準。X は画像添付時にリンクカードを抑制するので per-collage プレビューは無駄でもある。**レプリカ再構成（`captureMirrorToWebP`/`renderShareImage`）は完全排除**（バグの温床・ユーザー決定）。「画像添付の有無でプレビュー出し分け」は貼付がアプリ外で観測不能＝技術的に不可能 → **常に AllMarks 共通カード `/og.png`**。

### 実装（サブエージェント駆動8タスク＋各レビュー＋opus 全ブランチレビュー）
- **① メーター重なり解消**：純関数 `shouldShowScrollMeter(showOnboarding, sharePhase)`（[lib/board/scroll-meter-visibility.ts](../lib/board/scroll-meter-visibility.ts)）＝`!showOnboarding && sharePhase!=='arrange'`。配置中はスクロール不能でメーター(z400)無意味＋ SHARING バー(z116)の上に潜る件を解消。select 段では残す。
- **② 撮り方1行・OS判定**：`detectSharePlatform(ua, uaDataPlatform?)`＋`pickScreenshotHint(platform)`（[lib/share/screenshot-hint.ts](../lib/share/screenshot-hint.ts)・純関数・モバイル判定を先に）。ShareToast の従来ハードコード両OS併記を `hint` prop に置換。文言は globally-clear English。
- **③ COPY LINK（画像再構成なし）**：`copyShareLink(deps)`（[lib/share/copy-share-link.ts](../lib/share/copy-share-link.ts)・DI＝buildShare/createShare/writeClipboard/origin・**画像コード非 import**）。BoardRoot `handleCopyShareLink` は arrange 選択 `selectedInBoardOrder(items, selectedIds)`（**`shareSelectedItems` は arrange で null＝全件化の罠を回避**）から盤面順・`filter:null` で組み、**thumb 無しで** `createShare({share})` → `origin/s/<id>` をコピー。ShareToast に COPY LINK ボタン＋コピー状態（COPY LINK / LINK COPIED ✓ / COULDN'T COPY・1.6s で戻る・timer は ref で unmount クリーンアップ）。
- **サーバー2ファイル小変更**：[create.ts](../functions/api/share/create.ts) の thumb を任意化（`typeof thumb==='string' && length>0` の時だけ検証＋R2 put、無ければ KV だけ書いて 200）。[og.ts](../functions/api/share/[id]/og.ts) は R2 も KV thumb も無く**KV エントリは実在**する時 `/og.png` へ 302 フォールバック（genuinely-missing id は `encoded===null` で 404 維持）。`patch-share-html.ts` は `og:image=/api/share/<id>/og` を指したまま無変更＝透過的に既定カード。
- **④ 撮影ガイド**：配置突入で `.canvas` 縁を一度だけ緑 `box-shadow` グロー→フェード（`arrangeEdgePulse` 900ms・one-shot・`animation`/`box-shadow` のみ＝**レイアウト不変**・キーフレーム両端透明でスクショに残らない）。`.canvas` の `overflow:hidden` は自身の外側 box-shadow を切らない（既存 paper テーマが同 `.canvas` に 96px blur shadow を可視適用している実例で確認）。

### レビューで摘出・修正
- **Task 8 レビュー（Important）**：900ms 以内に RESELECT で arrange を抜けると `arrangeGuidePulse` が true のまま残り、再入場で className 不変＝グロー再発火しない → 非 arrange 分岐で `setArrangeGuidePulse(false)` に修正（再入場で replay）。
- **opus 全ブランチレビュー（READY TO MERGE）**：跨ぎ seam 3点 ✅（thumb-less create→KV→既定OG が接続／選択は selectedIds 由来で全件化しない／既存 SHARE 無回帰）。Minor「`buildShare()` が try/catch 外＝万一 throw でボタン凍結」を**その場で堅牢化**（copy-share-link.ts で buildShare を try/catch＋回帰テスト・fix commit）。二重クリック防止は非ブロッキングで送り。
- **Task 4 裁定**：`thumb:""`（空文字）を「thumb なし」扱いにする件は意図的仕様として確定（空文字＝画像なし＝既定OG・COPY LINK は thumb キー自体を送らない＝空文字を送る呼び出し元なし）。

### 結果
- tsc0 / **vitest 2072/0** / クリーンビルドOK（OG 注入アンカー assert 含む）。master へ `--no-ff` マージ・`allmarks.app` 反映済（deploy `b6c16360`）。
...
- **ユーザー実機目視の残**：メーター重なり消失・COPY LINK 動作・撮り方1行(Windows)・撮影ガイドがスクショに写らないか。

### 学び
- **設計質問は実コード確認まで即答しない**：手動スクショ採用理由（クロスオリジン汚染）を spec で確認 → 既存 OG サムネが `captureMirrorToWebP`（canvas 手描き再構成）だと判明 → 盤面カードは生 `<img>`（crossOrigin 無し・自ドメインプロキシ無し）で忠実自動キャプチャ不可、と実装本体まで追って確定。
- **`KVShareEntry.thumb` は型上すでに任意**（拒否はサーバー create.ts のみ）＝COPY LINK 無再構成に必要な変更はサーバー2ファイルだけ、と実測で分かった。
- **subagent-driven の要**：完全コード付きタスクは haiku 実装、統合/サーバー/配線は sonnet、全ブランチレビューは opus。ファイル受け渡し（task-brief / review-package）でコントローラ context を汚さない。

---

## セッション 171 (2026-07-07) — #3 タグ付け刷新 Phase 2+3（ドラッグ&ドロップでタグ付与 + インライン新規作成）

Phase 1（s170）で入れた TAG MODE（右端浮動パネル・複数選択機構の流用）に、実際のタグ付与機構を載せた。デプロイブロック（Phase 2 未完で MANAGE TAGS が空パネルだった問題）を解消し、本番反映済み。

### やったこと
- **純粋ロジックを先に切り出し + 単体テスト**: `lib/board/tag-assign.ts`
  - `resolveDropTargets(draggedId, selectedIds)` = 掴んだカードが選択に含まれれば選択全体、含まなければ掴んだ1枚だけを対象に。
  - `computeTagAssignments(cards, cardIds, tagId)` = 各対象に union（既存タグ保持・末尾追加）。既に付いてるカード/存在しない id/重複 id はスキップ。→ 9/9 PASS。
- **CardsLayer**: 選択モードの pointer handler を拡張。しきい値超えのドラッグで tag-drag を開始 → `document.elementFromPoint` で `[data-tag-id]`/`[data-tag-new]` を hit-test、対象行に `data-drop-hover`/`data-drop-count` を直付けして CSS が緑強調＋「+N」バッジを描画。カーソル追従ゴースト（カードアイコン＋枚数）は body 直下の imperative DOM（pointer-events:none で hit-test を邪魔しない）。pointerup で対象上なら `onTagDrop(key, cardIds)`。タップ（<5px/<200ms）は従来どおり選択トグル。selective-share は `onTagDrop` を渡さない＝ドラッグに入らずバイト等価。
- **BoardRoot**: `persistTags`/`createTag` を配線。`handleTagDrop` が実タグ id は即 `computeTagAssignments`→`persistTags`、`__new__` は `tagDraft` を立ててインライン入力を開く。`+ NEW TAG` クリックでも現在の選択に対して作成可能。作成時は色ローテーション（Triage と同じ 5 色）→ `createTag` → 付与。付与後も選択は維持（連続タグ付け）。
- **TagDropPanel**: 「Drag cards onto a tag →」ヒント、ドロップ強調 CSS（`data-drop-hover`）＋「+N」バッジ（`::after` + `attr()`）＋着地パルス（`@keyframes`）、`+ NEW TAG` を button 化してクリックでインライン input へ切替（Enter 確定/Esc 取消/blur 確定）。

### 検証
- tsc 0 / tag-assign 9/9 PASS（全体 vitest の 1 fail は BroadcastChannel の並列時のみのフレーク＝単体では 6/6 PASS・本変更と無関係）。
- Playwright（`out/` を静的 serve・seed-demos + IDB に tags 注入・モーダル抑止）で実測: TAG MODE 起動 → パネル描画 → **page.mouse でカードのタップ選択が通り**（setPointerCapture の壁は本 handler では未発生）→ **カードを work タグ行へドラッグ&ドロップ → IDB の bookmark.tags に `t-work` が入るのを確認**。ドラッグ中スクショで緑強調「+1」＋ゴースト「1 card」を目視。Phase 3 は input→Enter で新規タグが IDB に書かれるのを確認。
- 本番デプロイ済（`allmarks.app`）。
...
### 追記（同セッション・ユーザー実機フィードバック2件を修正・本番反映済）
- **バグ①（角丸OFFなのにライトボックス復帰で角が丸まる）を根治**：radius ミラー effect（[Lightbox.tsx](../components/board/Lightbox.tsx) `useLayoutEffect(…,[sourceCardId])`）が、close クロスフェードで `onSourceShouldShow` が `sourceCardId` を**着地の約150ms前に null**にする（[BoardRoot.tsx:1622](../components/board/BoardRoot.tsx#L1622)）ため、**モーフ途中で cleanup が発火し :root の `--card-radius`/`--lightbox-media-radius` を削除→globals の 20px にフォールバック→まだ飛んでいる復帰クローンが最後の数フレームで角丸**になっていた（角丸ON時は 20→20 で不可視、OFF(0px) で露呈＝今回の角丸機能が炙り出した）。修正＝**set と remove を分離**：set は `[sourceCardId]`（cleanup なし・nav で上書き）、remove は `[isLightboxOpen]`（`item===null`＝モーフ完了後のみ削除／unmount-while-open も cleanup で削除）。
- **バグ②（再生中の動画クリック→ライトボックス移行時に一瞬音あり）を修正**：`createLightboxClone` の cloneNode は**属性はコピーするが `<video>` の `muted` プロパティは引き継がない**（React は muted を属性でなくプロパティで設定）→ ミュート板動画のクローンが**ミュート解除で音付き autoplay**。open/close モーフのクローンで一瞬音が出ていた。修正＝クローンの全 `<video>` を `muted=true` に（映像はそのまま・音だけ消す。nav スナップショットが iframe を about:blank にするのと同じ意図）。YouTube 等 iframe は Tier1 が URL `&mute=1`、Tier3 は click 時 `setAudioActiveId(null)` で既に対処済み。
- 検証：tsc0 / vitest 2097/0 / クリーンビルド。**ライトボックスは Playwright で開けない**（`setPointerCapture` が合成ポインタ拒否＝クリックで開かない・テスト skip 実証）ため、モーフの角丸・音声は**実機目視が必要**。根本原因はコード追跡で確定。→ **ユーザー実機で①②とも OK 確認済み（s173 クローズ）**。

---

## セッション 174 (2026-07-08) — #8 共有画像の一時保管「画像＋リンクで SNS 投稿・X でも確実に画像表示」出荷（本番反映済）／#6 ライトボックス自動再生は見送り

### まず #6 を見送り判断（ユーザー合意）
ライトボックス自動再生（タグでスライドショー・動画は終了で次カードへ）は**要否低として見送り**。看板の「動画は終了で次へ」がきれいに作れるのは5種中2種（ネイティブ動画・YouTube）のみで、TikTok/Vimeo/SoundCloud は時間切れ代替＝一番コストの高い部分が一番不確実。中心はコラージュ閲覧/共有でメディア再生はズレる。再挑戦するなら静止スライドショーだけの軽版から。

### #8 完遂（ユーザー発案の流れ＋LoPo 実証レシピ）
ユーザーが手動で撮ったスクショを共有リンクに載せ、`/s/<id>` のプレビューが本物のコラージュになるようにした。狙い＝「画像で目を留めさせ、リンクで AllMarks に流入」＝バイラル導線の完成。

- **なぜ手動スクショか**: 盤面カードは外部オリジン画像（pbs.twimg.com 等）を crossOrigin 無しで読む＝dom-to-image/canvas 自動撮影はクロスオリジン汚染で真っ白（s169 が手動に倒した理由）。ユーザー自身のスクショは同一オリジンで汚染ゼロ＝唯一きれいな道。自動画像化（画像 proxy 新設）は手間＋プライバシー/帯域コストで対象外。
- **X で確実に画像を出す土台（LoPo=lopoly.app/FF14Sim の実証記録を移植）**: LoPo も「Discord は出る/X だけ小 summary」を経験。効いた修正は `twitter:card=summary_large_image`（既にあった）ではなく、**OG 画像を `/api/` の外の静的ファイル風 URL から、リダイレクトせず 200 で直接配信＋投稿前にキャッシュ温め**（コード内コメント「X クローラーが /api/ プレフィックスを嫌う」）。AllMarks は `/api/share/<id>/og`＋画像無し時 302 で両方の地雷を踏んでいた。
- **実装（TDD・8タスク）**: サーバー＝新ルート `functions/og/[id].ts`（`/og/<id>.jpg`・R2 SHARE_OG から 200 直接・無リダイレクト・ミス時は og.png バイトを 200 で・`immutable` 長期キャッシュ・拡張子剥がし）＋`patch-share-html` の og:image/twitter:image を `/og/<id>.jpg` に向け直し（height 630 統一）。旧ルートは後方互換で残置。保管路（create.ts・R2/KV）は無改造。クライアント＝`normalize-shot`（ユーザー画像→1200×630 cover→JPEG data-url・純関数 computeCoverRect テスト）、`create-hosted-share`（create＋温め）、`share-actions`（tweet intent＋Web Share ヘルパー）、`ShareToast` 3状態化（A: PASTE/DROP → B: サムネ＋CREATE LINK → C: LINK READY＋POST TO X）、BoardRoot 配線（arrange 中の document paste/drop＋hidden file input → 正規化 → createHostedShare → 各アクション）。**スマホ Web Share ボタンは今回外し、後日まとめて対応**（ヘルパーは温存）。
- **検証（全緑）**: tsc0 / vitest 全緑（新規テスト多数・既存 _handler/patch の URL 期待値更新／`channel.test` はフルスイート並列の既存 flaky で単独緑・無関係）/ クリーンビルド＋`assert-share-template` OK。
- **本番実測（allmarks.app）**: (1)`.jpg` ルーティングは preview で先行確認（scar 無し・`[id]` 動的セグメントは `.jpg` を捕捉）→本番デプロイ。(2)API 直投で実共有作成→`/og/<id>.jpg`=200 image/jpeg 直接・`/s` メタ=`/og/<id>.jpg`＋`summary_large_image`＋1200×630。(3)**Playwright で本番相手にクライアント全経路 E2E**（IDB seed→DataHomeカード dismiss→SHARE→SELECT ALL→ARRANGE→file-input で画像投入→正規化サムネ→CREATE→LINK READY＋POST TO X）＝STATE A/B/C 全 OK、実共有 `WziUuM` 作成、その `/og` も 200 image/jpeg 8532B の実 JPEG。
- **残る唯一の手動確認**: X に実際に貼って大きい画像カードが出るか（クローラー相手で自動不可）。それ以外の制御可能因子は全て LoPo 実証レシピに一致。
- 正本 [spec](superpowers/specs/2026-07-08-share-hosted-image-og-x-reliable-design.md) / [plan](superpowers/plans/2026-07-08-share-hosted-image-og-x-reliable.md)。

### s174 追補 — 実機フィードバック3点を修正・本番反映済
- **①角丸がコラージュに効かない（再発明の指摘）**：CollageCanvas は盤面のカード本体を流用済だが、角丸の式が CardsLayer と**重複**し片方が `roundedCorners` 分岐を落としていた（drift）。→ 共有純関数 `lib/board/card-radius.ts`(`cardCornerRadiusPx`) に一本化し両方が使用＋`roundedCorners` を CollageCanvas に配線。Playwright 実機で盤面/コラージュとも `--card-radius:0px` 確認。
- **②スクショにメニューが写る**：`HIDE TO SNIP` で**下の Share バーだけ**を隠す（他メニューは触らない＝ユーザー明示）。クリック/貼付/12秒で復帰。隠している間は自動フェードの微ヒント。実機で bar count 0・hint 1 確認。
- **③トンマナがAI/SaaSっぽい＋多言語**：アレンジバーを濃いガラス板＋光るピル＋破線チップ→**細いテキスト主体（上部クロム同系）**に。説明文だけ `getScreenshotHint(locale,platform)`（自己完結 locale マップ・15言語）で localize、ボタン語は世界共通英語のまま。実機で日本語説明文表示を確認。
- tsc0 / vitest 2131 / クリーンビルド / 本番 Playwright 実測 PASS。**微ヒント文だけは英語のまま**（transient・必要なら localize 可）。

---

## セッション 175 (2026-07-08) — SHARE 自動画像化の「実測」完了＝方向A（自動化全振り）確定（実装は次セッション）

**目的**：SHARE の手動スクショを無くせるか。唯一の技術的障壁はクロスオリジン汚染で、同一オリジン画像 proxy を通せば「本物の並べた画面」をそのまま dom-to-image で自動撮影できるはず。**実装前に全ブクマで実測して数字で判断**（ユーザー合意・s174 で方針化）。

**進め方**：推測せず実装本体を読んで proxy の fetch のやり方・カードの表示画像の決まり方を確定 → ユーザーのバックアップ JSON（非コミット）を全件サーバー側 fetch → dom-to-image で数枚実撮り。

- **カードの表示画像の決まり方（実コードで確認）**：`components/board/cards/index.ts pickCard` = youtube/tiktok→VideoThumbCard（`i.ytimg` 導出/oEmbed）、`item.thumbnail` あり→ImageCard、無し→PlaceholderCard（＝元から文字カード）。ImageCard の src 優先＝`mediaSlots[0].url`→`photos[0]`→`item.thumbnail`（[ImageCard.tsx:54-55,172-198](../components/board/cards/ImageCard.tsx)）。proxy の fetch 作法は `functions/api/ogp.ts`（ブラウザ相当UA・redirect 追従・着地再検証・`isBlockedHost` の SSRF 対策）。
- **実測（非削除 722 件）**：写真で撮れる **652（90.3%）**／取得失敗 **8（1.1%）**／元から文字カード **62（8.6%）**。画像持ち 660 中 **652（98.8%）成功**。ホスト分布は 82% が公開CDN（pbs.twimg 514・i.ytimg 30）、残りは website の og:image ロングテール。
- **失敗8件は全て「今の手動スクショでも既に壊れて写らない」もの**：404で画像消滅3（pbs.twimg 削除メディア/eklo-seven.vercel/vevaramotion）／framerusercontent 400×2／og:image が mp4 1（the-brandidentity）／HTML ソフト404 1（patterncraft.fun）／一時 429 1（uxsnaps・再試行で復活見込み）。→ **自動化が原因の新規劣化はゼロ**。hotlink 保護の 403 は corpus に1件も無く、Referer 再試行での回収も 0 件で不要だった。
- **実撮り検証（dom-to-image, 代表9枚）**：ローカルに同一オリジン proxy を立て、tweet3・YouTube1・website5（`http://animography.net` 含む）を `<img src="/img?u=…">` で描画→`dom-to-image-more` で PNG 化 → **9枚すべて本物の写真として焼けた**。汚染・空白ゼロ。「同一オリジンに通せば汚染が消えて写真が焼ける」を実証。
- **正直な限界**：fetch はローカルIP実行で Cloudflare edge と完全一致せず。82%の公開CDNは edge でも同一、website ロングテール(~115)は edge だと数件ズレ得る（データセンターIP弾き等）が、99%近い成功率のマージンで結論は動かない → 実装時に edge で数件だけ実地確認すれば十分。
- **判断＝方向A（自動化全振り）確定**（ユーザー「進めましょう」）。次セッションで①同一オリジン画像 proxy `functions/api/img.ts`（`isBlockedHost` 再利用）②SHARE 撮影時だけカード img を proxy 経由に③SHARE を「①選ぶ②並べる③作る」の1ボタン化（撮影モード/貼付/HIDE TO SNIP 撤去・既存 `create-hosted-share`/`/og/<id>.jpg` に接続）。
- 成果物（すべて `docs/private/`・非コミット）：測定ノート `2026-07-08-share-autocapture-measurement.md`／証拠画像 `2026-07-08-share-autocapture-proof.png`／実装メモは `IDEAS.md`「SHARE 自動画像化」§。**このセッションはコードを一切変更していない（測定のみ・docs 更新のみ）。**

---

## セッション 176 (2026-07-08) — ★SHARE 自動画像化 出荷（方向A：手動スクショ撤廃・撮影を自動化して1ボタン化）

s175 の実測（写真で撮れる 98.8%・自動化での新規劣化ゼロ）で確定した方向A を実装・出荷。SHARE の唯一の摩擦だった「OS スクショを撮って貼る」を撤廃し、**本物の並べた画面をそのまま自動撮影**する。

### 何を作ったか
- **同一オリジン画像 proxy `functions/api/img.ts`（新規 Pages Function）**：`?u=<url>` をサーバー側 fetch → bytes を `image/*` で返す。SSRF 対策は既存 `functions/api/ogp.ts` の `isBlockedHost` を再利用。raster allowlist（SVG は XSS リスクで拒否）・16MB 上限・リダイレクト着地ホストの再検証・`Cache-Control: immutable`・`X-Content-Type-Options: nosniff`。テスト付き。
- **`lib/share/proxy-image.ts`**（pure）：cross-origin http(s) 判定 + `/api/img?u=` URL 生成。**`lib/share/capture-collage.ts`**：dom-to-image で本物の CollageCanvas を撮影 → 1200×630 に cover 正規化。20s タイムアウト（ハング耐性）、撮影失敗時は null → thumb 無し作成にフォールバック（共有を絶対に壊さない）。
- **`render-share-image.ts` 拡張**：`bgColor`（JPEG の隙間/余白を地色で塗る）と `rewriteImageSrc`（dom-to-image の `adjustClonedNode` で **clone の `<img src>` だけ** proxy に差し替え＝ライブ DOM 無改変・フラッシュ無し・React 干渉無し）。撮影時の内部 XHR が同一オリジンになりクロスオリジン汚染を回避（＝手動スクショが不要になった唯一の理由）。cacheBust は rewrite 時 off で CF エッジがキャッシュヒット。
- **SHARE を「①選ぶ ②並べる ③作る」の1ボタン化**：`ShareToast` を CREATE 1ボタンに簡素化（HIDE TO SNIP／BROWSE／貼付／snip-hint を撤去）。`BoardRoot` の手動スクショ関連 state/effect（貼付・shareBarHidden・paste/drop・screenshotHint・file input）を全撤去し、CREATE で `captureCollageShareImage` → 既存 `createHostedShare`／R2／`/og/<id>.jpg`（s174）へ接続。ready 状態＝ LINK READY + COPY LINK + SAVE IMAGE（撮影画像 DL）+ POST TO X + DONE。
- **取得失敗フォールバックは自動**：死んだ画像はアレンジ時点で既に ImageCard の onError→PlaceholderCard（文字）になっているので、撮影はそのまま文字カードを写す（新規劣化なし）。地色は pattern テーマ＝`resolvedCustom.boardColor`、それ以外＝`.canvas` の `--bg-dark` を実測（default=near-black／paper=parchment 両対応）。
...
### 後半: スマホ専用ライトボックス（没入型）を新規実装・本番反映

スクロール修正の実機OK後、同セッションで本命の「スマホ専用ライトボックス」に着手（brainstorming → spec → plan → 実装まで一気通貫）。従来スマホでもデスクトップ用2カラム（左メディア/右テキスト）がそのまま出て窮屈だったのを、没入型に作り替え。

**確定した操作モデル（ユーザー承認）**: タップで主役を中央に大きく／**左右スワイプ=前後・下スワイプ=閉じる（指追従）・上スワイプ or 下端のつまみ=情報シート**（軸ロックで衝突なし）。開閉はPCと同じモーフ流用。「中央に大きく＝そのカードの主役（画像カードは画像、文字カードは文字）」、上シート＝補助情報（タイトル/説明/出典/翻訳）。

**アーキテクチャ（分離・再利用）**: 肥大した `Lightbox.tsx`(2263行) に詰めず新規5ファイルに分離。
- `lightbox-swipe.ts` — 純関数（`resolveAxis`/`resolveIntent`＋`SWIPE`定数）。**16 unit test**（実タッチ非依存の判定ロジックのみテスト）。
- `use-lightbox-swipe.ts` — pointer events 配線フック（実機のみ・純ロジックを消費）。
- `LightboxInfoSheet.tsx`/`.module.css` — 下端シート（つまみ常時 peek・上/タップで開く）。3 test。
- `MobileLightbox.tsx`/`.module.css` — 没入ステージ（主役中央大＋✕＋シート＋4方向ジェスチャ＋指追従transform）。3 test。
- `lightbox-nav-types.ts` — `LightboxNav` を共有 export 化（Lightbox から切り出し）。

**モーフ流用の要点**: PC の開閉モーフ（[Lightbox.tsx:993-1058](../components/board/Lightbox.tsx#L993-L1058) クローン方式、`R3F scene は SCENE_ENABLED=false`）は**着地先を `mediaRef.current` の実測rectで決める**。よってモバイルの主役ラッパ `.main` に**既存 `mediaRef` を付けるだけ**で、同じコードが中央大へ拡大／閉じで元スロットへ縮小。effect は無改造。

**配線**: `Lightbox.tsx` に `const isMobile = useIsMobile()` を足し、`.stage` 内を `isMobile ? <MobileLightbox .../> : <>{既存chevrons + frame}</>` の1分岐に。desktop の JSX は**そのまま包んだだけ＝byte不変**。`.stage` は `pointer-events:none` なので `MobileLightbox .stage` に `pointer-events:auto` を付与。

**種別対応の実態**: `LightboxMedia` が既に 動画（youtube/tiktok/vimeo/soundcloud）・一般サイト（画像/`LargePlaceholderCardScaler` の大テキスト）・instagram を適切に描画 → **画像・動画・サイト・文字カードは実装完了**。**ツイートは延期**（`LightboxMedia` はツイート非対応設計＝本来 `TweetColumns` 経由。①動画再生 ②複数画像ドット ③翻訳トグルは `useTweetTranslation` 共有が絡み、実機検証なしに組むと事故る＝推測抑制原則により実機フィードバック駆動に回した）。

**検証**: tsc 0 / **vitest 2176 全緑**（既存2154＋新規22）/ `pnpm build` OK。**ジェスチャ・モーフ・主役サイズ感は実機のみ検証可**（ユーザー合意で Playwright 不使用）→ 本番反映しユーザー実機確認へ。

**学び / 判断**: 「実装まで」の依頼でも、実機検証不能かつ複雑な部分（ツイート）を推測で作り込まず、機能する土台（画像/動画/サイト/文字）を先に出して実機の声で詰める、を選択（memory `feedback_fact_based`／CLAUDE.md 推測抑制5原則）。ジェスチャは s179→s180 一貫の「実機のみ検証」教訓を踏襲。

**spec/plan**: `docs/superpowers/specs/2026-07-08-mobile-lightbox-design.md`／`docs/superpowers/plans/2026-07-08-mobile-lightbox.md`。

### 追記: 実機フィードバック6ラウンドの磨き込み（同 s180・全て本番反映）

初回実装後、ユーザー実機フィードバックで反復。真因の多くは「デスクトップ専用要素（`.frame`/`frameRef`）へのゲートでモバイルが丸ごと bail していた」系。

- **R1**: 開くが一瞬・地味／背景が暗くならない → **開くモーフと backdrop フェードが `useLayoutEffect` 冒頭の `frameRef` ガードでモバイル全 bail**していた真因を特定。`mediaRef` 起点に修正し、メディアも `--lightbox-media-max-h` 全画面化。閉じも `requestClose` の `frameRef` 早期 return で無アニメだったのを mediaRef 起点に。
- **R2**: 開閉を Apple 風に（`*_MOBILE` イージング/尺、`back.out` overshoot 等）。閉じの穴（元スロットが空く）→ `back.in` 予備動作をやめ、縮み始めに元カード復帰。
- **R3**: ✕削除（画像に被る）／縦長画像がシート被り → `calc(100dvh - 76px)`／左右フリック強さで送り枚数＋慣性。
- **R4**: フリックが常に1枚＋強フリックで固まる → 完了判定を `view.url` 依存から**単調カウンタ**へ（重複URLで固まるバグ）＋フェイルセーフ。速度を**瞬間速度**に。開くを**クローン→transform-scale**（点滅・カクツキ解消）。
- **R5**: フリックが見た目だけで進まない → 各ステップが**古い onNav（古いindex）をキャプチャ**していた。**最新 onNav 参照**に。閉じも transform 化（点滅解消）。
- **R6**: キャプションを**2画面縦連結モデル**に（上スワイプでキャプション画面へ、カード超縮小して上、背景はPC同様ぼかし backdrop の上に透明）。`LightboxInfoSheet` 削除。

**教訓（memory `project_mobile_board_direction` 反映）**: ①モバイル分岐は「デスクトップ専用要素ゲートで全 bail」に注意（open/close 両モーフ・backdrop が該当した）②アニメ完了判定は content(view.url) でなく単調カウンタ＋フェイルセーフ（重複URL）③フリックは瞬間速度＋最新 onNav 参照④モバイルは transform-scale（クローンは点滅・reflowカクツキ）⑤全部実機のみ検証可。最終 tsc0/vitest2172/build OK。


---

## セッション 182 (2026-07-09) — ★スマホ専用ライトボックスの残り2つ（文字カード22px根治＋ツイート対応）を実装・本番反映

前提: s180-181 でスマホ専用ライトボックス（没入型・4方向ジェスチャ・キャプション2画面）は実機OK済み。残りは (A) 文字カード22px の取りこぼし修正 と (B) ツイート対応。両方 isMobile/tweetId 分岐＝デスクトップ回帰ゼロ。

### (A) 文字カード 22px を根治
- **原因**（コードで検証）: s181 の 22px 化は Lightbox.tsx mobile 分岐の `!view.thumbnail && shouldRenderLargePlaceholderCard(view)` 条件＝**サムネ無し経路だけ**を捕捉。サムネ持ちで小さい(<256px)/load 失敗するカードは `main=<LightboxMedia>` → `LightboxImageWithFallback` → `shouldFallback` → **`LargePlaceholderCardScaler` の zoom 拡大経路**（`inner.style.zoom = boxW/boardW` ≈ モバイル約3倍）を通り、巨大文字のまま未修正だった（＝ユーザーの「変わってない」）。
- **修正**: `LargePlaceholderCardScaler` に `useIsMobile()` を追加し、**モバイルは zoom effect を早期 bail ＋ `<div className={styles.mobileTextMain}>{fakeItem.title}</div>`(22px, `--text-primary`) を直描画**に。これでサムネ無し経路（LightboxMedia→Scaler）もサムネ失敗 fallback 経路（LightboxImageWithFallback→Scaler）も**1箇所で捕捉**。s181 の暫定分岐（mobile 分岐の main を `<LightboxMedia>` に単純化）と dead code `shouldRenderLargePlaceholderCard` は撤去。
- **検証**: tsc0 / vitest2172 / **Playwright(390px, native onClick で開く) で path A(サムネ無し=`TextOnlyCard`) と path B(壊れ画像 fallback=`BrokenImageCard`) の両方が font-size=22px** を実測。学び: テスト seed の example.com は sandbox で到達不可→背景 revalidation が `linkStatus:'gone'` を付け `handleCardClick` が dead-link でライトボックス開放をブロック→IDB で `linkStatus:'alive'` 上書きして回避。

### (B) ツイート対応（①動画 ②複数画像ドット ③翻訳）
- **設計（ユーザー承認）**: デスクトップの `TweetColumns`（動画/ドット/翻訳）は全て承認済み部品。これらを**モバイルの没入シェル `MobileLightbox` に載せ替え**る新規薄ラッパ `MobileTweetLightbox`（Lightbox.tsx 内）を追加。`useTweetTranslation` を1回呼んで main/caption 両画面に共有（＝TweetColumns がフックを持ち上げるのと同じ理由）。mobile 分岐は `tweetId ? <MobileTweetLightbox/> : <MobileLightbox generic/>`。
- **①動画**: `TweetVideoEmbed` に `fullBleed` prop 追加。デスクトップ `lightboxWrapper` は横に文字カラムを残すため 60vw キャップだが、没入モバイルは文字カラムが無いので `mobileWrapper`（`width:min(100vw, 100vh*aspect)`）で**アスペクト充填**。再生ディスク/タップ再生/playsInline は `variant='lightbox'` のまま不変。`TweetMedia` に `mobile` prop を通して両 video パスへ伝播。
- **②複数画像ドット**: `LightboxImageDots` に `mobile` prop＝`.mobileTweetDots`（メディア下部16px オーバーレイ）。デスクトップは `.media` 下 `bottom:-22px` にぶら下げ＝`.main` 基準では画面外に落ちるため。モバイルは左右スワイプ=カード送りなので**画像切替はドットタップ**（既存 `tweetSlotIdx`/`setTweetSlotIdx` 流用）。
- **③翻訳**: `TweetTranslateControls`（トグルボタン＋失敗通知）を抽出し TweetText と共有。`TweetText` に `hideToggle` 追加。写真動画ツイート=本文+トグルをキャプション画面。**文字ツイート=本文(22px)+トグルをカード画面に同居**（読む画面で翻訳・ユーザー決定）＝caption は著者+出典のみ。翻訳フック確認: `playEntry(el=null)` は `setText(finalText)` で即確定（テスト済）＝モバイル text-only で bodyRef 未接続でも訳文は反映（swap アニメだけ省略）。
- **CSS**: `.mobileTweetTextMain`（縦並び・翻訳ボタン `align-self:center`）／`.mobileTweetMediaMain`（position:relative でドット基準）／`.mobileTweetDots`（Lightbox.module.css）。
- **検証**: tsc0 / vitest2172（1件 flaky BroadcastChannel は単体で 6/6 緑）/ build OK / **Playwright(390px) で ②ドット2個＋main画像1＋文字ツイート本文22px＋ページエラー0** を実測。①動画・③翻訳の実動作は X の syndication/翻訳 API 依存＝**実機のみ検証可**（承認済み部品の再利用でリスク低）。

**commit**: (A) `b3098111` / (B) `d20832c8`。両方 `allmarks.app` 本番反映済。**残＝実機フィードバック**（動画サイズ感・ドット位置/当たり・文字ツイートの本文+ボタンの見え方・長文ツイートのカード画面スクロール不可＝Task A 文字カードと同じ既知制約）。**次セッション＝実機反映 → スマホのタグ付け（未着手）**。

### セッション 182 追記 — 実機フィードバック3件を systematic-debugging で根治＋角丸トグルのモバイル露出

ツイート対応・22px 出荷後、ユーザー実機で3件報告→ systematic-debugging（推測禁止・Playwright 実測で原因確定）で根治。

- **(1)(2) ツイートメディアが画面に入りきらない／上スワイプでキャプションを見ても見切れる**: モーフは `.main` を「ソース rect→自然サイズ」に transform-scale するだけ＝カードサイズは中身の CSS で決まると特定。差分調査で、**新ツイート経路（`.mobileTweetMediaMain` の bare img は `.main :global(img){max-height:100vh}`、fullBleed video wrapper も `100vh`）だけが `100vh` 制約**で、他のライトボックスメディアは `.imageBox` 経由で `--lightbox-media-max-h`（モバイル override＝`calc(100dvh - 76px)`）を使うと判明。実機の `100vh` はブラウザツールバー領域を含む→可視域からはみ出す。**Playwright 実測で確証**: 縦長写真ツイート `main=281×844`（上下端接触）に対し通常画像カードは `256×768`（上下38px余白）＝76px の差。**修正**: `.main img/video` の max-height と fullBleed wrapper を `var(--lightbox-media-max-h)` に統一。再実測で写真ツイートが `256×768` に＝通常画像カードと完全一致。非ツイートメディアは `.imageBox` で既に bound 済のため不変。
- **(3) ボードでテキストカード上だとスクロールが効かない**: `PlaceholderCard` の `.titleScroll`(`[data-card-scroll]`) に globals.css `[data-lock-card-scroll] [data-card-scroll] { touch-action:none }` が当たり（s180 由来）、指がテキスト本文に落ちると**ネイティブ board pan がキャンセル**されていた。s179 のコメントの意図（「指スワイプでボードをパン、カード本体は動かさない」）と実装が逆。`overflow:hidden !important` で内部スクロールは既に無効なので、`touch-action` を `none→pan-y` に（垂直スワイプが board scroller にフォールスルー）。`.titleScroll` は onScroll/onWheel のネイティブスクロール要素なので none 依存の JS ジェスチャは無い（安全）。**Playwright 実測**: テキストカード中心の hit chain が `[card-scroll]=none → =pan-y`。
- **(新規要望) スマホでカード角丸 ON/OFF**: デスクトップは TUNE ドロワーの CORNERS スイッチだが、モバイルに TUNE 導線が無く切替不能だった。**`BoardMobileNav` に CORNERS タブを追加**（MOTION と MORE の間・5タブ、`flex:1 1 0` で収まる）。既存の `roundedCorners` state / `handleToggleRoundedCorners`（`BoardConfig`・IDB 永続）にそのまま配線。アイコンは `<rect rx={rounded?4.5:0}>` で状態を可視化・緑=角丸 ON（MOTION と同パターン）。**Playwright 実測**: タップで `--card-radius` 14.4px⇔0px、data-active 緑⇔灰、リロード後も square 永続。

**commit**: 修正 `3fbb0009` / 角丸 `5b570b66`。tsc0 / vitest2172 / build OK / `allmarks.app` 反映済。学び: モバイルの高さ envelope は `100vh` でなく `100dvh - chrome`（ツールバー）を使う＝Playwright(viewport 固定)では露見せず、縦長メディア×実機でのみ出る。**残＝実機で再確認 → スマホのタグ付け（未着手）**。

### セッション 182 追記2 — スマホのタグ付け（下部の横スクロールタグ帯・タップ付与）

未着手だった「スマホのタグ付け」を実装・本番反映。brainstorming で意図を確認: デスクトップは「カードをタグ行へ**ドラッグ**」だが、スマホはドラッグ＝ボードスクロールで不適。ユーザーが「タグは画面下部で左右スクロール、と決めていたはず」と指摘→ IDEAS.md を探索: 書面の明確な決定は無かったが、Triage の「tag chip 下部並べ + 1-9 速選」決定と「タグ直接 multi 選択が最速」結論に整合する形として**下部の横スクロールタグ帯＋タップ付与**を提示・承認（spec `docs/superpowers/specs/2026-07-09-mobile-tagging-bottom-bar-design.md`）。

- **新規 `BoardMobileTagBar`**（mobile-only）: 画面最下部の固定帯。上段=`N SELECTED`／`DONE`、下段=**横スクロールのタグチップ列**（FilterPill 語彙＝中空ドット＋小文字＋3桁カウント・カプセルにしない）。先頭に `+ NEW TAG`。タグチップをタップ→選択カード全部に付与（緑フラッシュ）。選択ゼロ時はチップを薄く無効。
- **`CardsLayer` 変更**: モバイルのタグモードは **pointer capture しない**（capture するとネイティブ board scroll が死ぬ＋デスクトップのタグドラッグ ghost が誤発火）。`isMobile && isTagMode` のとき `onPointerDown` を bail（ドラッグ＝ネイティブ pan-y スクロール）、`onClick`(native) で `selectionToggle`。デスクトップのドラッグ付与・モバイル SHARE 選択は不変。
- **`BoardRoot` 配線**: `tagMode` で desktop=`TagDropPanel`／mobile=`BoardMobileTagBar`。`tagMode` 中は `BoardMobileNav` を非表示（帯が最下部を占有・集中モード）。新 `handleAssignTagToSelection(tagId)` は既存 `assignTagToCards`（additive・既付与スキップ・選択は残す）を流用。
- **検証**: tsc0 / vitest2172 / build OK / **Playwright(390px)**: TAG→帯表示＋ナビ非表示、カードタップで `N SELECTED` 更新、`+ NEW TAG` 入力→作成＋選択カードに付与（IDB tags 反映）、既存チップタップで別カードに付与、DONE で退出（ナビ復帰）。※横スクロールの感触・実機タッチの「タップ選択／ドラッグでスクロール」共存は**実機のみ**。学び: Next dev の開発インジケータが**左下のナビ／帯に重なる**→テストは `.dispatchEvent('click')` で駆動（本番にインジケータ無し）。

**commit**: spec `b740e45f` / 実装 `e7cf88c9`。`allmarks.app` 反映済。**残＝実機で帯の感触確認 → スマホ保存（束B）**。

---

## セッション 183 (2026-07-09) — ★スマホ・タブレット保存（束B）：賢い「+」ボタン＋入力シート＋Android 共有受け口を出荷・本番反映

**束B完了。** 現状スマホ/タブレットの保存導線は実質ゼロ（ブックマークレット=ドラッグ前提・拡張=PC のみ・share_target=manifest 宣言だけで受け側なし）だった穴を埋めた。brainstorming→spec→plan→**サブエージェント駆動8タスク＋各タスクレビュー＋opus 全ブランチレビュー（READY TO MERGE）**。`merge --no-ff`・tsc0 / **vitest 2188** / build OK / e2e 5本 PASS・`allmarks.app` 反映済。

### 決めたこと（ユーザーと相談して確定）
- **賢い「+」ボタン**（本命・全タッチ端末）: タップ→コピー済み URL を自動で読んで即保存、読めない時だけ入力シート。ユーザー要望「コピーしてきたら + 押すだけで勝手に追加」を `navigator.clipboard.readText()` で実現（iOS は Apple 仕様で「ペースト」確認が1回入る＝消せない・実質2タップ入力ゼロ、Android は初回許可のみ）。
- **端末の線引き（正直に説明・合意）**: Xの共有ボタンに AllMarks を出すのは **Android のみ可**（要ホーム追加）、**iPhone/iPad は Apple 仕様で不可**（Web Share Target 非対応）→ iOS は「+」が本命。B3（ホーム追加案内）は「アプリ化＝金かかる」懸念に対し「ホーム追加は無料の PWA・ネイティブアプリとは別」と誤解を解いた上で**後回し**に合意。
- **タブレット対応**: 「+」は `pointer: coarse`（タッチ端末）ゲート＝スマホ＋タブレット両方に出す。マウスの PC には出さない（**デスクトップ byte-identical**）。盤面レイアウトは触らずタブレットはデスクトップ表示のまま「+」だけ追加。
- **テーマ拡張性**: 見た目を CSS 変数（`--save-btn-*`/`--save-sheet-*`）＋`:root[data-theme-id]` 上書きで着せ替え可能に。将来の構造変種は `ThemeMeta.saveButtonVariant` の縫い代を予約（v1 未実装＝YAGNI）。

### 実装（3入口→1芯）
- **`normalizeToUrl`**（[lib/board/paste-url.ts](../lib/board/paste-url.ts)）: `https://` 補完＋検証の純関数。3入口共通。**ただし scheme 無しの裸トークンは「.」を含む時だけ domain 扱い**（`hello`→null→シート、`example.com`→保存）＝最終レビューの「単語がゴミ保存される」指摘をユーザー意図「URLでない→シート」に合わせて修正。デスクトップ貼り付けは従来 `extractSinglePastedUrl`（補完なし）のまま＝回帰ゼロ。
- **`useSaveUrl`**（[lib/board/use-save-url.ts](../lib/board/use-save-url.ts)）: 保存＋フィードバック＋onSaved の芯。`useUrlPasteSave` はこれを使う薄いラッパへ（公開 `{feedback}` 不変・PiP も無傷）。busy ガードの `preventDefault` 前チェック順序を復元して byte-identical 維持。
- **`useIsTouchDevice`**（[lib/board/use-is-touch-device.ts](../lib/board/use-is-touch-device.ts)）: `matchMedia('(pointer: coarse)')`。
- **`MobileSaveButton`** / **`MobileSaveSheet`**: 右下フローティング「+」＋下から出る入力シート（≥16px でiOSズーム抑止・無効時は開いたまま赤ヒント・reject 耐性）。
- **`BoardRoot` 配線**: タッチゲートで +/シート mount＋Android `?shared=true` 受け口（起動時に url|text を読み保存→`history.replaceState` でクエリ除去→リロード再保存防止）＋2 save インスタンスの feedback マージ。
- **i18n**: 新規 `board.saveInvalidHint` を15言語（入力欄 placeholder は既存 `board.urlPlaceholder` 再利用）。

### 検証と実機に残す部分
- tsc0 / vitest 2188 / build OK / e2e `tests/e2e/mobile-save.spec.ts` 5本（入力シート保存・無効ヒント・Android クエリ保存＋消滅・デスクトップで「+」非表示・タブレットで表示）。
- **実機のみで確認可**（自動不可・ユーザーの実機確認待ち）: クリップボード自動読み（iOS のペースト確認含む）・Android 共有メニューからの起動・タッチのタップ感・キーボード上のシート位置。
- 正本 [spec](superpowers/specs/2026-07-09-mobile-tablet-save-smart-add-design.md) / [plan](superpowers/plans/2026-07-09-mobile-tablet-save-smart-add.md)。

**残（次セッション）**: (1) ユーザーの実機確認（スマホ/タブレットで「+」保存・iOS ペースト確認・Android 共有）(2) 束Bが実機OKなら**公開日宣言可**→ 束C（13言語仕上げ＋規約正文条項）or 残りのモバイル磨き（ピンチリサイズ等）。**deferred backlog**: B3 ホーム追加案内 / iPhone ショートカット共有 / タブレット盤面最適化 / テーマ別「+」構造変種。

### セッション 183 後半 (2026-07-09) — ★PC盤面＋共有の磨き4点（①テキストカードのフェード ②共有受け取り ③CREATEインジケーター ④TUNEマージンスナップ）を出荷・本番反映

**束B（スマホ保存）を実機OKにした後、ユーザーが挙げた5件を subagent 並行調査で事実確認→整理→①②③④を実装（⑤Pinterest=N-28 は来週）。** brainstorm→spec→plan→**サブエージェント駆動6タスク＋各レビュー＋opus 全ブランチレビュー READY TO MERGE**。`merge --no-ff`・tsc0 / **vitest 2198** / build OK / 新 e2e 4本・`allmarks.app` 反映済。

- **① テキストカードのフェード（N-42）**: PC の醜いネイティブスクロールバー（薄く整えてはいたが `overflow:auto` の実バー＋下端のみフェード）を、**FILTER のタグ一覧で完成済の機構をそのまま流用**して両端フェード＋バー完全非表示に。純関数 `computeTagScrollEdge`（`none/top/middle/bottom` 4状態・`data-scroll-edge`）を `PlaceholderCard` の2描画箇所（通常＋paper-note）に配線。card は開閉アニメが無いので `maxHeight: clientHeight` を渡す。**ライトボックスの大文字カードにも波及（意図どおり）**。規模：小。
- **② 共有受け取り画面（N-43）**: ユーザーの「再発明してない?」に対し調査で**再発明していない**ことを確認（`SharedBoard` は本物の `CardsLayer`/`ScrollMeter`/`computeSkylineLayout` を既に再利用）。乱れは**移植し忘れ2点**だけ：(a) スマホ1列＝本物の「isMobile 時に3列・カード幅上書き（`MOBILE_LAYOUT`）」が受け取り側に無かった→`BoardRoot` の導出を逐行ミラーして幅/gap/customWidths を CardsLayer と spacer 両方へ。(b) PC メーターが古い＝部品は最新だが配置が s170 以前（canvas 内 absolute）→今の `frame.frameBottomChrome`（canvas 外の枠下帯）へ。**重要＝`CardsLayer` に isMobile を渡さない**ことで受け取り専用タップ処理（`handleReceiverPointerDown`）を温存（isMobile が receiverMode より先に判定される罠を回避）。規模：小〜中。**実機確認の残**＝スマホ受け取りの3列/タップ/削除/メーター。
- **③ SHARE「作成中」インジケーター（N-44）**: 100枚シェアで CREATE 押下後に進捗表示が消える件の原因特定＝唯一の進捗（ボタン「CREATING…」）が撮影対象 `.outerFrame` の中の `[data-no-capture]` にあり、撮影中 `[data-capturing]` の `visibility:hidden` で丸ごと消える（100枚＝画像100取得で数秒〜十数秒）。→ 新規 `ShareCreatingIndicator` を **`document.body` への portal** で出し、`shareCreateState==='creating'` の間だけ「CREATING YOUR LINK…」を表示（撮影クローンに写らず・非表示 CSS も回避・成功/失敗/RESELECT で確実にリセット）。`BOARD_Z_INDEX.SHARE_CREATING=500`。規模：小。
- **④ TUNE マージンスナップ（N-27）**: ユーザー実機で「5列にしたいのに離すと4列に矯正される」＝s173 実装の `snapToFill` が**全列数の最近傍候補**に飛ぶのが真因（当初調査の「吸着範囲2pxで届かない」は誤診）。さらに「中央寄せは絶対ダメ・左詰め維持・スナップ時に左右一致でよい」と確定。→ 純関数 `currentColumnCount`/`fillValueAtColumns`/`snapToFillAtCurrentColumns` を追加し、**今表示されている列数の fill 値だけ**に吸着（列数を変えない）。`FaderColumn` は候補マークを1本にし、**吸着範囲に入ると緑に太く光る**（`data-in-range`）＋しきい値を画面px基準（110px トラックに ~8px）に。中央寄せはしない（左詰めのまま）。規模：小〜中。**実機確認の残**＝吸着の効き・範囲UIの見え。
- **検証と学び**: フェーダー吸着は `setPointerCapture` で Playwright 不可＝純関数の単体テストで担保。②受け取りは `/s` の HTML シェル再現＋API モックで**実 `SharedBoard`** に対し 3列/メーター位置を回帰チェック（フェイクなし）。③は create ルートを hang させて indicator の可視を確認。**pre-existing の腐り発見（N-45）**＝旧 SHARE e2e 3本が消えた `share-composer` testid を参照（フル e2e で赤・本ブランチ由来でない）。旧 `fillCandidates`/`snapToFill` は本番未使用化（テストのみ生存）。
- 正本 [spec](superpowers/specs/2026-07-09-board-share-polish-batch-design.md) / [plan](superpowers/plans/2026-07-09-board-share-polish-batch.md)。**残（次セッション）**: 実機確認（②④中心）→ ⑤ **Pinterest 保存ボタン連動（N-28・優先度高）** or 公開関連。

---

## セッション 184 (2026-07-10) — N-46 共有受け取り画面のスマホ・スクロール不全を根治（実測で真因確定）

**症状（s183 実機FB）**: 100件を共有→スマホのプライベートウィンドウで `/s/<id>` を開くと、3列にはなるが**スクロールがさくさく動かない**。

**systematic-debugging Phase 1（原因究明）**: コード追跡3本＋実ブラウザ計測1本を並列実行。仮説（TODO の候補(a)）は的中したが、**推測で終わらせず computed style を実測**した。

- **真因**: `SharedBoard` は `CardsLayer` に `isMobile` を渡していない → `data-lock-card-scroll` が付かない（[CardsLayer.tsx:1304](../components/board/CardsLayer.tsx#L1304)）→ `.cardNode` が `touch-action: none` のまま（[CardNode.module.css:12](../components/board/CardNode.module.css#L12)）。受け取り画面の3列密グリッドは画面を覆うので**指は必ずカードに落ちる** → ブラウザのネイティブ縦スクロールが打ち消される。**s180 で本物盤面を直したのと同一のバグが、受け取り側だけ未修正**だった。
- **実測（実 Chromium・390×844・dpr3・hasTouch・ビルド済 `out/`）**: 受け取り `.cardNode`=`none` / lock属性 **0-of-100**、本物盤面 `/board`=`pan-y` / **51-of-51**。
- **`isMobile` 直渡しは不可**と判明: `hoverActive = !isMobile && …`（[CardsLayer.tsx:1287](../components/board/CardsLayer.tsx#L1287)）なので、受け取りの `×`（[:1453](../components/board/CardsLayer.tsx#L1453)）と送信者タグピル（[:1440](../components/board/CardsLayer.tsx#L1440)）がスマホで消える。加えて `if (isMobile) return`（[:1315](../components/board/CardsLayer.tsx#L1315)）が `receiverMode` 分岐より先に走り、受け取り専用タップ処理を飛ばす。

**修正（最小・回帰ゼロ）**

- `CardsLayer` に **属性だけを駆動する専用 prop `lockCardScroll`** を新設 → `data-lock-card-scroll={isMobile || lockCardScroll ? 'true' : undefined}`。`BoardRoot` は無変更（`isMobile ||` で従来どおり）。
- `SharedBoard` は `lockCardScroll={isMobile}` を渡す（**`isMobile` は渡さない**）→ `×`・タグピル・タップ開閉はすべて現状維持。
- `.scroller` を本物の `.mobileScrollContainer` に整合（`overscroll-behavior: contain` / `-webkit-overflow-scrolling: touch` / `touch-action: pan-y`）。
- **`setPointerCapture`（`handleReceiverPointerDown`）は温存**。W3C Pointer Events 3 の規範文「ビューポート操作（パン・ズーム）は pointer event の default action ではなく、キャンセルでは抑止できない／代わりに `touch-action` を使え」＋触りポインタは pointerdown 時点で**暗黙にキャプチャ済み**＝明示 capture は冗長。スクロール開始時は `pointercancel` が飛び、既存の `if (ev.type !== 'pointerup') return` が正しく弾く。

**検証**: tsc 0 / vitest **2201**（+3＝配線テスト `SharedBoard.mobile-scroll.test.tsx`）/ `pnpm build` OK / e2e `board-share-polish.spec.ts` 6本。**修正後の再計測で受け取り `pan-y` 100-of-100・盤面は 51-of-51 のまま不変**。e2e 回帰ガード2本（モバイル=`pan-y`／デスクトップ=`none`）は、修正を一時的に外すと確実に落ちることを確認済み。`merge --no-ff` `8d4495c0` → `allmarks.app` デプロイ済。

**学び（memory 化済み）**

- **CDP `Input.synthesizeScrollGesture`（touch）は実スクロールを再現できない**。old/new headless・headed・dpr1 の4モードすべてで scroll イベント 0 件、**既知の正解である本物盤面（`pan-y`）ですら動かない**。よって `none` と `pan-y` を判別できない＝合成での RED は不可能。**代わりに computed `touch-action` を assert する**のが正しい合成検証。
- **pointer capture はスクロールを止めない**（仕様一次情報で確認）。「capture がネイティブスクロールを壊す」というコード内コメントの主張は、スクロールに関しては誤り。

**残（実機のみ）**: ①受け取りが実際に滑るか ②受け取りは間引き無効（`UNCULLED_VIEWPORT_H`）で100枚全マウント＝残ジャンクの有無 ③スクロール中に触れたカードの `×`／タグピルが一瞬点灯する見え方。**新規 N-47**: タブレット（>640px の触り端末）は `lockCardScroll={isMobile}` の外＝同じ不全が残る（ユーザー判断待ち）。

## セッション 184 後半 (2026-07-10) — スマホ/タブレットの致命傷を連続出荷（N-47 / N-48 / N-51 / N-52 / N-45）

**N-46 実機OK後、ユーザーのフィードバックを受けて同セッションで5件を出荷・本番反映。**

- **N-47 タブレットのスクロール**: 判定を**幅ではなくポインタ**に（`lockCardScroll={isMobile || isTouchDevice}`＝`(pointer: coarse)`）。Chromium は `hasTouch` だけで 1024px でも coarse を返すと実測し、e2e 1024×768 で固定。
- **N-48 受け取りの下部 IMPORT バー**: **実測でスマホは IMPORT が 0×0（`.frameTopChrome` が `display:none`）＝取り込む手段ゼロ**、タブレットは 196×27px＝指の最小 32px も Apple 44pt も下回る。→ 触り端末に `ReceiverImportBar`（52px・`BoardMobileNav` と同素材・`TOUCH_BOTTOM_BAR=150`）。上部 IMPORT と ScrollMeter（掴む部分 360×18px）は触り端末では非描画。押下は既存 `handleSave` のまま。
  - **確立した規則＝「大きさは入力で決める。並べ方は幅で決める」**。一次情報で裏取り: Apple HIG 44pt / Material 48dp は入力基準、Google は `isTablet` 型のレイアウト分岐を明確に非推奨、Apple は WWDC で「iPad は Mac と iPhone の中間の縮尺物ではない」。**「触りにドラッグ式スクラブバーは不適切」と述べたガイドラインは存在しない**（探して無かったと報告）。
  - **副産物の発見**: **このアプリにタブレット用レイアウトは存在しない**。分岐は 640px だけで 744〜1180px は 1489px の PC と同一描画＝主要操作が全て 27px（→ N-50）。
- **N-51 スマホ盤面の余白**: モック5案から (c) を採用。`MOBILE_LAYOUT.SIDE_MARGIN_PX=16` / `GAP_PX 6→14`。**出荷後に実機で「受け取り画面が左に寄る」＝私の回帰**。真因＝`CardsLayer` は `position:absolute` で `.scroller` の padding を無視する一方、幅計算 `containerWidth` は padding を引く。→ `CardsLayer` を位置合わせ div で包んで根治（実測 左16/列間隔124/右16）。e2e で固定（両端を**同時に** poll する。半端に落ち着いた状態は left=16 のまま right が古い）。
  - **恒久ルール化**: ボードの見た目を変えたら受け取り画面も必ず確認する（memory `feedback_board_change_check_receiver`）。同種の事故が s183 N-43 でも起きていた。
- **N-52 パターンの太さスライダー**: 太さが**2箇所にベタ書き**だった（`patternSvgDataUri`＝受け取り/OG、`themes.module.css` の CSS グラデーション＝本物盤面/SHARE スクショ）。両方を `patternStroke` と純関数 `effectivePatternStroke`（`min(太さ, 間隔/2 − 1)`・下限0.5）に集約。**既定は 1px も動かない**（線1・ドット1.4、**パターン種別ごとの既定**）。DB バージョン上げ不要。共有スキーマは `.optional()` 必須。opus 敵対的レビュー＝SHIP・欠陥0。
  - **出荷後の実機FB**: グリッド/クロスハッチの**交点だけ濃くなる**（重ねグラデーションの二重合成）→ N-54 として記録。
- **N-45 掃除**: 消えた `share-composer` testid を参照する e2e 3本を削除（フル e2e 19 passed）。

**学び（memory 化済み）**
- CDP `Input.synthesizeScrollGesture`(touch) は実スクロールを再現できない（既知の正解=本物盤面すら動かず・4モードで確認）。合成では **computed `touch-action` を assert** するのが正解。
- **pointer capture はパンを止めない**（W3C Pointer Events 3 の規範文）。触りは pointerdown 時点で暗黙キャプチャ済み。
- ユーザー方針: **目視確認は本人に振る**（screenshot ループを回さない）。tsc/vitest/e2e と「測定でしか証明できないもの」は残す（memory `feedback_delegate_visual_verification`）。
...
**成果物（docs/superpowers/plans/）**
- `2026-07-11-s186-remaining-work-roadmap.md` — 正本ロードマップ（全残タスクの分類・セッション割り s187〜・モデル割り当て・恒久ルール）
- `2026-07-11-n56-mobile-share-image-fix.md` — ★★ブロッカー。調査で確定: 失敗の握り潰し13箇所（`withTimeout` null / `toJpeg().catch(()=>null)` / `thumb ?? undefined` / `{imageUrl && …}`）・ログ皆無・デバッグ導線皆無＝「静かに壊れて成功に見える」構造。対策=`captureCollageShareImageDetailed`（段階別 CaptureAttempt を持ち帰る）＋倍率フォールバック `[1]`＋一様色（真っ白）検出＋`fonts.ready` 3秒見切り＋結果シートに `NO IMAGE — LINK ONLY` と診断行。デスクトップはラッパ経由でバイト同一。実機診断の対応表（診断行→真因→恒久対応 F1〜F4）付き
- `2026-07-11-n58-mobile-collage-editing.md` — ★ユーザーの s185 決定撤回を受けた編集段の解禁（段階1）。`handleMobileCreateShare` を Enter（配置して止まる）と Capture（撮って作る）に二分割。新部品は帯ガイド `MobileBandOverlay`（box-shadow 減光＝写らない領域の WYSIWYG）と `MobileArrangeBar`（BACK/CREATE）の2つだけ。CollageCanvas は編集フル装備済み（s184/185 の資産）。RETRY が「編集を保ったまま撮り直し」に改善。**N-55 は結果シート下の透明スクリムで同時解消**。段階2（ピンチズーム）は実機の感触待ち
- `2026-07-11-n57-n59-mobile-board-polish.md` — N-57=`BoardRoot.tsx:3213` の `!isMobile` ゲート除去＋モバイル文字床 96→40px 提案（共有画像には写らない=デスクトップと一貫、を明記）。N-59=`BoardConfig` に `mobileColumns(2|3|4)`/`mobileGap(S/M/L)`（board-config JSON 塊＝DB bump 不要・medium=出荷値でアップグレード無変化をテストで固定）＋MORE パネル VIEW 節にセグメントボタン
- `2026-07-11-n54-pattern-svg-unification.md` — 盤面パターンを受け取り/OG と同じ `patternSvgDataUri` の単層 SVG に統一（useMemo 必須）。交点の二重合成根治＋dom-to-image のグラデ片方向落ちも同時解消。ShareMirror が SVG 背景撮影の実績あり=撮影は壊れない根拠つき。死んだ CSS グラデ規則4ブロックの削除まで
- `2026-07-11-n53-e2e-repair.md` — 病巣3つを特定: ①8ファイルが `/board` の後に `indexedDB.open(dbName, 9)`（v16 相手に VersionError。`[data-theme-id]` 待ちはプリペイント script が立てるので偽 readiness）②`networkidle` が dev サーバーで永遠に成立せず 30s 焼き切れ（11ファイル）③v13 addInitScript 系はインデックス欠落。対策=**版数を固定しない共有 seed ヘルパー**（アプリにスキーマを作らせ、無版数 open のポーリングで接続＝将来の DB bump 耐性）へ全ファイル移植

**調査での発見（記録訂正）**
- **(N-45) は完了済みだった**: 旧 SHARE spec 3本は commit `ac0b35da` で削除済み。TODO.md の記載が古かった（訂正済み）。
- (N-07) は N-53 の VersionError 群の一部＝修理に吸収。
...
## セッション 188 (2026-07-12) — ★N-56 スマホ共有画像の「診断可視化＋倍率フォールバック＋真っ白検出」を出荷・本番反映

**実行フェーズ第1弾。N-56（★★ローンチブロッカー＝スマホで共有画像が作られない・iPhone Safari・4枚でも）を計画書 [n56](superpowers/plans/2026-07-11-n56-mobile-share-image-fix.md) どおり subagent-driven-development で完遂・`allmarks.app` 反映済。** Task 1・2 は前セッションで完了済（台帳）、本セッションは Task 3〜5＋フル検証＋opus 全ブランチレビュー＋デプロイ＋ドキュメント更新。

### 何を直したか（設計）

スマホの撮影パイプライン（`captureCollageShareImage` → `renderShareImage` → `normalizeShotToJpegDataUrl`）は**全失敗を `null` に握り潰して無ログで進む**設計だった（共有を絶対に壊さないため）。この退路のせいで、撮影が失敗しても結果シートは無言で「LINK READY」を出し、**静かに壊れたまま成功に見えていた**（失敗の握り潰し13箇所・ログゼロ・UI無表示）。この設計は維持したまま、各段の失敗を持ち帰って**画面に見える化**し、倍率を落とす**自動再挑戦**で直る失敗をその場で救い、iOS の**真っ白な成功画像**を失敗扱いにした。

### 実装（Task 1〜5・全て Sonnet 中心／各タスクレビュー Approved）

- **Task 1** `lib/share/uniform-image.ts`（前セッション完了）＝`isUniformSample`/`isUniformImage`。真っ白（一様色）出力の検出器。canvas 不可環境では `false`（撮影を殺さない保守設計）。
- **Task 2** `render-share-image.ts`（前セッション完了）＝`onError?(message)` で失敗理由の持ち帰り口（未指定なら挙動不変＝デスクトップ・バイト同一）＋`document.fonts.ready` を `Promise.race` で3秒見切り。
- **Task 3** `capture-collage.ts`＝`captureCollageShareImageDetailed` ＋ `attemptCapture`（段階別診断 `no-frame`/`timeout`/`render`/`decode`/`blank`/`normalize`）＋ `fallbackScales` 連鎖 ＋ `formatCaptureAttempts`（例 `#1 x3.08 render 9000ms RangeError… / #2 x1 ok 2100ms`）。`withTimeout` を `TimedResult<T>` に変えて timeout と render 失敗を区別可能に。旧 `captureCollageShareImage` は薄いラッパ＝**デスクトップはバイト同一**（レビュアーが呼び出し元 BoardRoot で検証）。スコープ+: `normalizeShotToJpegDataUrl` を try/catch で包み `fail('normalize')` に（never-break-sharing 不変条件の潜在ギャップを塞ぐ・レビュアー確認済）。
- **Task 4** `MobileShareResult.tsx`＋`.module.css`＝画像だけ失敗（リンクは生存）を見せる **NO IMAGE — LINK ONLY** の琥珀警告枠＋RETRY IMAGE＋**1行の診断文字列**（`user-select:text` でコピペ可）。成功でもフォールバックが要った時は診断行を出す。リンク作成失敗時は理由行。琥珀＝「注意・でも壊れてない」（エラー赤は使わない＝リンクは生きている）。
- **Task 5** `BoardRoot.tsx`＝モバイル撮影を `captureCollageShareImageDetailed` ＋ `fallbackScales:[1]` ＋ `rejectUniform:true` に配線。`captureAttempts`/`shareErrorMessage` の state、`handleExitShareMode` のリセット、`MobileShareResult` への props 配線。**デスクトップ `handleCreateHostedShare`（`captureCollageShareImage` 使用）は無改変**（レビュアーが diff に出ないことを確認）。

### 検証・レビュー

- **フル検証ゲート**: tsc 0 ／ **vitest 2262/2262 全緑**（前セッションで見えた channel.test.ts のフレークは再現せず）／ `pnpm build` export OK（`assert-share-template` OK）／ **Playwright mobile-share 5/5**（"desktop SHARE — unchanged" 含む）。
- **opus 全ブランチレビュー（5f58ec31..870818c5・6コミット）＝ READY TO MERGE = YES**。Critical・Important ゼロ。Minor 5件は全て「計画で先送り済み or グレースフル劣化に限定」: ①RETRY IMAGE が丸ごと再作成（N-58 で撮影のみ再実行に）②診断行に proxy URL が出得る（F3 の切り詰め・現状は自端末/自データ/非送信で出荷可）③診断 `<code>` の2箇所重複（プレゼン用・DOM バグなし）④成功時の生エラー行（意図した実地診断）⑤`isUniformImage` の理論的偽陽性（link-only に劣化するだけ）。レビュアーが独立に確認: デスクトップ・バイト同一／`captureCollageShareImageDetailed` は絶対 throw しない（never-break-sharing 成立）／Task1〜5 が端から端まで合成する。

### 出荷

`merge --no-ff` で master 反映・`out/`（870818c5 ビルド）を `wrangler pages deploy --branch=master` で `allmarks.app` にデプロイ済。

### ★次セッション（最優先・ユーザー協働）

**実機で診断行を1回読む**。iPhone/Safari で `allmarks.app` → SHARE → SELECT ALL → CREATE → 結果シートの (A)〜(D) と診断行を報告してもらう。その1行で真因が確定し、恒久対応（F1 倍率／F2 timeout＋温め／F3 proxy 漏れ＋URL 切り詰め／F4 真っ白＝ユーザー相談）を1つ選んで実装。**(A)＝プレビューが出た なら N-56 は実質解決 → N-58 段階1 へ**。手順と対応表は [CURRENT_GOAL.md](CURRENT_GOAL.md) と n56 計画書 Task 6。

---

## セッション 189 (2026-07-12) — ★N-56 スマホ共有画像を「canvas 直描画」へ全面移行・出荷（iOS foreignObject 回避）

### 背景（s188.2 実機で確定した真因）

s188 系列で OOM タブクラッシュは画像縮小で解消したが、**6枚でも100枚でもプレビューが暗い＝写真が写らない**（枚数非依存）ことが実機で確定。原因は **iOS Safari の dom-to-image が SVG `foreignObject` 内の `<img>` を描画できない**（PC Chrome では出る＝iOS 固有）。小技では直らない＝**canvas 直描画（foreignObject 不使用）が唯一確実な根治**。ユーザー承認済（¥0・安全）で計画書 [2026-07-12-n56-mobile-canvas-renderer.md](superpowers/plans/2026-07-12-n56-mobile-canvas-renderer.md) を作成し、本セッションで subagent-driven-development により実装。

### 実装（Task 1〜4・各タスクレビュー＋opus 全ブランチレビュー）

- **Task 1**（Haiku）: `lib/share/capture-mirror.ts` の描画 primitives 6個（`roundRectPath`/`drawWrappedText`/`drawClippedText`/`loadCrossOriginImage`/`canvasToJpegUnderTarget`/`dataUrlByteLength`）に `export` を付与（ロジック不変・既存挙動バイト同一）。commit `de5d0312`。
- **Task 2**（Haiku・TDD）: 新規 `lib/share/collage-canvas-render.ts` に純関数 `coverRect(imgW,imgH,dstW,dstH)→{sx,sy,sw,sh}`（アスペクト維持の中央 cover 切り出し）＋`mapBandToOutput(pos,band,outW,outH)→{x,y,w,h}`（band 座標→1200×630 線形写像）。数値アサーションでレビュアーが手計算検証。commit `aa0e0ecd`。
- **Task 3**（Sonnet）: 同ファイルに `renderCollageCanvasToJpeg(input)→Promise<string|null>` 本体＝**DOM を読まず配置データから直接 canvas に描く**。全体 try/catch→null（絶対 throw しない）／画像は**1枚ずつ await**（100枚でも巨大 SVG を作らず OOM しない）／写真は cover-fit 9引数 `drawImage`（naturalSize 0 は 4引数 stretch fallback）／角丸クリップ／写真は `toProxyUrl`（同一オリジン proxy）経由でロード＝taint 回避／画像なし・失敗は `pickPlaceholderImage` の生成アート＋scrim＋中央タイトル／`allmarks.app` 焼き込み。commit `8502730c`。
- **Task 4**（Sonnet）: `components/board/BoardRoot.tsx handleMobileCreateShare` を canvas レンダラーに配線。`fitSelectionToScreen` の結果をローカルに保持→`chosen`＋positions から `canvasCards`（rect 欠落はスキップ）を構築→`renderCollageCanvasToJpeg({cards,band,1200,630,bgColor,roundedCornersPx,toProxyUrl,targetBytes:180KB,q:0.82→0.5})`。s188.2 の縮小 Map（`buildCaptureThumbnailMap`）と関連 import を撤去、**クラッシュ耐性パンくずの write/clear は安全網で温存**。角丸は median カード幅×(1200/band.width) の一律出力半径＋レビュー指摘で **flat をテーマ追従**（`themeMeta.decorations===true`＝paper 系は 3px）。commits `b66548e3`＋`7dadcd48`（fix）。
- **描画ループの自動テスト追加**（opus レビュー指摘 = 従来テストは canvas 不可で早期 return しか通っていなかった）: mock 2d context で①逐次ロード（Promise.all 退行検出）②proxy はサムネのみ・placeholder は直ロード③画面外スキップ④placeholder 分岐⑤ループ内 throw→null を実測。見せかけテストを是正。commit `1cb94794`。14/14（+5）。

### 不変条件（opus レビュアーが呼び出し元を直読して検証）

デスクトップ SHARE **バイト同一**（`handleCreateHostedShare`・`capture-collage.ts`・dom-to-image 経路は無改変で diff に不在／`capture-mirror` の diff は export 追加のみ）。撮影失敗でもリンクは必ず作る（レンダラー失敗時 null→`thumb ?? undefined`）。画像は同一オリジン proxy 経由（canvas taint 回避）。出力 1200×630 cover。opus 判定＝**Ready to merge・Critical/Important ゼロ**（1件の Important＝描画ループのテスト欠落は本セッション内で是正済）。

### 検証・出荷

tsc0 / **vitest 2291/2291**（276 files・flake なし）/ `pnpm build` OK（assert-share-template OK）。`merge --no-ff`（6 commit 履歴保存）で master 反映＝`9c40a6a2`、`out/`（1cb94794 と同一 tree）を `wrangler pages deploy --branch=master` で `allmarks.app` にデプロイ済。
...
## セッション 193 (2026-07-13) — ★スマホ縦4:5 段階2・第1弾（編集チロムの核）を出荷・本番反映

**同一セッションで段階1（縦4:5 土台）に続けて段階2・第1弾まで自走。** ユーザー指示＝「おすすめで全部進めてOK・後でまとめて確認」。superpowers:brainstorming（事実採取 Explore＋選択肢提案・ユーザーが (a)〜(d) 全承認）→ writing-plans → subagent-driven-development。

### 確定判断（ユーザー承認 (a)〜(d)）
(a) 第1弾＝選択時ツールバー（前面/背面・削除）＋取り消し/やり直し（1操作=1手）＋余白ダブルタップで整列。(b) ズームスライダーは当面**残す**＋ダブルタップ整列を追加（100枚到達性を実機確認してから廃止検討）。(c) **複製は第2弾へ先送り**（`bookmarkId` 単一キー→インスタンスID層が要る・DB移行は不要）。(d) 取り消しは1操作=1手。

### 事実採取（memory `reference_stage2_mobile_collage_factmap`）
段階2 の7機能を現行コードで棚卸し→大半が既存部品で安く作れ、DB変更が要るのは複製だけと判明。モバイルの連続ジェスチャは2種のみ（1本指移動＋2本指ピンチ／四隅リサイズ・回転ノブは `!touchMode`＝デスクトップ専用）＝取り消しの確定フックが単純化できると確定。

### 入れたもの（計画 Task 1〜7・モデル割当 haiku/sonnet）
- **T1 `collage-layer-order.ts`**（haiku・4/4）: `sendToBack`＝`bringToFront` の対称（`[id, ...filter]`）。
- **T2 `collage-remove.ts`**（haiku・3/3）: `removeFromCollage`＝3マップから不変除去（回転0を落とさない `!== undefined` ガード）。
- **T3 `collage-history.ts`**（haiku・6/6）: `CollageSnapshot`型・`MAX_COLLAGE_HISTORY=40`・`snapshotsEqual`（**値比較**＝不変stateの新参照でも値等価なら no-op を積まない・key非対称はサイズガードで閉じる）・`pushSnapshot`（古いものから捨てる）。
- **T4 `MobileArrangeTopBar`＋z-index**（haiku・3/3）: `BOARD_Z_INDEX.SHARE_ARRANGE_TOOLBAR:402`。UNDO/REDO常時（disabled 連動）・選択中は TO FRONT/TO BACK/DELETE・`data-no-capture`・グラス。
- **T5 ジェスチャフック**（sonnet・既存18/18）: `CollageCanvas` に任意 `onEditGestureStart?(id)`/`onEditGestureEnd?`（`bindPointerGesture` に任意 `onEnd?`）／`MobileArrangeGestures` に任意 `onSelectedPinchEnd?`/`onDoubleTapFit?`＋余白ダブルタップ検知（300ms/24px）。**全て任意 prop＝デスクトップはバイト同一**。
- **T6 BoardRoot 配線**（sonnet）: `collageUndoStack`/`collageRedoStack`＋ref ミラー・`handleCollageGestureStart(reorderId?)`/`End`（`snapshotsEqual` 差分ありなら1回 push・選択タップの自動前面化は履歴に含めない）・`pushHistoryBeforeDiscreteEdit`・`handleCollageUndo`/`Redo`（ref 読み・setState ネストなし）・削除/前面/背面/ダブルタップ整列ハンドラ・上部バーのマウント・新 prop 配線・入退場でスタック空。
- **T7 e2e**（sonnet・15/15）: 選択でツール表示／削除でカード数減＋UNDO 復元＋REDO／**ピンチ→UNDO で幅復帰**（下記 Critical の回帰テスト）／ズーム→余白ダブルタップで scale=1／デスクトップは上部バー count 0。

### ★per-task レビューで Critical を1件捕捉＋修正（opus）
`handleSelectedPinchStart` で `handleCollageGestureStart()` を `collageArbiter.cancelActive()` の**前**に置いたため、arbiter の同期ドラッグ終了が `handleCollageGestureEnd` を呼びピンチ用 pending を消費→**2本指ピンチ（拡縮＋回転）が undo に一切積まれない**バグ。修正＝順序入替（cancelActive を先に）。再レビューで「ドラッグ=1手・ピンチ=1手・ボードズームは0手」が成立と確認。e2e にピンチ→undo を追加。計画 Step 5-1 の「先頭に」指示が誤りだった旨を計画doc に訂正注記。

### 不変条件（opus 全ブランチレビュー Ready to merge=YES で実コード直読検証）
デスクトップ(>640px) **バイト同一**（新 prop は `isMobile ? x : undefined`・`enabled=false` で wrapper 無し・回転ノブ経路は `onEnd` 無し・`ShareToast`/`handleCreateHostedShare`/`handleMobileCaptureAndCreate` 無改変）／**撮影は state ベース**（削除＝3マップ実除去で `CollageCanvas` の `if(!p) return null` により撮影からも消える・取り消し/やり直しは編集 state 差し替え・ボードズーム/ダブルタップ整列は `stageTransform` のみ＝画像無影響）／`renderCollageCanvasToJpeg`・サーバー・OG 無改変（ホストOG は1.91:1・`assert-share-template OK`）。

### 最終レビューの Minor#3 を出荷前に修正
opus 提案どおり、撮影中(~1-2秒 `creating`)も上部バーの DELETE/UNDO/REDO が押せた点を、上部バーを `shareCreateState==='idle'` のみ表示に修正（撮影中は編集不可・下部バーは CREATING… 表示で据え置き）。再ゲート緑。他 Minor 2件（16ms未満連打の stale ref／`lastBlankTap` の稀な誤整列）は無害で defer。

### 検証・出荷
tsc0 / **vitest 286ファイル2357全緑**（初回1件フレークは2回連続で 2357/2357＝既知 BroadcastChannel フレーク・段階2 と無関係）/ pnpm build OK（`assert-share-template OK`）/ playwright mobile-share **15/15**。`merge --no-ff` **1ff07486**（マージ木＝ブランチHEAD）→ master 再ビルド → `wrangler pages deploy --branch=master` で `allmarks.app` 反映済。

### 残（USER 実機確認）
段階1＋段階2第1弾をまとめて確認（手順は CURRENT_GOAL）。OK なら段階2 第2弾（複製[instanceID]・吸着＋触覚・ドラッグ削除演出・前へ/後ろへ単一ステップ・ズームのスライダー廃止検討）の詳細計画へ。

---

## セッション 193 追補 (2026-07-13) — スマホのアレンジUX作り直しを設計＋計画（実装は次セッション）

段階1/2第1弾を出荷後、ユーザーが実機スクショを共有。「常時チロムが画面を食いすぎ・カードズームとボードズームの区別が不明瞭・2本指ボードズームは密ボードで必ずカードに当たる」との指摘から、**スマホのアレンジ体験を作り直す方針をブレスト→設計書→計画書まで**（コードは未実装で区切り）。

### 対話で確定した重要な設計判断
- **画像とリンクは別レイヤー**（ユーザーの言葉で確定）: 選ぶ段階＝送りたいURL（リンクの中身・確定）／並べる段階（コラージュ）＝共有画像の見た目づくり。**DELETE は画像から外すだけ**（`buildArrangeShare`＝`selectedInBoardOrder(items, selectedIds)` は無改変＝リンクは減らない・ブックマークも無傷）。実コードで `buildArrangeShare` が `selectedIds` 由来・画像は `collageOrder` 由来と確認し、当初「食い違いだから直す」と言ったのを撤回（今の挙動が正しい）。
- **外すたびに毎回、母国語トーストで確認**（「これは自由なコラージュの操作で送る中身は減らない」を毎回伝える）。i18n（`board.collageRemoveToast`・15言語・parity テスト対象）。
- **言語方針を明文化**（s124 の既存ルールと一致・記憶 `feedback_ui_vocabulary` に既載）: 1単語アクション＝英語orアイコン／説明を含む文＝母国語。
- **9:16 に決定**（4:5 はスマホ画面を埋めない＝19.5:9/20:9 の実機で余白が残る、をユーザーが指摘）。リンクカードは 1.91:1 のまま（X等のカードは横長規格しかない＝縦にできない）＝9:16 を中央にレターボックス（細い帯）で割り切り。
- **PC にも「画像から外す」を追加**（ホバー×・承認済みのデスクトップ意図的変更）。DELETE がスマホ限定なのは不揃い、というユーザー指摘から。
- **2本指ボードズームは温存**（効きにくくても邪魔にならない＝ユーザー判断・`MobileArrangeGestures` 無改変）。ボードズームの確実な導線はドックのボタン。

### 成果物
- 設計書 `docs/superpowers/specs/2026-07-13-mobile-arrange-ux-redesign-design.md`
- 計画書 `docs/superpowers/plans/2026-07-13-mobile-arrange-ux-redesign.md`（完全・6タスク・安価モデル実装可）
- ブランチ `mobile-arrange-ux-redesign`（docs を master にマージ済み）。**次セッションで実装→ゲート→デプロイ**。

---

## セッション 194 (2026-07-13) — ★スマホのアレンジUX作り直しを計画どおり実装・本番反映（subagent-driven 自走）

s193 で設計＋計画（コード未実装で区切り）したスマホのアレンジ体験の作り直しを、`mobile-arrange-ux-redesign` ブランチで **subagent-driven-development** で完遂・本番反映。ユーザー指示＝「計画どおり実装→各レビュー＋opus 全ブランチレビュー→ゲート→merge→デプロイ→docs更新→実機確認依頼まで自走」。

**進め方**: pre-flight で計画の不正確を6件検出（全て実装者への指示で解決＝ユーザー判断不要）→ 6タスクを安価モデル中心で1つずつ（実装→タスクレビュー→ledger）→ opus 全ブランチレビュー → コメント1件だけ controller が直接修正 → ゲート → merge → deploy → docs。

- **モデル割当**: T1(9:16定数) haiku／T2(削除トースト15言語) haiku／T3(ドック＋トースト部品) sonnet／T4(BoardRoot配線) sonnet／T5(PC ホバー×) sonnet／T6(e2e＋ゲート) sonnet／最終 opus。各タスクレビューも sonnet。**全タスク review clean（Critical/Important ゼロ）**。
- **pre-flight で解決した計画の不正確**（ledger `.superpowers/sdd/progress.md` に記録）: R1=`STAGE_ZOOM_MIN/MAX` は BoardRoot 未 import（計画は import 済と誤記）→ `handleZoomSliderChange` が内部で[1,6]クランプするので不要／R2=トースト z は `UNDO_TOAST`(130) だとドック(402)の裏＝新 `SHARE_REMOVE_TOAST:404` を新設しドックの上に／R3=`handleBoardZoomFit` は既存 `handleDoubleTapFit` を流用（重複回避）／R4=撮影中(creating)は編集不可を維持＝ドックの undo/redo/選択ツールを `shareCreateState==='idle'` で gating（s193 hardening の温存）／R5=削除トーストはモバイル＋PC 両方＝`isMobile` 三項の外にマウント（body portal）／R6=15言語には zh も（計画の例示が漏らしていた）。
- **入れたもの**（Task 1〜6）: ①`lib/share/mobile-band.ts` `SHARE_PORTRAIT_ASPECT` 1080×1350→1080×1920（`mobileCollagePortraitBandRect` は `collageBandRect` 委譲で自動追従・撮影 w/h も定数由来で自動9:16）②`messages/*.json`×15 に `board.collageRemoveToast`（en/ja 逐語＋13言語 忠実訳・parity 30/30）③`MobileArrangeDock.tsx`/`.module.css`/`.test.tsx`＋`MobileArrangeToast.tsx`/`.module.css`＋`BOARD_Z_INDEX.SHARE_REMOVE_TOAST:404`（トーストは4秒 auto-dismiss を latest-ref パターンで＝stale closure/leak なし）④`BoardRoot.tsx` 配線（旧チロム撤去・ボードズームボタン handlers・削除トースト state＝enter/exit でリセット・撮影窓 gating）⑤`CollageCanvas.tsx`/`.module.css` に任意 `onRemoveCard?`＋`!touchMode` のホバー×（`.rotateHandle` の hover 流儀・local z=31・`data-no-capture`＋`stopPropagation` で移動を始めない）／BoardRoot `removeCollageCardById(id)` 汎用化・`handleDeleteSelectedCollage` は薄いラッパ⑥`tests/e2e/mobile-share.spec.ts`（15→19・SEED_COUNT 16→20＝9:16 の帯で全行両端到達を実 packer で再校正・payload 不変を実 POST body で検証）。
...
### サブ6（拡張 options フラット中立化＋15言語 i18n）— master merge `9daa80b7`・push 済・デプロイ不要
- **T1（Sonnet）**: `options.html`/`options.css` 全置換（脱サウンド演出＝オシロ/EQ/フェーダー/globe 除去→フラット中立）＋`options.js` 精密編集（i18n ローダ・文字列置換・pulseSaved 簡略化・#buildNum 除去）＋`popup` 全置換＋`manifest` L5 のみ `__MSG_extDescription__`。全 input id 保持。レビュー: byte-fidelity を plan と直接 diff で検証・✅。
- **T2（Haiku）**: `_locales/<15言語>/messages.json`（en+14）を plan §4 から逐語転記・各38キー。レビュアーがスクリプトで plan ブロックとファイルを byte 比較＝15言語完全一致・description は en のみ・✅。
- **T3（Sonnet）**: `tests/extension/locales-parity.test.ts`（キー集合一致＋非空）新設＋全ゲート（JSON構文・parity 30/30・node --check・拡張テスト 161/161・package-extension OK）・✅。
- **opus 全ブランチレビュー**: data-i18n↔ロケールキー集合一致（宙ぶらりん参照ゼロ）・manifest default_locale:en+extDescription 実在で解決・盤面ビルド無影響。Ready=YES。唯一の Minor（左ナビ非翻訳）は plan §0「訳さない語」の意図仕様。**extension/ のみ＝盤面不変ゆえ allmarks デプロイ不要**（拡張は Web Store 別配布）。翻訳は AI 下書き＝公開前に native/Sonnet+ レビュー。

### サブ1（chrome スキントークン基盤）— master merge `1122314a`・push・`allmarks.app` デプロイ済
- **T1+2（Sonnet・密結合ゆえ統合）**: `:root` に9 chrome スキントークン定義＋ChromeDrawer/ChromeButton をトークン参照化＋バイト維持 e2e。**plan の placeholder（radius 16px/blur 18px）は誤りで、統括が実測した現物 14px/14px を採用**。opus/sonnet レビューが全プロパティで「token :root 値=元値」を静的証明・✅。既存 backdrop-filter=none は LightningCSS prefix-collapse の既存バグ（前後とも none=不変・test で lock）。
- **T3（Sonnet）**: 残り chrome（ExtensionEntry/FilterPill/ThemeCustomizeSection）トークン化。**opus レビューが「:root 定義済みトークンの fallback は死にコード＝置換は元値が :root 値と一致する時のみバイト維持」の罠を表で検証＝20/20 PASS**。ThemePicker sans-pin 温存・TuneTrigger .drawer は不一致ゆえ正しくゼロ変更。
- **T5（Haiku）**: `html[data-theme-id="dotted-notebook"]` chrome ブロックの器（:root と同値=バイト同一・サブ2 のミキサー皮の所番地）。
- **T6（Sonnet）**: `chrome-theme-coverage.test.tsx`（全3テーマ×SETTINGS/THEMES/TUNE/FilterPill を getByTestId で lock＝抜けゼロ・欠落で失敗する実効テスト）。
- **T7（Sonnet）**: 統合ゲート全緑＋`use-is-paper-theme.ts` コメント修正。調査で「メニュー中立化は既存 489caf7e(7/5)が実施済」を確認。
- **★opus 全ブランチレビューが統括の見落とした退行を捕捉（多層レビューの価値）**: sub1 が `--chrome-btn-color` を live 化したため、紙テーマ(paper-atelier)の `html[data-theme-id]` レベルの死蔵上書き（489caf7e が面消費だけ剥がし定義を残した残骸）が発火→FilterPill ドロップダウン行等が暗インク×暗ガラス＝不可視。統括は先に LIGHT_EDGE_CHROME 経路のみ追って「退行なし」と誤判定していた。
- **fix-1（Sonnet）**: 紙の `--chrome-btn-color`/-hover 上書きを中立化（:root 0.85 light にフォール）＝紙を sub1 以前に厳密復元。paper-atelier e2e ケースで「ドロップダウンが暗インクでない」を lock。再レビュー RESOLVED。紙 chrome の色付け（暗インク＋クリーム面）はサブ2/3 の紙皮で `--paper-panel-*` と対で復活。
- **旧 Task 4（light-BOARD の chrome 反転）は保留**: 明色盤面テーマが出るまで意味を持たず視覚変更ゆえサブ2/3 とモック承認つきで一緒に。
...
## セッション 207 (2026-09-02) — N-76(SHARE自動配置のツリーマップ化)・N-74(SHAREタイトルTO FRONT/TO BACK)を完遂／Private表記統一の漏れ2件を追加発見・修正／拡張機能v0.1.25審査通過／TODO.mdの肥大化を解消

**session204提案の残り2件(N-74/76)を「N-76から、ついでにN-74も」という順で完遂。途中の割り込みでPrivate表記の追加バグ2件・拡張機能の状況確認も同セッション内で処理。**

### N-76: SHARE画面の自動配置をスクエア化ツリーマップ方式に(トライアル導入)

superpowers:brainstormingをスパイクとして開始。まず技術調査: 今のARRANGE開始時レイアウト(`fitSelectionToScreen`)は既にFlickr自身の`justified-layout`ライブラリと同じアルゴリズム系統(目標行高＋縦横比比例充填)と判明、差し替えても改善しないと確認。ユーザーから「回転ではなく、選んだものが盤面いっぱいに隙間なく配置されるライブラリはないか、自作できないか」と明確な訂正を受け方向転換。調査の結果、**スクエア化ツリーマップ(Bruls et al.、`d3-hierarchy`の`treemapSquarify`が代表実装)**が該当し、面積比に応じて長方形で完全に敷き詰める(隙間ゼロ保証)アルゴリズムと判明。d3丸ごとは入れず、アルゴリズム自体(約80行)を`lib/share/collage-mosaic.ts`に自前実装(TDD)。

- **画像のトリミングは既存仕様と確認**: `ImageCard.module.css`に`object-fit: cover`が既に標準で、「見切れは承認済み」という過去のコメントも発見。新規改修なしでマス目に合わせたトリミングが機能する。
- **ユーザーの要望「一旦試して、気に入らなければ削除せず眠らせておきたい」に対応**: `lib/share/collage-arrange-mode.ts`に`ARRANGE_AUTOLAYOUT`という1つのスイッチを新設、デスクトップ・モバイル両方のARRANGE開始処理をこの関数経由に変更。旧`fitSelectionToScreen`は1行も変更せず併存、スイッチを`'justified'`に戻すだけで完全復帰可能。
- **実装中に発見・修正した設計判断**: 隙間(gap)を持たせると、ツリーマップ特有の「カードが縦/横に積み重なって配置される」構造により、積み重なった内側の隙間がまれに画像の端(外周)にも露出することが判明(既存のmobile-share e2eの「レターボックスなし」テストが検出)。ユーザーの本来の要望(余白ゼロ)により忠実な形として、**隙間を完全ゼロ(gap=0)**に変更して解決。
- 副産物として、既存e2eの「DOM上のrowでグループ化して端まで届くか判定する」チェックが、justified-rows特有の実装詳細に依存していたと判明、より一般的な「水平スキャンラインで判定」する形に汎化(ツリーマップでも正しく機能)。
- **検証**: vitest新規9件緑(zero-gap coverage・境界flush・squareness regression等)/ 既存2541〜2551件緑(既知の無関係flaky=channel.test.ts) / SHARE関連e2e30〜31件緑 / `pnpm build`成功。`allmarks.app`にデプロイ済。

### N-74: SHAREタイトルの前面/背面トグル(4ラウンドの対話で文言まで練り上げ)

現状のタイトル(自由配置・自由リサイズ可能)は常にカードの背後に固定描画で、前面に出す手段が無いと判明(コード内コメントで明記済み)。`ShareTitleConfig`に`layer: 'behind'|'front'`(既定behind＝既存挙動と完全一致)を追加、`CollageCanvas`が全カードより1つ上のz-indexを計算して`ShareTitleElement`に渡す設計で実装(TDD、desktop限定=タイトル機能自体が現状デスクトップ限定のため)。

- **ラウンド1(場所)**: 当初トグルをヘッダーの既存「TITLE」ON/OFF横に置く案を提示→ユーザーが「シェア専用の要素なのでSHAREのボタン群(`ShareToast`)の中がいい」と訂正、ヘッダーは通常盤面とも共用する場所ゆえ不適切という理由で合意。
- **ラウンド2(文言v1)**: `share.toFront`/`toBack`(モバイルのカード操作で既に15言語対応済みの「TO FRONT」「TO BACK」)をそのまま流用する案で実装まで進める。
- **ラウンド3(ユーザー指摘で差し戻し)**: 「後ろの文字をタイトルとユーザーが認識できないと思う」と指摘。代替案3つ(①テキスト自体にアイコンを直付け②アイコンのみに変更③掴んだ方が自動前面化)を提示、ユーザーは「単語を変えて調整したい、タイトルを各言語に訳す、日本語はカタカナで」と方針決定。
- **ラウンド4(文言v2・15言語)**: `share.titleToFront`/`titleToBack`を新設、英日は精査(日本語は「タイトルを最前面へ」と助詞付きで自然化)、他13言語もそれぞれの語順に合わせて調整(トルコ語は目的格変化、ベトナム語は動詞先頭など単純結合を避けた)、既存の慣例通りAI下訳・公開前ネイティブレビュー要。
- **検証**: vitest新規十数件緑 / SHARE関連e2e31件緑(新規デスクトップe2eでBoardRoot配線まで実機同等に確認) / `pnpm build`成功。`allmarks.app`にデプロイ済(2回、機能実装→文言修正の順)。

### 割り込みで発見・修正: Private表記統一の漏れ2箇所＋拡張機能状況確認

ユーザーから「拡張機能v0.1.25の審査通過・フロートボタンでPrivateタグ付けが出ない・カードの＋ボタンのPrivateアイコンが古いまま」と報告。systematic-debuggingで根因確認:

- **確定した実バグ2件(修正済)**: session206の「4箇所統一」(FilterPill/TagDropPanel/BoardMobileTagBar/拡張機能)に、**カードの＋ボタンのポップオーバー(`TagAddPopover`)とSETTINGS→PRIVATEボタン(`ExtensionEntry.tsx`)が含まれておらず**、両方とも古い絵文字🔒/🔓のままだったと判明。`PrivateLockGlyph`(モノクロ線画+アニメーション)に統一、`lib/private/ui-labels.ts`の死んだ絵文字定数(`PRIVATE_LOCKED_ICON`等)も削除。
- **「フロートボタンでPrivateが出ない」は実バグではなくキャッシュ**: コード確認で、フロートボタンのPrivateチップ配線自体はv0.1.25に既に含まれていると確認(該当コミットがバージョンアップより前)。ユーザーに`chrome://extensions`のバージョン確認を依頼したところ、Chrome側の自動更新が追いついておらずv0.1.24のままだったと判明、手動更新で解決(実バグではなかった)。
- **拡張機能のアイコン修正は既に別コミットで完了済と確認**: `extension/floating-button.js`のアイコンは既にv0.1.26向けに修正済み(未提出のみ)、提出用zip(`dist/booklage-extension-0.1.26.zip`)も生成済みと確認、ユーザーがv0.1.25審査通過後に提出。

### TODO.mdの肥大化を解消

ユーザーからの「常に能力を高く保ってほしい」という要望で、`docs/TODO.md`の「現在の状態」節に約55セッション分(セッション152〜206、636行)の詳細narrativeが蓄積していた問題に対応。うち大半は既に`TODO_COMPLETED.md`に重複してアーカイブ済と確認できたが、**10ブロック(セッション177/179/185/187/191追補2/196/198/199/201/204)は一度もアーカイブされていなかった**と判明、内容を1文字も失わず時系列順でTODO_COMPLETED.mdへ移動してからTODO.mdの当該セクションを短いポインタに置換(Node scriptで機械的に処理、移動前後で全セッション番号の存在を検証)。TODO.md: 985行→348行。ついでにN-69/74/75/76の記載をアクティブbacklogから除去(全完了のため)、残りはN-78のみに整理。

### 検証・本番反映(全体)

各項目ごとに tsc0 / vitest(2541〜2551件緑、既知の無関係flaky=`channel.test.ts`) / SHARE関連e2e(30〜31件緑) / `pnpm build`成功を確認、都度`allmarks.app`にデプロイ・push済(TODO掃除のみdocsのみのためpush保留、次の実務pushに同梱の方針)。
...
- ユーザーが「パスワード復旧手段は必ず必要」と明確に決定 → 1Passwordの Emergency Kit 方式(端末内で生成する復旧キー)を採用。設計スペック `docs/superpowers/specs/2026-09-21-private-recovery-key-design.md`・実装計画 `docs/superpowers/plans/2026-09-21-private-recovery-key.md`(9タスク・subagent-driven-development)。
- 新規vault作成時は自動発行、既存vaultは次回解錠時に**強制的に**作成させる(ユーザー追加指示「気づいた人だけでなく全員に」)。「パスワードを忘れた場合」リンク→復旧キー入力→新パスワード必須設定、の一連の流れ。復旧キーは`lib/private/crypto.ts`の`generateRecoveryKey`(31文字アルファベット・30文字・150ビット相当)。既存の暗号プリミティブを流用し新しい暗号方式は追加していない。

### 最終ブランチレビュー(opus)で発見・修正した5件のImportant指摘

1. 復旧キー用のPBKDF2繰り返し回数がレコードに自己完結しておらず、将来のパスワード側定数変更で全復旧キーが無言で失効する不具合
2. 端末間同期のLWWマージが発行したての復旧キーを無言で消しうる不具合(**1回目の修正が不十分と2回目のレビューで判明**→`updatedAt`が同点になるタイブレーク経路にも引き継ぎ処理を適用する形に修正し直し・3周期にわたる収束を実際にトレースして確認)
3. コピー完了フィードバックの欠落+計画書から漏れていたi18nキー
4. 既存ユーザーへの案内不足→ユーザー指示で「強制作成」に格上げ
5. エラーハンドリングの取りこぼし
...
3. Chromeがパスワード更新を提案してこない → 3つの入力画面を本物の`<form>`に作り直し。それでも改善しなかったため、隠しユーザー名欄(`autoComplete="username"`・視覚的に完全非表示・読み取り専用)を追加。**値は「AllMarks」**(ユーザー指定。一度「Private」で実装してしまい訂正指示を受けて直した経緯あり — 教訓としてCLAUDE.md運用ルールに反映済み)。

### 副作用・検証

- `<form>`化で3画面の余白が詰まる視覚退行(テストでは検知不可・レビューで実測発見)→ CSS修正。
- Playwrightのクリップボード権限をグローバル付与すると既存テスト(`mobile-save.spec.ts`)と衝突 → ファイル単位に権限スコープを限定。
- 最終状態: フルスイート2980/2980・tsc0・playwright(`private-vault.spec.ts`)16/16・build OK。masterへマージ・push・`allmarks.app`へデプロイ済・ユーザーが実機で複数回確認済。

## s218(2026-09-22) — 同期の近リアルタイム化 + 公開前タスクPL-1/PL-2完了

### 近リアルタイム同期の配線

- 既存の`markDirty`(20秒デバウンス)の仕組みはあったが、全保存箇所への配線がゼロだった問題を解消。
- `lib/storage/indexeddb.ts`の`initDB()`が返す`db`をProxyで包み、`put`/`add`/`delete`/`clear`または書き込みトランザクションが呼ばれるたびに自動で「変更あり」信号(`lib/sync/sync-signal.ts`の`notifySyncDirty`、`vault-session.ts`と同じモジュール単体パターン)を出す一箇所配線に変更。将来保存処理が増えても追加配線不要・取りこぼしなし。
- 同期処理自身が行うローカル書き戻し(`engine.ts`の`applySnapshotToLocal`等)は`withSyncWritesSuppressed`で信号から除外 — 除外しないと同期完了→自身の保存で信号発生→また同期、と無限ループする不具合になるところだった(回帰テストで確認済み)。
- iPhone Safariでは`beforeunload`が確実に発火しないため、`pagehide`リスナーを追加(`visibilitychange`/`beforeunload`と並行)。
- デバウンス間隔は既存の20秒のまま(ユーザーとの相談の結果、秒数より「タブ切替・アプリ切替時に確実に同期される」ことを優先する方針に決定)。
- 検証: tsc0・vitestフルスイート2990/2990(新規10件、うち無限ループ防止の回帰テスト含む)・`pnpm build` OK。commit `1322da85`・push・`allmarks.app`へデプロイ済。

### PL-1/PL-2(公開前タスク・ユーザー自身の作業)完了

- PL-1: OAuth同意画面のサポートメール/デベロッパー連絡先を個人Gmailから`allmarks-support@googlegroups.com`(Googleグループ)へ切替済み。
- PL-2: Search Consoleで`allmarks.app`のドメイン所有権をCloudflare連携のワンクリック確認で検証 → OAuth同意画面を「テスト中」から「本番環境」へ公開。`drive.file`+`openid`/`email`/`profile`は全て非機密スコープのため追加審査は発生せず即時反映。これでrefresh tokenの7日失効問題が解消し、放置運転の自動同期が実用になった。
- ブランディングページの「ホームページ/プライバシーポリシー/利用規約」リンクも`https://allmarks.app`配下の既存ページで埋めるようユーザーに案内。

## s218続き(2026-09-22〜23) — K3運用開始・レート制限・同期UXの全面作り直し・テーマ端末別化・端末数表示

### K3運用セットアップ・完了

- 招待コード(192ビット相当のランダム文字列)を生成し`wrangler kv key put`でK3_KV本番へseed(`maxIssue:50`)。実際に`/claim?c=...`を叩いて本番で1回分動作確認(発行された鍵はそのままユーザー本人の実利用鍵として使用)。
- ユーザーがPC・スマホの2端末で同期を実際に接続・確認完了。

### Cloudflareレート制限ルール・設定はしたが未確認のまま保留

- `/claim`・`/activate`に対し、無料プランの範囲(10秒間隔・1ルールのみ)でレート制限ルールを設定・デプロイ済み。ただし**実際にブロックが発動するか複数回テストしたが確認できず**(仕様の反映待ちか、他の原因かは特定できていない)。実害はない(本来の防御=招待コードの高いエントロピーは既に機能している)ため保留と判断。**次回、時間を置いてから再確認するか、Cloudflareサポートに問い合わせるか要検討**。

### 同期セットアップのUX全面作り直し

- ユーザーが実際に同期を接続してみて「UXが非常に分かりづらい」とフィードバック。旧作りは、SETTINGS内にいきなり鍵入力欄が出て、鍵を入れると別途「Googleと連携する」ボタンが現れる、という唐突な2段階だった。
- 新規`components/board/SyncConnectDialog.tsx`(+ CSS・テスト)を作成: 「Google Driveで同期する」という1つの分かりやすい入口ボタン→画面中央のモーダル(Private機能のダイアログと同じ視覚言語)→鍵入力→Google連携→大きく分かりやすい待機インジケーター→完了画面(「他の端末でも同じ鍵を入力すれば同期されます、端末5台まで」)、という誘導型の流れに全面作り直し。`SyncPanel.tsx`はほぼロジック不変(`activateLicenseKey`/`requestAuthCode`/`exchangeCode`/`connectSync`/`runSyncCycle`は一切変更なし)、表示の再構成のみ。
- 実装はsubagentに委譲・司令塔が受け取り後に独立検証(tsc/vitest/build)。
- **ユーザーの実機フィードバックで2件追加修正**: (1) ボタンが「キャンセル」から離れた位置にあり分かりづらい→両方を同じ並びに配置し直し(お手本にしたPrivateダイアログの実際の作法から、指示書の設計段階で誤って外していたことが判明)、(2) ボタン文言「解錠」が伝わりづらい→「つづける」に変更(この文言はこの1箇所でしか使われていないことを確認してから変更)。
- 13言語のうち日本語以外は、同期関連の文言がそもそも全て未翻訳(英語のまま)だったと判明(今回新たに生じた問題ではない、既存の状態)。追加した新規文言もそれに合わせて英語のままにした。多言語仕上げの際にまとめて対応すべき項目として記録。

### テーマを端末ごとに独立させる修正(実データで発覚した不具合)

- ユーザーがPC(デフォルトテーマ)とスマホ(Flat+格子)を同期接続したところ、PC側のテーマがスマホのものに置き換わってしまった。原因調査の結果、`board-config`(テーマを含む盤面設定一式)が1つのレコードとしてまるごとLWW(最終更新優先)で同期される設計だったため、と判明(バグではなく元の設計どおりの挙動)。
- ユーザー判断: **テーマだけは端末ごとに独立させたい**。`lib/sync/merge.ts`の`mergeBoardConfig`を修正: 他の設定(frameRatio・displayMode・motionEnabled・bgTypoEnabled・roundedCorners等)は従来どおりの全体LWWのまま、**themeId/themeCustomizationsだけは常にローカル(この端末)の値を残す**例外処理を追加。新規端末の初回同期(ローカルにまだ設定が無い状態)はテーマも含めてリモートを初期値として継承する既存挙動は維持。
- この変更で`mergeBoardConfig`が意図的に非対称(呼び出し順で結果が変わる)になったため、`mergeAll`の「順序に関係なく同じ結果になる」という既存の決定性テストが正しく赤くなった→テーマ以外の可換性は維持しつつ、テーマの非対称性を明示的に検証するテストに書き直し。
- **ユーザーへの補足**: 本修正は「今後」の上書きを防ぐのみで、既にPC側に伝播してしまった分は自動で戻らないため、手動でPC側のテーマを設定し直すよう案内済み。

### 「X/5 devices」表示の追加

- ユーザーから「招待リンクのしくみが理解できない」「あと何台まで同期できるか画面に出ない」という率直なフィードバック。前者はチャットで一から丁寧に説明(招待リンクは正式機能ではなく開発中の裏口的な入手経路であることを明言)。後者は実際に機能として追加。
- 新規`functions/activate-status.ts`(GET `/activate-status?kid=`、読み取り専用・端末登録は一切行わない、既存`POST /activate`の兄弟エンドポイント)+`lib/board/license-activate.ts`の`fetchDeviceCount`(失敗時は必ずnull、古い数字や偽の数字を絶対に見せない設計)。`SyncPanel.tsx`の接続済み画面に「最終同期」の下へ表示。

### 検証・出荷

- 全工程で tsc0・vitestフルスイート(最終3038/3038)・`pnpm build`OK を都度確認。全てmasterへ直接コミット・push・`allmarks.app`へ都度デプロイ済み。

**★次セッション**: 未確認のCloudflareレート制限の再確認(時間を置いて再テスト)。それ以外の優先タスクは未定 — 次回`docs/CURRENT_GOAL.md`を参照。同期機能(束1-6+今回の全面UX改修)は実用段階に到達。次は収益化・ローンチ準備の棚卸し(2026-08-26のs205以降、ほぼ機能構築に専念していたため、計画自体が古くなっている)が候補。

## s219(2026-09-23) — テキストカードのガラス化を全テーマへ展開・アバター系バグ2件・収益化棚卸し

### テキストカード背景のガラス化(dotted-notebook→Flat→Paperの順に展開)

- 発端はユーザー提供のUiverse.io参考コード(JohnnyCSilva作)。生成アート画像+べた塗り黒背景だったテキストカードを、透明感のあるグラデーション(薄い白ハイライト+斜めの光沢)に置き換え。青→紫のぼやけた光(`::before`)はAIグラデーション禁止ルールに抵触するため除外。まずdotted-notebook(デフォルト)テーマ限定で実装・本番反映。
- **ライトボックスでアイコンが出ない不具合を発見・修正**: テキストのみのツイートをライトボックスで拡大表示する際、実データではなく`fakeBoardItem`という仮データを組み立てて渡していたが、この仮データに`authorAvatar`/`authorName`が含まれていなかった。`LightboxItem`型・`normalizeItem()`・`fakeBoardItem`の3箇所に authorAvatar/authorName を追加して解消。
- **ライトボックス下2角の黒いにじみを発見・修正**: 新しく追加した`box-shadow`は実は板の小さいカードでは完全に見えない(`CardNode`の`.inner`がoverflow:hiddenで切り取るため)。しかしライトボックスの拡大表示は`zoom`で2〜3倍に引き伸ばす作りのため、影も比例して巨大化し、外側の枠でハードに切り取られて濃い黒の塊として見えていた。ライトボックスの`.imageBox`内でのみ`box-shadow: none`で無効化(板側は元々見えていないので影響なし)。
- **Flatテーマへ展開**: ユーザー指摘により方針転換 — 「背景を白く塗り替える」のではなく、**そもそも背景は透明(background-color未指定)で薄いグラデーションを重ねているだけ**という設計を正しく理解し、dotted-notebookと全く同じ背景/枠線/影/光沢のCSSをFlatにも共有、文字色(タイトル・ホスト名・投稿者名)だけをFlat用の濃いインク色に反転。板の色がそのまま透けて見える構造なので、明暗どちらの板でも同じCSSで成立する。
- **Paperテーマへ展開**: ユーザー指摘で発覚 — Paperテーマの文字カードは`PlaceholderCard.tsx`内で`if (paper) { return 全く別のJSX }`という完全に独立した分岐になっており、投稿者名/アバター表示機能(s219で追加したばかりの`renderMetaStrip`)がそもそも実装されていなかった(実害: 投稿者不明のツイートが紙のノートにそのまま貼られる)。`renderMetaStrip()`という共通関数に切り出し、デフォルト・Paper両方から呼ぶ形に統一(以後この部分への変更は全テーマへ自動的に反映される)。
- **Paperの位置・固定仕様の修正**: 最初の実装ではアバター+名前をスクロール領域の中に入れてしまい、中央寄せ・スクロールで流れる、というデフォルトテーマ(上固定・左寄せ)と異なる挙動になっていた(ユーザー実機スクリーンショットで発覚)。スクロール領域の外の兄弟要素として置き直し、デフォルトと同じ`position:absolute`の仕組みを継承(座標だけ紙のスパイラル穴を避ける値に調整)。目一杯スクロールすると文字がアイコンの後ろに回り込む挙動は、デフォルトテーマでも全く同じに起きることを検証で確認済み(新規の不具合ではなく既存の共通の仕様限界)。
- **副産物: ライトボックス内アバターの角丸崩れ(全テーマ共通)を発見・修正**: `.media img`という汎用ルール(過去にも"N-23"「session 56」として同種の事故が複数回起きている既知の落とし穴)が、想定していなかったアバター画像にまで`border-radius`/`object-fit`を上書きしていた。Paperで正方形になって初めて目視で発覚したが、実はdotted-notebook/Flatでも微妙に不正確な角丸だった(たまたま丸く見えていただけ)。既存の`favicon`/`embedPoster`/`tweetPhoto`と同じ「`[class*="..."]`で上書きする」パターンで`authorAvatar`にも同じ対策を追加、3テーマ全て解消。
- 各段階でPlaywrightで板・ライトボックス両方を実機同等の条件で確認しながら進行、tsc0・vitestフルスイート3063/3063を都度確認。全てmasterへコミット・`allmarks.app`へデプロイ済み(sw.js キャッシュバージョンも都度bump)。

### 収益化・ローンチ準備の棚卸し(調査のみ・実装は次セッション)

- `docs/CURRENT_GOAL.md`(s218時点)の「次にやること②」に着手。`docs/private/`の既存計画群(2026-06-30モデルA・2026-07-01ローンチ計画B・2026-07-01支援PF調査・2026-07-01 K3設計/計画・2026-07-08リリース滑走路・2026-07-12収益化再点検・2026-09-02同期設計)を読み直し、既に決定済みの事項を再確認(**これらは既に十分調査・決定済みであり、次セッションで再度ゼロから調べ直す必要は無い**):
  - **集金プラットフォームは FANBOX が本命で確定済み**(2026-07-01支援PF調査で9PF比較済み)。理由=ユーザー本人の客層(FF14/イラスト系・約600フォロワー)がpixivエコシステム地続きで摩擦が最も低い・円建て・pixiv公式もソフトウェア開発費支援クリエイターの増加を報告済み。Patreonは海外向けの後付けで良い(客層に不利・手数料高め・ドル建て)。Stripe系(Ko-fi/BMAC/Stripe直)は不使用と確定済み(ユーザー本人の別プロジェクトがStripe審査落ちした経緯があるため、FANBOX/Patreonはどちらも非Stripeなので影響を受けない)。
  - **鍵配布の仕組みも確定済み・技術的にはs218で実証済み**: FANBOX含めどの日本国内PFも「会員を自動確認する公式API」を持たないため、鍵配布は「合言葉リンク方式」を自作する(`allmarks.app/claim?c=<秘密文字列>`をFANBOXの会員限定投稿に貼る→クリックでプール済みのユニーク鍵を1個自動払い出し)。この仕組み自体はK3として既に実装済みで、s218でユーザー自身の招待コードを実際に発行・2端末で同期接続まで実機確認済み。**技術的な土台はゼロから作る必要がなく、同じ仕組みを流用するだけでよい**。
  ...
  - LP(app/(marketing)/)を実際に確認 → 現状5つの訴求ポイント(CAPTURE/LAYOUT/LIVE GRID/ORGANIZE/PRIVACY)に同期機能の言及が一切ないことを確認(同期機能自体はs218で実用段階に達したのに未宣伝)。ユーザー提案「月額会員なら複数端末同期、とLPに大きく出すべきでは」に同意しつつ、**受け皿(FANBOX)が無い状態で先にLPで訴求すると、興味を持った人が申し込めない**という順序の問題を指摘・合意形成: ①受け皿を整える→②「支払う→鍵が届く」導線を配線→③その上でLPに大きく出す、の順で進めることに決定。
- ユーザーからのフィードバック: 一度この棚卸しの結果を口頭で共有した際、「棚卸はすでにしている部分もあるのにまた同じことを聞いている」と指摘を受けた。教訓: 収益化のような大きな話題を再開するときは、`docs/private/`の既存資料を実際に読み直して既決事項を洗い出してから会話を始めること(ユーザーに再度決めさせる/繰り返し尋ねるのは避ける)。

### TODO.mdの整理(200行目安を超えていたため、古い/解決済みのnarrativeをここへ退避)

以下は`docs/TODO.md`から移動した項目。実装や結論そのものは変わらず、置き場所だけの変更。

- **(N-56/N-58) スマホ共有画像作成不可 → コラージュ編集要望(session 183-188)**: N-56(iPhone Safariで共有画像が真っ白/作成不可)はcanvas直描画レンダラー(`lib/share/capture-mirror.ts`系、計画書`2026-07-12-n56-mobile-canvas-renderer.md`)で恒久修正・出荷済み(memory `project_mobile_share_canvas_renderer`)。N-58(スマホでもコラージュ編集させたい)はs192-194で実装・出荷したが、ユーザー実機フィードバックで「思ったのと違う」との判断でs195にSHELVED(棚上げ、削除ではない — memory `project_mobile_portrait_collage`参照)。N-55(撮影成功後もコラージュに触れる小さなUXの傷)はN-58実装で解消済みのはず。TODO.mdにあった詳細な診断ログ(OOMクラッシュ調査・倍率フォールバック等)はここでは省略、経緯を知りたければgit historyとこのファイルのs188前後を参照。
- **(N-50) タブレットレイアウト未対応** — s184で発見(iPadで主要ボタンが指の最小寸法未満)。その後タブレット専用対応をしたという記録が見当たらないため、**未解決の可能性が高い**。再度必要になったら実機で現状を確認してから着手すること(このnarrative自体は古いので鵜呑みにしない)。
- **(N-57/N-51の残り) スマホの盤面に背景タイトル(ワードマーク)が出ていない** — s184時点の報告。その後の対応記録なし。再確認要。
- **(N-59) スマホの列数・余白を簡易調整したい** — 小さい新規要望、未着手のまま記録が途切れている。優先度低。
- **★★最優先: スマホ本格対応(s161)** — 「最後に回す→最優先」という2026-07時点の指示だったが、その後スマホ保存(束B)・スマホ閲覧(束A)・モバイル3列レイアウト(s178)など多数のモバイル対応が出荷され、この節が指す「最優先」状態はとうに過ぎている。**「最優先」という表題は完全に古い**(現在の最優先は収益化・ローンチ準備)。この節にあった「B-#10 モバイルUX本格チューニング」(3列化・列数調整)はs178の`project_mobile_board_direction`で対応済み。
- **(N-33) タグの大文字表示** — 調査の結果「機能的に直すものは無い、意図的な統一デザイン」と判明済み。大文字化する場合はアプリ全体の視覚変更としてbrainstorm要(N-26/32/35フラット化と合流予定だった)。フラット化自体はs199で完了しているが、タグ表記を大文字にするかどうかの決定は記録が見当たらない = 決定していない可能性が高い。必要なら再度ユーザーに確認。
- **(N-26/32 フラット化サブ②「白フラットをdefaultテーマに」)** — 完了。Flatテーマは現在完全に機能しており(本セッションs219でもテキストカードのガラス化を展開した対象)、この節が指していた「次はサブ②」は既に達成されている。
- **(N-07) e2eシード版数ズレ** — 解決済み。`tests/e2e/helpers/seed-db.ts`という無版数(version-less)のシードヘルパーが現在は標準で使われており(`board-b0.spec.ts`も含め)、この問題が指していた「ハードコードされた古い版数でVersionErrorになる」状況は発生しない。
- **角丸24→20の検討** — 完了済み。commit `7bb0529`で`--card-radius`/`--lightbox-media-radius`は20pxに統一済み(`Lightbox.tsx`のコメントで確認)。
- **B-#3 重複URLでサムネ等が出ない問題** — 「重複URLの扱い」自体はその後正式な仕様として決定・実装済み(memory `project_duplicate_url_policy`: 重複は確認してから許可)。この古い調査タスクが指していた具体的な表示不具合は、当時の実装(旧TextCard等)を前提にしており現行のPlaceholderCard構成とは既に乖離しているため、再発するなら新規に調査し直すべき。
- **MinimalCard polish** — 削除(dead)。`MinimalCard`というコンポーネントはsession 88で`PlaceholderCard`へ統合され、現在はコードベースに存在しない(コメント内の歴史的言及のみ)。
- **session 150続き(ユーザー実機メモ7件)** — 元の3行(✅完了2件・⏹対応不要1件・🅿保留1件)はTODO.mdに残置する価値が薄いため要約のみ記録: N-17(TRASHのEMPTY TRASHボタン赤化)・N-18(拡張クイックタグ窓の見切れ)は完了・本番反映済み。N-14(Lightbox中のボードモーション)は対応不要と判定済み(`ambientOn`gateで既に停止)。N-16(空ボードの青モーダル・スマホ限定)は保留のまま。


## s220 (2026-09-24) — 有料プランの受け皿(Paddle)整備・審査提出

- **決済サービスの変更**: 支援ページ文面の検討中に、候補だった支援プラットフォームの AI生成コンテンツ禁止規約(制作の主要部分にAIを使ったコンテンツの投稿・誘導を禁止、プラン文も審査対象)が判明し不採用。非Stripe・販売代行(MoR)の Paddle に決定(判断根拠・比較は docs/private/)。
- **公開ページ(15言語・本番反映済)**: 料金ページ新設(3カード・月/年トグル・スマホは無料カードを細く畳む)、返金ポリシー新設、利用規約(有料プラン節・運営者節=Masaya Maeno を初出で定義)、プライバシー(端末間同期・同期キーと端末数・お支払い・Google Limited Use)、問い合わせ(Paddle 窓口への誘導・メール窓口・フィードバック欄削除)、FAQ 12問追加。本文リンク用に `lib/i18n/rich-text.tsx`(`[[label|token]]`)を追加。commit 2ab313a6 / 6dbea524(スマホ調整)。フルスイート 3082/3082。
- **運用**: サポート用グループアドレスの外部受信を設定・着信確認。Paddle 本番/Sandbox アカウント作成、本番の本人・サイト審査を提出(結果待ち)。
- **決定(設計の前提)**: 鍵は支払い完了画面でその場で表示、紛失時は注文番号で再表示(こちらからメール送信なし)。解約・返金は期間末で同期停止、1日1回の契約確認+7日の猶予、支払い失敗中は止めない、無償発行の鍵は無期限。

## s221 (2026-09-24〜25) — 有料化の仕組みを完成(Sandbox 通しテスト成功・本番未公開)

- **Paddle 本番審査通過**(Sumsub 本人確認)。のち住所書類が非対応として差し戻し → ユーザーが銀行取引明細で再提出予定。Payoneer 申請(出金先・約2営業日)。
- **設計**(`docs/private/2026-09-24-paddle-license-lifecycle-design.md`): 1契約=1鍵、鍵の中身は変えずサーバー側 `issued:<kid>` で有料/無償を区別。webhook を使わず確認のたびに Paddle API へ問い合わせる方式(¥0・KV 書込みゼロ)にユーザー了承で変更。規模上限=有料2〜4万人で無料枠到達(対策を §7 に記載)。
- **サーバー**: `/api/license/purchase`(取引確認→鍵発行/再表示)、`/api/license/status`(無償鍵は常に有効・Paddle 鍵は契約状態・外した端末は停止)、`/api/license/release`、activate に端末名+日時。検収で「登録失敗の端末まで止まる」誤りを発見→外した端末だけを記録する `rm:<kid>` 方式に修正。
- **アプリ**: `runSyncCycle` 入口に関門(24時間ごと確認・7日猶予)。鍵が無くても Drive 接続があれば同期できた抜け道も解消。SYNC 欄に停止表示3種・端末一覧と2段階の「外す」(案A)。
- **公開側**: 料金ページのボタン→Paddle.js 決済(環境変数が無ければ押せないまま)、購入完了ページ `/purchase`(15言語・noindex)、「取引ID」表記に統一(領収書メールの表記に合わせた)、プライバシーの Paddle/端末情報の記載を実装に合わせて改訂(15言語)。
- **Sandbox 通しテスト**: 偽カード購入→鍵表示→取引IDで再表示→有効→解約で停止→端末を外すと停止/入れ直しで復帰、すべて成功。領収書メールに txn_ が載ることを確認。
- **本番準備**: 商品・価格(税込)・Default payment link・Google Pay・公開キー・Cloudflare Secret(PADDLE_API_KEY 読み取り専用・PADDLE_ENV)。`.env.production` の公開値は書類確認までコメントアウト。
- 学び: Next.js の `.env.local` は本番ビルドにも読まれる → Sandbox 値は `.env.sandbox`(自動読込されない名前)に。手元の wrangler は `--compatibility-date=2026-04-22` が必要。ユーザーはターミナル不可→秘密の登録は管理画面で案内。

## s221 続き (2026-09-25〜26) — 同期を業界水準へ作り直し・スマホ操作・LP 自動言語

- **症状**: スマホ/PC で「オフライン」「同期中が10分続く」「リロードしないと反映されない」。実測で原因を切り分け: 家の回線は上り90Mbps・Cloudflare へは高速だが、**Google ドライブのアップロード窓口だけ ≈20KB/s で20〜30秒で切断**(ダウンロードは高速)。
- **同期形式 v2**(設計 `docs/superpowers/specs/2026-09-25-sync-format-v2-design.md`): ブクマ/カードを16個の gzip 箱に分割、版番号で比較して変わった箱だけ送受信(1.48MB→≈30KB)、v1 から非破壊で自動移行、3秒デバウンス+10秒ポーリング。敵対レビュー2回で発見した重大問題(キー順での延々送り合い・board-config 往復・送信失敗後に送り直さない・重複ファイル・ゴミ箱を空にしたブクマの復活・保存画面からの書き込み漏れ)を全修正。
- **その他の同期修正**: 全サイクルを Web Locks で直列化、全通信にタイムアウト+5分上限、端末の記録をバックアップから除外(インポートで端末番号がずれる不具合)、ボードが同期受信で即更新、リンク確認の日時が同期で巻き戻り延々書き込む不具合(merge で lastCheckedAt 新しい側を保持)。
- **実機合格**: PC↔iPhone 追加が約20秒・同時追加の取りこぼし無し(ユーザー確認)。
- **UI**: SYNC 欄を作り直し(状態の帯=同期ボタン、Google ドライブ/端末の行、同期の記録は見出し5回タップで表示)、スマホで長押し複数選択→ゴミ箱/復元+元に戻す、スマホ絞り込み一覧がナビに隠れる問題と指スクロールで並べ替わる問題を修正。
- **LP**: 訪問者の言語で自動表示(Cloudflare で振り分け・自分で選んだ言語は Cookie で優先・検索ロボットは英語のまま)。
- **無料キーのプレゼントページ `/gift`**(日本語のみ・購入完了ページと同じデザイン): 「キーを受け取る」を押した時だけ発行(LINE 等のリンクプレビューや再読み込みで枠を消費しない)。旧 `/claim?c=` は `/gift` へ 302。発行は `POST /api/license/claim`。友達用リンク=`claim:` KV の既存レコード(label launch・上限50)。
- **`/gift` のデザイン作り直し**(ユーザー「完璧」): 左=大見出し「AllMarks / 同期キーのプレゼント」+説明、右=白いチケット型カード(切り取り線・光が横切る・「無料・期限なし」札・期限/端末/保存先の3行)+横幅いっぱいの黒い「キーを受け取る」(緑の点・脈打つ輪・矢印)。受け取り後は同じカード内にキー+コピー/開く。スマホは縦並び。`GiftContent.tsx` + `gift-page.module.css`(purchase/legal の CSS 依存を撤去)。
- スマホ絞り込み一覧が「+」保存ボタンにも被らないよう修正。

## s222 (2026-09-26〜27) — LP の手直しを本番反映 → LP を作り直す見本 v10 を承認 → 本番実装を開始

- **本番反映(master・allmarks.app)**: LP ヘッダーのガラス(スクロール時 白30%+ぼかし22px)と、落ち着いた事実だけの文面(15言語・日英はユーザー承認)。commit 5932d87d。
- **見本 v10(ユーザー承認・`docs/private/lp-v10-mock.html`)**: 名画素材をやめ、白黒+3色のアクセントで自作した絵とツイート風カード(AllMarks のアイコンと名前)で構成。白いボードが動くヒーロー(貼り付け→保存の繰り返し)、「溜めるだけで、見返さない。」(一覧→線→ボード)、流れる帯(スクロールの速さで加速・逆向き)、固定して見せる6段の機能紹介(01保存・02ドラッグ・03本当に再生される動画と MOTION で停止・04タグで絞り込み・05登録なしでボードが開く・06 SHARE→画像→リンクをコピー。カーソルが実際に押す)、回る円のボタン(触れると巨大な弧)と大きな AllMarks の流れる輪郭文字。ユーザーの好み(直線・スクロール連動・超軽量・うねる線/ロゴの大きな使用/文字化け演出は不可)は memory `feedback_lp_motion_direction` に記録。
- **本番実装を開始(ブランチ feat/lp-v10・未マージ)**: 設計 `docs/superpowers/specs/2026-09-26-lp-v10-design.md`、計画 `docs/superpowers/plans/2026-09-26-lp-v10.md`(10タスク)。サブエージェント方式で Task 1(新しい文言11キー+見出しの改行を15言語に。タイ語の改行で承認文に空白が混ざった不具合をレビューで検出・修正、回帰テスト追加)と Task 2(動きの計算の純関数・19テスト)を完了。Task 3 以降は次セッション。同期区画は有料公開日に出す。
- **ボード UI の棚卸し**: `docs/superpowers/specs/2026-09-26-board-ui-inventory.md`(BoardRoot 約4,800行・部品約150・文言の多言語化の混在などの不揃いを記録)。白い既定テーマ+モダンな UI へのボード刷新は LP の後。
- 途中で API の利用上限に一度かかり、実装係を再開して継続。夜の中断時点で作業ツリーはクリーン(Task 3 は未着手)。
