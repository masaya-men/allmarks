...
user 提供の SVG (= 黒 A 形 + 緑チェック overlay) を `extension/icons/floating-button-mark.svg` に配置。 黒 A は **AllMarks のロゴモチーフ** を直接アイコン化したもの。

レイヤー構成 (= SVG 内):
- `mark` パス = 黒の A 形 (= AllMarks の A をデザインモチーフに、 シャープなスラブ風)
...
|---|---|---|---|---|
| idle (= 未保存、 アイドル) | 黒 A 表示 | 非表示 | なし | 設定値 (= デフォルト 30%) |
| idle-hover | 黒 A 表示 | 非表示 | なし | 100% |
| saving | 黒 A 表示 | 非表示 | spinner ring | 100% |
| saved | 黒 A 表示 | reveal アニメ | なし | 100% |
| already-saved (= 既保存ページ再訪) | 黒 A 表示 | 永続表示 (= アニメ無し) | なし | 100% |
| already-saved-idle | 黒 A 表示 | 永続表示 | なし | 設定値 (= デフォルト 30%) |
| error | 黒 A 表示 | 非表示 | red bang + shake | 100% |
...
ボタン本体は **背景パネルなし** (= 黒 A マークそのものをボタンに)。 user 提供 SVG はそれ自体が形を持っているので、 ガラス pill の背景は不要。 hover 時に subtle なグロー halo を mark 自体に重ねる。
