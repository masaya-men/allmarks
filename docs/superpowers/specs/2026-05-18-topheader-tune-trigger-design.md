...
## Amendment 1 — 数字 = スライダーハンドル化 (= 2026-05-18 セッション 41 後半)

### きっかけ

セッション 41 初回 ship 直後の user feedback: 「ホバーで出てくるのはスライダーにできる？短めでいいから超精密、 ハンドルを数字に」 + 「W と G ラベルも書かなくていい、 触って気付いてもらう」 + 「敢えて情報削って」

### 変更点

**TUNE → scramble → settle 後の readout 形** (= idle-readout 状態) を text-only から「短いピル track + ナンバーチップ × 2 + ↺」 に変更。

**Before** (= 初版 ship):

```
TUNE   →   W 267.84 · G 97.21 · ↺
```
...
各 `[───●───]` は 100px のピル track + 黒 chip ナンバー。 chip 位置 = `(value - min) / range × travel`。

### 確定仕様

- **ピル track**: 100px 幅、 `1.5px` 高さ、 `rgba(255,255,255,0.18)` 背景、 `border-radius: 1px`
- **ナンバー chip** (= ハンドル本体):
  - 数字の text は orange accent `rgba(255,200,120,0.95)` 維持
  - chip 背景 `rgba(0,0,0,0.85)`、 `border-radius: 3px`、 padding `2px 4px`
  - chip 位置 = track 上で `(value - min) / range` に応じて `translateX`
  - `cursor: ew-resize`、 `user-select: none`
- **`W` `G` ラベル削除** = chip 数字単体で表示 (= user の「情報削って」 方針)
- **`·` separator** 維持 (= 2 つの slider 間の視覚区切り)
- **`↺` reset cell** 維持 (= 末尾、 chip 化はせず文字のまま)
- **scramble アニメは維持** (= v4-inplace、 stagger 11ms / scramble 125-190ms)
  - ただし cells が ~18 個に減る (= "W " "G " 4 文字消えた分): 旧 22 cells → 新 18 cells
  - 完了時間: `17 × 11 + 190 = 377ms` (= 旧 421ms から短縮)
  - scramble 中も chip 背景 + pill track は表示 (= chip 位置は target value で固定、 内部の digit cell だけが random char を巡回)

### 超精密 drag 化

- **MOUSE_PX_FOR_FULL_RANGE** = 10000 → **30000** に bump (= 3× より精密)
- 計算結果: 1 マウスピクセル ≈ 0.02 W 単位 / 0.01 G 単位 = サブ整数を smooth に狙える
- **Shift + drag = 10× 速い** (= 既存挙動維持、 SHIFT_SPEED_MULTIPLIER 10)
- 制約: フル範囲を移動するには 30000 マウスピクセル必要 (= 大画面 16 枚分)、 急ぎ大ジャンプは Shift+drag で

### chip 位置計算 (= 実装メモ)

ピル track 上での chip の `left` (px) を計算:

```typescript
const fraction = (value - min) / (max - min)
const trackPaddingPx = 18  // chip 半幅 ≈ 9px × 2、 chip が両端を超えないよう inset
const travelPx = TRACK_WIDTH_PX - trackPaddingPx
const chipLeftPx = (trackPaddingPx / 2) + fraction * travelPx
```

`TRACK_WIDTH_PX = 100` (確定)。 chip は `position: absolute; top: 50%; transform: translate(-50%, -50%); left: ${chipLeftPx}px`。

### DOM 構造変更

旧 `buildReadoutCells` は flat な cell 配列 (= 22 個の単一 span)。 新構造は階層型:

```
.readout
├── .slider-wrap (W)
│   ├── .track
│   └── .chip (= 6 個の cell span: 2, 6, 7, ., 8, 4)
├── .cell.dim (= " · ")
├── .slider-wrap (G)
│   ├── .track
│   └── .chip (= 5 個の cell span: 9, 7, ., 2, 1)
└── .cell.reset (= ↺)
```

scramble 中も同じ階層、 chip 内 cell が random char に置き換わるのみ。 chip 位置は scramble 中も target value で固定。

### data-cell-kind マッピング更新

旧:
- `num-w` / `num-g` / `dim` / `reset` / `label`

新:
- chip 内の digit cell には個別 testid 不要 (= drag は chip 全体で受ける、 setPointerCapture)
- `.chip[data-scope="w"]` / `.chip[data-scope="g"]` でドラッグ判定
- `↺` は `[data-cell-kind="reset"]` 維持 (click handler)
- `·` separator は `data-cell-kind="dim"`

### 範囲外 (= Amendment 1 では実装しない)

- chip click (= drag せず) で値直接入力モーダル: 将来案
- chip ダブルクリックで default 値戻し: 将来案 (= 現状 ↺ で OK)
- mobile での touch 対応: 既存通り B-#10 に合流
- chip の hover-only ハイライト: 必要なら polish 後で

---

*Amendment 1 確定: 2026-05-18 セッション 41 後半。 Visual Companion で A 案 (ピル+黒 chip) ライブデモ確認 → user 即承認。 実装 → 直接 inline (= 既存 TuneTrigger.tsx + .module.css の少量変更で済むため subagent-driven は skip)。*
