...
> Your AllMarks data is stored only in your browser on this device, and no copy is kept on our servers. Because of this, keeping your own backups of anything important is your responsibility: use the EXPORT feature in Settings to save a copy regularly — especially before changing devices, clearing your browser, or resetting AllMarks. We cannot recover data lost through browser data clearing, device loss or failure, software updates, or any other cause.

**日本語（参考訳）**：
> あなたの AllMarks のデータは、この端末のブラウザ内にのみ保存され、当方のサーバーには一切保存されません。そのため、大切なデータの控えを保つ責任はあなたにあります。設定内の EXPORT 機能で定期的に控えを保存してください（特に端末の変更・ブラウザデータの消去・AllMarks のリセットの前に）。ブラウザデータの消去・端末の紛失や故障・ソフトウェア更新・その他いかなる原因で失われたデータも、当方では復元できません。

※ §6 と重複気味だが、§6 は「無保証・責任を負わない」（免責）、§3 追記は「あなたに控えを取る責任がある」（積極的義務）＝役割が違うので両方あってよい。**最終文面は弁護士確認**。
...
**文言（前向き・怖くしない・英語ソース）**：
- 見出し: `Your boards live here — on your device.`
- 本文: `AllMarks keeps everything in this browser, on this device. No account, no cloud, nobody watching — your collages are private and yours. Because they live only here, keep a copy of anything you love with EXPORT (in Settings), so it stays safe and you can carry it between devices.`
- ボタン: `Got it`
- （任意の副リンク）: `Learn more`（→ 規約 or ヘルプ）は入れなくてよい（摩擦を増やさない）。

**日本語（参考訳）**：
- 見出し「あなたのボードはここに — この端末の中に。」
- 本文「AllMarks は全部この端末のブラウザ内に保存します。アカウントもクラウドも監視もなし＝あなたのコラージュは非公開であなたのもの。ここだけにあるから、大切なものは EXPORT（設定内）で控えを。安全に保て、端末間でも持ち運べます。」
- ボタン「Got it」
...
**離脱回避の要**：警告でなく「プライバシー自慢」として見せる／一度きり／ボタン1つ／怖い言葉・チェックボックス・「責任を負う」はUIに出さない（それは規約が担う）。
...
1. 前回 EXPORT から**新規で貯まった件数 ≥ N**（初期値 **15**、`last-backup-at.count` と現件数の差）。一度も控えが無い場合は「現件数 ≥ N」。
...
**文言（優しい・英語ソース）**：
- 本文: `You've saved {n} new bookmarks since your last backup. Keep them safe?`（一度も無い場合: `You've saved {n} bookmarks. Keep a copy so they're safe?`）
- ボタン: `Back up (EXPORT)` / `Later`
- 日本語（参考訳）「前回の控えから新しく {n} 件たまりました。控えを取っておきますか？」／ボタン「控えを取る（EXPORT）」「あとで」
...
| `last-backup-at` | `{ key, at: ISO, count: number }` | EXPORT 成功時（`BackupButton.onExport` に副作用追加。count=EXPORT時のブクマ件数） |
