...
- **D1 指＝カード操作が主**: タップ＝選択／1本指＝移動／**2本指＝選択カードの拡縮＋回転**。既存の「非選択で2本指＝ボードズーム」は**温存**（密ボードでは効きにくいが、機能として残しても邪魔にならない＝ユーザー判断）。＝**`MobileArrangeGestures` のピンチ振り分けは無改変**。
- **D2 ボードズームの主導線は専用ボトムナビのボタン**（−／＋／全体表示(fit)）。指のボードズームは残すが、確実に効くのはボタン。
...
- **D5 DELETE＝共有“画像の見た目”から外すだけ**。リンクの URL 集合（`buildArrangeShare`＝選択で確定した全URL）とブックマーク本体は**無傷**。**モバイル＋PC 両方**に用意。
  - **D5-a 外すたびに毎回、非ブロッキングのトーストで確認**（「画像から外しました — リンクには全URLが残っています ・ 元に戻す」）。＝「これは自由なコラージュ（見た目づくり）の操作で、送る中身は減らない」を毎回伝える。**この説明文は母国語（i18n）で出す**（下記 言語方針）。任意で隅に小さな常設一言も可。
...
- **言語方針（本設計で明確化・ユーザー確定）**: **1単語の標準アクション（UNDO/REDO＝矢印アイコン・CREATE・BACK・前面へ/背面へ・外す＝ゴミ箱アイコン等の短いラベル）＝英語 or アイコンで統一**（世界共通の UI 語彙）。**説明を含む文（DELETE のトースト等）＝ユーザーの母国語**（`useI18n` の `t()`／`messages/*.json` 15言語／`all-keys-parity.test.ts` があるので**新キーは15言語全部に追加**）。BoardRoot は既に `const { t } = useI18n()` を持つ。
...
### 2. モバイルのジェスチャー＝`MobileArrangeGestures` は無改変（温存）
- 既存のピンチ振り分け（選択中＝カード拡縮回転／非選択＝ボードズーム）・1本指移動・余白タップ解除・余白ダブルタップ整列は**そのまま残す**（ユーザー判断＝2本指ボードズームは効きにくくても邪魔にならない）。
- ＝**この作り直しでジェスチャーのコードは触らない**。ボードズームの“確実に効く”導線としてナビのボタン（下記3）を**追加**するだけ。
...
- 外した瞬間、非ブロッキングの小トースト（body portal・`data-no-capture`・自動で数秒後に消える）。**説明文は母国語（i18n）**：新キー（例 `board.collageRemoveToast` ＝「画像から外しました。リンクには選んだ URL が全部残っています」相当）を **`messages/*.json` 15言語すべて**に追加し `t('board.collageRemoveToast')` で表示。**「UNDO」ボタンは英語1単語**（or 矢印アイコン）。
- UNDO は既存の `handleCollageUndo`（＝直近の外すを1手戻す）に接続＝「消えてない」を毎回体感。
- 任意: アレンジ画面の隅に小さな常設一言（同じく母国語・別キー）。まずはトーストのみで出荷し、常設一言は要否を実機で判断。
...
- e2e `tests/e2e/mobile-share.spec.ts`:
  ...
  - ボードズーム: ナビの `−`/`＋`/`全体` で `mobile-arrange-stage` の scale が変化／`全体` で 1。既存の2本指ボードズームのテストは**そのまま**（温存＝変えない）。1本指移動・カード2本指拡縮も従来どおり。
