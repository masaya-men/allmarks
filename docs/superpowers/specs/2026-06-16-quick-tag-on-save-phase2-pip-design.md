...
- **おすすめ順は既存の共有関数 `orderTagsForSave` をそのまま呼ぶ**（[lib/tagger/order-tags-for-save.ts](../../../lib/tagger/order-tags-for-save.ts)）。これは「既存タグを関連順に並べた `QuickTag[]`」を返す関数で、第1段の帯（save-iframe 経由）が既に使っているもの。PiP は既存タグのみ・新規作成なしなので、新規候補を混ぜる `computeSuggestedEntries`（CardsLayer 内）ではなく `orderTagsForSave` が正しい再利用先。新たな切り出し・refactor は不要。PiP は本体オリジンなので `getAllBookmarks`（corpus）＋ `getAllTags` を読んで `orderTagsForSave(bookmark, corpus, allTags)` を直接呼べる。
...
5. **PiP カードのタグ付け（本体・React）** — PipCard にアクティブ時の「＋」＋コンパクト帯。`getAllBookmarks` + `getAllTags` + `orderTagsForSave`（既存共有関数）/ `addTagToBookmark` / `postBookmarkUpdated` を直接利用。
...
- PiP タグ付け：`orderTagsForSave` で並べた既存タグをチップ表示し、タップで `addTagToBookmark` + `postBookmarkUpdated` が呼ばれることの単体テスト。`orderTagsForSave` 自体は既存テスト済みなので再利用のみ。
