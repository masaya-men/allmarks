...
5-1. 既存 `handleSelectedPinchStart` に `handleCollageGestureStart()` を1行足す。**⚠️ 訂正（実装時に opus レビューが Critical を検出）**: この行は既存の `collageArbiter.cancelActive()` の**直後**に置くこと（当初「先頭に」は誤り）。理由＝`cancelActive()` は進行中の1本指ドラッグの終了を**同期的に**呼び、その終了が `handleCollageGestureEnd` を発火して pending を消費する。`handleCollageGestureStart()` を先に置くと、この同期終了がピンチ用に確立した pending を消してしまい、**2本指ピンチが undo に一切積まれない**。正しい順序:

```ts
    collageArbiter.cancelActive()   // 先: 進行中ドラッグを終了（その pending を消費）
    handleCollageGestureStart()      // 後: ピンチ用 pending を新規確立
```
