# 次セッションのゴール — N-70修正の本番デプロイ確認→残り候補から1つ選んで着手

## ★s205 の到達点(N-77根治・デプロイ済／N-70根治・未デプロイ)
- **N-77**(YouTubeサムネがスクロール中に灰色になる): `public/sw.js`がサードパーティ画像まで無差別インターセプトし、スクロールでのアンマウント時に`respondWith(undefined)`→ネットワークエラーになっていたのが原因。クロスオリジンの早期returnを追加(`CACHE_VERSION`もbump)。TDD。tsc0/vitest2514緑/e2e確認/build成功。**push・`allmarks.app`デプロイ済**。
- **N-70**(デッドリンクの誤判定): `lib/board/tweet-liveness.ts`の`checkTweetLiveness`が「200だがTweet形式でなければgone」と判定していたのが原因。本番`curl`で実CDN検証し「本当に存在しないツイートは404、200のtombstoneは"匿名チェッカーから見えないだけ"(鍵アカウント等)」と実証、後者を`gone`→`unknown`(安全側)に変更。TDD。tsc0/vitest2514緑/e2e緑/build成功。**未push・未デプロイ**。
- 副産物: `functions/api/ogp.ts`の502丸め込みバグ(一般サイト側は`gone`に絶対到達しない)を発見、N-70調査で原因ではないと確認済(修正は別スコープのため見送り)。
- 詳細は`docs/TODO.md`セッション205節・`docs/TODO_COMPLETED.md`参照。

## ★次セッション最優先＝N-70デプロイ確認→残り候補
1. ユーザーにN-70修正のcommit・push・本番デプロイ可否を確認 → OKなら実行・実機確認依頼。
2. その後、`docs/TODO.md`「session 204 で提案」節の残り7件(N-69/71〜76)から次を選ぶ。着手前に該当項目だけ改めてsuperpowers:brainstormingから。
