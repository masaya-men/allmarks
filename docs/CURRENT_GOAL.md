...
3. **#7 ライトボックスの戻り先【実装・デプロイ済・目視待ち】** — ナビで①source を見ていたカードに更新 ②そのカードが画面外なら盤面を一発ジャンプ（アニメ無し＝frosted backdrop の連続再ブラー回避）でセンタリング → 閉じると既存 close FLIP が最後のカードへ着地。ロジック `lib/board/lightbox-return.ts`（`computeNavReturnScrollY`・単体7本）、BoardRoot `returnToViewedCard` を `handleLightboxNav`/`handleLightboxJump` に配線。tsc0・vitest2086。**⚠ ライトボックスは Playwright で開けない**（reorder-drag の `setPointerCapture` が合成/CDP を弾く・probe 済）→ 見た目は**本番 目視**必須。#6 も同様。
