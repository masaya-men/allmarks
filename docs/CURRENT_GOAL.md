# 次セッションのゴール — 端末間同期 束6③(vault食い違いUI・isPrivateVaultタグ重複・EMPTY TRASH文言)
...
- **束6②(SyncPanel UI本体)の残り、全て完了(s214)**。初回接続フロー・接続後の同期状態表示・「今すぐ同期」・エラー表示(破損ファイル・大量削除確認・容量不足・トークン失効時の再接続)・裏で動く自動同期ランナー(タブを閉じる/裏に回す/次に開く時の自動pull・push)を実装。`feat/sync-panel-connect-flow`ブランチで8タスクplan完遂・各タスクレビュー済・フルスイート**2892/2892**・tsc0・build OK。**まだmasterマージ前**(このブランチのまま次セッションへ引き継ぎ)。詳細=TODO.md s214ログ参照。
...
- **リアルタイム反映(タブを開いたまま・編集直後に他端末へ即反映)が可能かどうか、必ず検討・記録すること**(2026-09-16 ユーザー指定)。現状の設計(`sync-controller.ts`の`markDirty`)は「編集20秒後」の仕組み自体は既にあるが、呼び出し元(全保存箇所への配線)がゼロ。束6②では**配線しない**提案でユーザー承認済み(今回は「タブを閉じる/裏に回す/次に開く」タイミングのみ自動同期)。リアルタイム化の可否・コスト・採用するかは**別途この場で検討し、ここに結論を書くこと**(検討済みにせず先送りするだけの扱いは禁止)。

## ★次セッション = 束6③(vault食い違いUI・isPrivateVaultタグ重複・EMPTY TRASH文言)— 唯一の残タスク

設計書 §4.1(SyncPanel)参照。責務(設計書§4.1)+ これまでの申し送り:

### ②SyncPanel UI本体 → ✅完了(s214)
初回接続フロー・同期状態表示・「今すぐ同期」・エラー表示(破損ファイル・大量削除確認・容量不足・トークン失効時の再接続)・裏で動く自動同期ランナーの`BoardRoot`常時マウント、全て実装・テスト済(未着手ゼロ)。詳細=TODO.md s214ログ参照。

### ③vault食い違いUI・isPrivateVaultタグ重複・EMPTY TRASH文言(未着手・次にやること唯一の残り)
- **vault(金庫)食い違い時のUI**: 黙って進めずユーザーに選ばせる、または今回作ったパスワード再設定導線への案内。②が完了したので設計しやすい(「食い違ったら再設定してね」と言える)。
...
- 実装ノート: `sync-controller.ts`の`onResult`コールバックをここで初めて使う。コールバック内で例外を投げないこと(`void flushNow()`経由だと未処理rejectionになりうる)。

### ★束6着手前 or 並行の運用セットアップ(ユーザーの手作業・実機確認に必要)

- claimレコードを1件seed: `wrangler kv key put --binding=K3_KV "claim:<合言葉>" '{"label":"launch","issuedCount":0,"maxIssue":50,"active":true}'`(②が完了したので着手可能・支援者に配る合言葉を決めてから)
- ユーザー本人が無料で使うための専用キーを1本発行して渡す(秘密鍵で直接署名・課金フロー不要)。②が完了したので着手可能だが、まだ発行していない。
