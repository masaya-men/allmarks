...
**監査で確定した44件の修正を「安全に確実に」continue。重い項目は実機計測/確認しながら、バッチごとに commit+deploy。次は B4(保存経路のセキュリティ)から。**
...
- 修正完了＋本番反映済（4 commit）:
  ...
  - **rank1 スクロールでカードが並び替わる**（カード高さ計算を決定論化＝マウント順非依存。共通純関数 itemSkylineHeight に一本化）→ **ユーザー実機での最終確認待ち**（計測はGSAP tweenで交絡、決定論は単体テストで証明済）
  - B2 プライバシー掃除（実メアド/実名/競合・収益記述を docs/private へ退避、robots/sw 微修正）
  - 実機FB: 旧ブランド紫アクセント→ブランド緑 #28F100 に統一、PWA色も黒へ

## 残りの作業キュー（B4 以降。詳細は progress.md）
- **B4 保存経路セキュリティ＋重複統合**（rank2 危険URL保存→後で開くと攻撃 / rank12 任意サイトから勝手保存 / rank14 重複判定3箇所コピペ統合 / rank30 同時保存で2枚）★次ここ
- **B5 バックアップ安全化＋配線**（rank3 復元でカード配置消失 / rank8 無言失敗 / rank15 BackupButton配線=**置き場所要相談**）
...
## ユーザー確認待ち / 要相談
- **rank1 実機確認**: allmarks.app をスクロールしてカードが動かないか（動く/左すき間残るなら F5=skyline詰め方を追加対応）
