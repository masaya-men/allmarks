...
2. **まず user に「削除ツイートが DEAD LINKS に出たか」 の実機確認結果を聞く** (session 90 で本番 ship 済、 user の実 IndexedDB に残る削除ツイートで確認してもらう手はず)。 もし出ない場合: 7日 guard により「前回チェックから 7 日未満のカードは再チェックされない」 のが原因の可能性 → そのカードを Lightbox で開いて時間を置く or 別の古いカードで試す。 真に出ないなら調査 (`linkStatus` が gone になってるか devtools の IndexedDB で確認 → なってればフィルター側、 なってなければ検出側)
