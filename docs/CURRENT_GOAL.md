...
7. **★同セッション追補＝下部のズーム・スライダー**（実機で「100枚だと余白が無く選択解除できずボードズームに入れない」と判明→追加）: **カードを選んだ状態のまま**下のスライダーを右へ → ボードが拡大するか。**選んだカードが中心に大きくなる**か。**100枚でもスライダーでズームできる**か（＝穴が塞がったか）。1×（左端）でフィットに戻るか
8. **CREATE** → できた画像が「並べたとおり」か（**ボードのズームは画像に影響しないはず**）
9. 100枚でも 2〜8 が破綻しないか（重い/カクつく等あれば教えて）
