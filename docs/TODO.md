...
### 直近の状態 (セッション 89 — board 重さの実測で「枚数は主因でない/culling 完璧」と判明→据え置き / LoPo の mixed-media tweet 抽出 skip 最適化を移植 ship)

**調査 → 据え置き**: 2 大タスクの「重い問題」に着手。 実バックアップ (567 件) + 実 GPU で計測した結果、 **culling は既に完璧** (567 件でも DOM 約 20 枚、 通常スクロール 54fps、 メインスレッド詰まりゼロ、 高 DPR でも劣化なし) → **「枚数が多いから重い」 は誤りで virtualization は解決済み**と判明。 唯一の難点は「スクロールメーターで遠ジャンプした時に着地点画像が 1〜2 秒遅れて出る (空箱 pop-in) + 約 100ms の mount ヒッチ (開発ビルドで誇張)」 のみ。 業界ベストプラクティス (即プレースホルダ / prefetch / scroll-seek / content-visibility) を出典付き調査済。 **user 判断で据え置き** (発生は遠ジャンプ限定・自己解消・現状で最適)。 研究と 4 層計画は IDEAS.md に退避。

**ship 済 (= master + 本番 booklage.pages.dev 反映済、 commit `perf(board): skip video-frame extraction for mixed-media tweets`、 1 commit + 1 deploy)**: LoPo (FF14Sim ハウジング) の最適化「**動画 + 写真 両方の mixed-media tweet は動画フレーム抽出を skip**」 を移植。
- ゲート: tweet で写真スロットがあれば抽出しない → `lib/board/tweet-video-extraction.ts` に切り出し (純粋・テスト可能、 CardsLayer から移設)
- ambient: `resolveSlideshowFrames` が mixed なら写真群 + 動画ポスターを返す (= 抽出フレームの代替)
- spotlight (hero 本再生) は別経路で不変
- 効果範囲 (実データ): mixed = **4 件** / video-only 151 (従来どおり抽出) / photo-only 50。 純粋なコスト削減 (video decode + canvas + JPEG 省略)、 mixed が増えるほど効く
- **検証の教訓**: `/api/tweet-video` は抽出と hero 再生の両方が使うためネットワーク信号では区別不能 → ゲート関数を lib 化し**実 mixed 形状でユニットテスト**して決定論的に証明。 tsc clean / 全 914 tests pass

詳細 narrative: [TODO_COMPLETED.md](./TODO_COMPLETED.md) セッション 89 セクション
