...
- **ユーザー実機メモ7件を分類・記録。うち実施2件を `fix/board-small-nits` で出荷 → `--no-ff` で master マージ済（ローカル/リモート削除）。tsc0 / vitest1867 / build OK。**
  - **✅ N-17 EMPTY TRASH を赤 danger 表示**（ChromeButton に `data-variant="danger"`＝TRASHで出る破壊操作を目立たせる。他ボタンは byte-identical）。**本番反映・ユーザー確認OK**。
  - **✅ N-18 拡張クイックタグ窓の見切れ**（2列固定グリッド→**1列スクロール**。full幅で "read later"/"ui inspo" が切れない・…安全網・下端フェード。実CSS再現で検証）。**拡張 v0.1.22 パッケージ済（`dist/booklage-extension-0.1.22.zip`）→ ストア更新提出待ち（ユーザー操作）**。
  - **🅿 N-16 空ボードの青モーダル**＝**スマホ限定**（未対応）→ 保留。色トークンだけダーク化して残置（デスクトップは背景ワードマークに occlude され不可視＝実害なし・将来スマホ対応時に効く）。
  - **⏹ N-14 Lightbox 中のボードモーション**＝カード浮遊/スライドショー/動画は既に `ambientOn` gate で停止済＝**対応不要**（ScrollMeter だけ常時稼働だが index 表示兼務）。
- **残り（次セッション）**：N-15 拡張の再起動後 初回保存失敗（要 reboot 再現・SW コールドスタート調査）／ N-19 カードのサイズ・位置を default に戻す（要設計）／ N-05 LP ナビ格納演出（要設計）。詳細 [CURRENT_GOAL.md](CURRENT_GOAL.md)。
