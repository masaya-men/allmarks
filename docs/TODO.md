...
### 直近の状態 (セッション 205 — ★N-77(YouTubeサムネがスクロール中に灰色になるバグ)+N-70(デッドリンクの誤判定)をsystematic-debuggingで根治・N-77は本番デプロイ済/N-70は未デプロイ)

**session 204で出た9件のブラッシュアップ候補(N-69〜N-77)から「好きに進めてOK」を受けて、N-77→N-70の順に選定・着手。** 詳細は[TODO_COMPLETED.md](TODO_COMPLETED.md)。
...
**N-70(修正済・未デプロイ)**:
- **調査で判明**: 一般サイト(非tweet)側は`functions/api/ogp.ts`が上流の非2xxを常に502へ丸めてしまい、`gone`判定に絶対到達しない(常に`unknown`扱い)ため、この経路は誤判定の原因ではないと判明(修正は別スコープ、副産物として記録のみ)。
- **根本原因**: tweet側`lib/board/tweet-liveness.ts`の`checkTweetLiveness`が「200だがTweet形式のペイロードでなければ`gone`」と判定していた。本番`curl`で実際のCDN(`cdn.syndication.twimg.com`)を検証したところ、**本当に存在しないツイートは綺麗な404を返す**ことを確認(=200のtombstoneにはならない)。つまり200+非Tweet形式が起きるのは「ツイートIDは有効だが、鍵付き(protected)アカウントや年齢制限等で、ログインしていない匿名チェッカーからは見えない」場合のみ。ユーザー自身がフォローしている鍵アカウントのツイートは本人には生きて見えるのに、これが永久に「デッドリンク」(灰色化+クリックブロック)として扱われていた。
- **修正**: 200+非Tweet形式の場合の判定を`gone`→`unknown`に変更(安全側=見た目は変わらず1時間毎に再試行されるだけ、灰色化・クリックブロックはされない)。本当に存在しない場合(404)は引き続き`gone`のまま。TDD(既存3テストの期待値を書き換えてRED確認→修正→GREEN)。
- **検証**: tsc0/vitest全301ファイル2514緑/関連e2e(board-b-11-source-hide/triage-flow)緑/`pnpm build`成功。
- **★次セッション最優先**: ユーザーにN-70修正の本番デプロイ可否を確認してから`allmarks.app`へデプロイ。OK後は残り7件(N-69/71〜76)から次を選ぶ。
