...
- **(N-49) ★スマホ・タブレットから SHARE できない（s184 発見・ローンチ前・規模M）** — 本体ボードも 640px 以下で `TopHeader` ごと `display:none`（[TopHeader.module.css:65](../components/board/TopHeader.module.css#L65)「TITLE / TUNE / POP OUT / SHARE are desktop-only for v1」）＝**スマホから共有リンクを作れない**。「コラージュを画像で SNS シェアさせてバイラルを起こす」というミッションの根幹。**調査済（s184）＝真のブロッカーは2つだけ**:
...
- **(N-51) スマホでボード背景（タイトル・パターン）が見えず、テーマの意味が薄い（s184 ユーザー要望）** — **事実**: スマホのボードでは背景ワードマークは**そもそも描画されていない**（`!isMobile` ゲート・実測 390px で 0 個）。受け取り画面では逆に描画されている。加えて 3 列密グリッド（`MOBILE_LAYOUT` 列3・gap6・外枠0）が画面をほぼ埋める。**候補**: ①2 列にする ②3 列のままカードを縮めて左右・カード間に背景を覗かせる ③背景ワードマークをスマホでも描画。調査＋モック撮影中。
- **(N-52) パターン（グリッド・ドット等）の太さをスライダーで調整したい（s184 ユーザー要望・PC/スマホ共通）** — **調査済**: 太さは**2箇所にベタ書き**＝`patternSvgDataUri`（`stroke-width='1'` / dots `r='1.4'`、受け取り画面とミラーが使用）と `themes.module.css` の CSS グラデーション（`1px` / `1.4px`、**本物盤面と SHARE スクショが使用**）。両方を変数化しないと盤面と共有リンクで線の太さが食い違う。`patternStroke?: number` を追加（**線は既定1・ドットは既定1.4** ＝ 既存の見た目を1px も動かさない）。**DB のバージョン上げ不要**（`board-config` の JSON 塊の中）。共有スキーマは `.optional()` 必須（必須にすると既存の共有リンクの `custom` が丸ごと落ちる）。描画時に `min(太さ, パターン間隔/2 − 1)` で頭打ち（間隔16でベタ塗りになるのを防ぐ）。UI は既存 DENSITY 行の真下に同じ見た目の THICKNESS 行。実装中。
