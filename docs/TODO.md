...
- **(N-48) ★共有受け取り：スマホ・タブレットに「取り込む」導線が無い（s184 実機・必須改善）** — **実測**（本番 `/s/LJ41eU`）: スマホ 390px では `.frameTopChrome` が `display:none` のため **IMPORT ボタンが 0×0 ＝完全に不可視**（上部クローム全体が消えるので受け取り側 SHARE も同時に消失）。タブレット 820/1024 では見えているが **196×27px**＝指の最小タップ寸法（32px）未満で実質使えない。**確定した設計（ユーザー承認済）**: 触り端末（`isMobile || isTouchDevice`）に**下部固定バー**を新設し、画面幅いっぱいの主役ボタン `IMPORT <n> TO YOUR BOARD`（高さ 52px）を置く。上部の細い IMPORT は触り端末では描画しない。スクロール中も出しっぱなし。押下は**既存 `handleSave` をそのまま**（× で消した残り＝見えている全部を取り込み→進捗→ボードへ）。見た目は `BoardMobileNav` と同素材（暗いガラス・上端ヘアライン・`env(safe-area-inset-bottom)`）。**未決**＝タブレットで ScrollMeter を出すか（下部バーと衝突・調査中）。マウス PC は不変。
- **(N-49) ★スマホ・タブレットから SHARE できない（s184 発見・ローンチ前・要専用設計）** — 本体ボードも 640px 以下で `TopHeader` ごと `display:none`（[TopHeader.module.css:65](../components/board/TopHeader.module.css#L65)、当時のコメント「TITLE / TUNE / POP OUT / SHARE are desktop-only for v1」）＝**スマホから共有リンクを作れない**。タブレット（>640px）では SHARE ピルは出る（ただし触り寸法は要確認）。**「コラージュを画像で SNS シェアさせてバイラルを起こす」というミッションの根幹**なので公開前に必要。選ぶ→並べる→作る の全工程を指で成立させる話＝**別途 brainstorm→spec→plan**。最小 MVP 候補＝「並べる」を飛ばして選ぶ→作る（既存の自動配置のまま）。調査中。
