...
**s222 (2026-09-26〜27)**: LP のヘッダー透過と落ち着いた文面(15言語)を本番反映。続けて LP を作り直す見本 v10 を作成・ユーザー承認(白いボードが動くヒーロー・6段の機能紹介でカーソル操作と本当に再生される動画・回る円のボタン)。本番実装をブランチ `feat/lp-v10` で開始し、Task 1(15言語の文言)・Task 2(動きの計算)まで完了。ボード UI の棚卸し `docs/superpowers/specs/2026-09-26-board-ui-inventory.md` を作成。詳細 TODO_COMPLETED.mdの「s222」節。
...
### LP v10 の持ち越し(s222・本番実装の設計書 §6 と検収より)

- **アラビア語の右から左の組みが、サイト全体で未対応**(LP も含め左から右で表示されている)。LP v10 でも変えない方針。対応するかは要相談。
- **LP のボードの絵を、ボード刷新後に実物へ合わせ直す**(LP v10 は今後の白い既定テーマを先取りした絵)。
- **日英以外13言語の新しい文言(見本の中の札・ツイート風カード3枚・見出しの改行位置)は Claude 訳**。いずれネイティブ確認をしたい。

### s222 で Claude が気づいた（ユーザー報告ではない・直すかは要相談）

- テキストだけのツイートカードにマウスを乗せると、左上の「+ TAG」(CardsLayer、top:8/left:8)が投稿者表示(`PlaceholderCard.module.css` の `.author`、top:10/left:12)と同じ位置に出て重なる(ユーザーが LP の見本として送ったスクショに写っていた)。サイト名表示(`.hostname`、同位置)も同じ可能性。ボードの UI 刷新のときに扱う。
