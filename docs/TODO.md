...
- **(N-13) 画像カードの台紙リデザイン = session 141 で実験→混乱→revert。次回小さく区切ってやり直す** — 試した順: ①方眼/ノートを台紙プールに追加 → ②シート全面表示（穴/罫を見せる）→ ③白窓撤去+cover+低品質台紙除外。一気にやって「意味が分からない」状態になったため **画像カードの台紙だけ実験前(pre-N-13)へ revert 済**（白窓+contain+8枚プール）。**良くなった分は維持**（下線/N-11緑/N-12写真持ち上げ/テキスト先頭切れ/テキスト紙のままLB）。**次回はこの順で1つずつ実機確認しながら**:
