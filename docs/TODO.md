...
**s217 (2026-09-21)**: **I-5(パスワード復旧手段が無い問題)を解決・本番反映・ユーザー実機確認済**。ユーザー決定「復旧手段は必ず必要」→1PasswordのEmergency Kit方式(復旧キー)を採用。設計→実装計画(9タスク・subagent-driven-development)→全タスクレビュー→最終ブランチレビュー(opus)で5件のImportant指摘発見・修正(復旧キー専用のPBKDF2繰り返し回数をレコード自己完結化/端末間同期のLWWが復旧キーを無言で消す不具合を2回のレビューを経て根治/コピー完了フィードバック欠落/既存ユーザーへの案内不足→ユーザー指示で「強制作成」に発展/エラーハンドリング取りこぼし)。新規vaultは自動発行、既存vaultは次回解錠時に強制作成(見落とし防止のためユーザー指定)。本番確認後にユーザーから3件のフィードバック→全対応: (1)復旧キー画面の誤操作防止(コピーするまで閉じられない・AWS方式)、(2)無意味になったパスワードヒント欄を完全削除、(3)Chromeのパスワード更新提案を促すため3画面を本物の`<form>`化+隠しユーザー名欄追加(値はユーザー指定の「AllMarks」— 一度「Private」と実装ミス→訂正の教訓あり)。フルスイート**2980/2980**・tsc0・playwright16/16・build OK。詳細はCURRENT_GOAL.md。次=①リアルタイム同期化の検討(継続保留・今回も未着手)②運用セットアップ(claimレコードseed・ユーザーが「きちんとやろう」と明言・着手待ち)③PL-1/PL-2(ユーザー本人の作業と確認済)。
...
**s211 (2026-09-04〜09-14)**: 端末間同期 **束3(足し算マージ + Drive 読み書き)実装・レビュー完了・master マージ済**（merge commit `1885308d`。**デプロイは未実施**＝同期が実際に使える形（束4以降）になってからまとめて行う方針）。`lib/sync/merge.ts`(純関数6種 + `SyncSnapshot` 型)＋`lib/sync/drive-adapter.ts`(fetch のみ・token 注入・Drive REST v3 folder/file CRUD)。**呼び出し元ゼロ=既存挙動は完全不変**。フルスイート **2706/2706**・tsc 0・eslint 0・build OK。opus 全ブランチレビューで重大バグ発見(タグの無条件和集合が Private の平文/暗号文の結合を壊す)→暫定修正→**ユーザーに「業界標準は？」と問われ再調査した結果、根本原因は和集合そのもの（タグ外しを伝播できない）と判明**→**`tags[]` を他フィールドと同じ whole-record LWW に統一**（集合和撤廃）に revise・再レビュー clean。タグ外し伝播も同時に解決。調査詳細は memory `project_allmarks_sync_tag_merge_strategy`（再調査不要）。詳細=設計書 §15 束3 節 ＋ `docs/superpowers/plans/2026-09-04-device-sync-bundle-3-merge-drive.md`。次=束4(`engine.ts` pull/merge/push ＋ `sync-store` ＋ `vault.json` 授受)。詳細 CURRENT_GOAL.md。

**s210 (2026-09-03〜04)**: 端末間同期 **束2(ログイン受け渡し)実装・本番反映済**。`functions/api/gauth/{token,refresh}.ts`(stateless OAuth 中継・無保存無ログ)＋`_shared.ts`、`lib/sync/{gauth-types,google-identity,auth}.ts`。**UI ゼロ配線=既存挙動は完全不変**。master マージ `c63d1724`・`allmarks.app` デプロイ・本番実機で OAuth 中継チェーン生存を確認。Google Cloud `allmarks-sync` プロジェクト作成済(同意画面テスト中・非機密スコープ3種・クライアント "AllMarks Web")、client_id → `.env.production`+CF、secret → CF 暗号化+`.dev.vars`。フルスイート 2642/2642(`channel.test.ts` 既存 flake も修正)。**束3〜4 必須制約は設計書 §15**(特に `merge.ts` は `updatedAt` を `typeof x === 'number' ? x : 0`)。**公開前タスク PL-1/PL-2 発生**(下記§公開前)。

**s209 (2026-09-03)**: 端末間同期 **束1(下ごしらえ)実装・本番反映済**。`BookmarkRecord`/`CardRecord` に `updatedAt`(v16→v17 migration・実機545件で通過確認)、`touchBookmark` スタンプを全書き込み経路に、タグのソフト削除、`lib/sync/device-id.ts`。同期ロジックはまだゼロ=既存挙動は完全不変。実装計画=`docs/superpowers/plans/2026-09-02-device-sync-bundle-1-groundwork.md`。
