...
### 直近の状態 (2026-05-26 セッション 77 — /triage 全面再設計 ship 済 (= 4 辺 chip 帯 + 大 canvas + 横並び 2 カラム + ambient backdrop + Liquid Glass refraction)、 ただし屈折評価不能問題で 校正グリッド導入が次セッション最優先)

**ship 済 (= 本番反映、 session 内 14+ deploy)**:

1. **EXPORT/IMPORT chrome ボタン削除**: session 74 の保険、 user 「残ってる」 指摘で撤去、 backup.ts / BackupButton.tsx file は残置
2. **ja.json sidebar i18n**: 「MOODS → TAGS」、 「+ 新しい mood → + 新しいタグ」 (= 他 14 言語は D4 持ち越し)
3. **キー操作改革**: 矢印 + WASD で 4 方向 swipe、 Space スキップ、 数字 1-9 は co-tag toggle のみ、 DirChip 内 「↑ W」 形式
4. **TagPicker overhaul**: 白テキスト + 2 段 text-shadow 統一、 chip 大 (= padding 18/28 min 132×88)、 co-tag chip pill 化
5. **全面 layout 再設計**: TagPicker から DirChip / CoTagStrip / useTagPickerKeys を export 化、 TriagePage で「4 辺 fixed strip + 中央 fixed canvas」 構造、 strip 帯 background 透明、 entry picker は simpleRoot 分離
6. **TriageCard Lightbox 視覚統一**: 縦長 4:5 白カードから横並び 2 カラム (= 左 thumbnail 自然 aspect + 右 320px text panel)、 Lightbox 完全同等の視覚言語
7. **canvas に card auto-fit + AmbientBackdrop + 強屈折 Liquid Glass**: canvas 内 flex 1 で card 自動 fit、 ambient backdrop は inset -2% / blur 6 / opacity 0.70 で背景拡張 + swipe 連動 12% translate、 canvas は 白 frosted (= rgba(255,255,255,0.10)) + border + inset highlight + drop shadow、 backdrop-filter url(#triage-glass-refract) scale 80 + blur 12 + saturate 160% で強屈折 (= 既存 LiquidGlass scale 12 は board 用、 触らず分離)

**user 視点**: 中央に大きい白 frosted ガラスパネル、 中に横並び 2 カラム card、 4 辺に WASD ラベル付き chip、 背景は現カードサムネ拡大、 swipe で背景も同方向に流れる

**設計上の重要発見 (= memory 化済)**:
- **Liquid Glass scale**: 12 → 80 で初めて屈折 visible、 dark background は屈折殺す、 白 frosted 必須 (= kube.io / mycatwrotethis 参考)
- **校正グリッド戦略** (= user 天才提案、 新 memory `feedback_calibration_grid_for_visual_effects`): 視覚効果は背景に直線格子を一時配置で客観評価
- **AskUserQuestion で design 系を聞かない** (= 新 memory `feedback_no_question_box_for_design`): 平文対話
- **strip 帯 dim 0** が正解、 contrast で縁線出る
- **解釈 mismatch 注意**: user 「白い縁」 = strip 帯 dim、 私が canvas border と誤解 → mask 追加 → revert

**テスト**: 829 PASS 維持、 tsc 0 errors、 build success

**未確認 (= 次セッション最優先)**:
- **屈折評価不能**: scale 80 で実装したが Claude/user 両方「屈折効いてるかわからん」、 user 提案の校正グリッド (= 蛍光色直線格子) を canvas 下に一時配置で可視化 → 数値調整 → グリッド撤去
- **fps カクつき**: backdrop-filter url() + scale 80 は DPR 2.58 で性能 risk、 user 体感判定待ち
- **canvas rectangular silhouette 残存**: 校正グリッド評価後に再調整

詳細 narrative: [TODO_COMPLETED.md](./TODO_COMPLETED.md) セッション 77 セクション
