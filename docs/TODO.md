...
### 直近の状態 (セッション 83 — シェア機能完全作り直し設計 + Phase 1-2 データ層 + Cloudflare 基盤実装)

**ship 済 (= 安定区切り、 11 commits、 既存挙動への副作用ゼロ)**:
...
3. **Phase 1 完成** (Tasks 1-7): `lib/share/` に v2 系 8 ファイル + テスト 7 ファイル
   - `types-v2.ts` / `validate-v2.ts` / `kv-id.ts` / `encode-v2.ts` / `decode-v2.ts` / `snapshot.ts` / `board-to-share.ts` / `api-client.ts`
4. **Phase 2 完成** (Tasks 8-11): Cloudflare 基盤
   - `wrangler.toml` 新規 (= SHARE_KV binding、 ID placeholder)
   - `functions/api/share/create.ts` (= POST 短縮 URL 発行)
   - `functions/api/share/[id].ts` (= GET シェアデータ)
   - `functions/api/share/[id]/og.ts` (= GET OG image WebP)

**新方針 (= シェアの作り直しコンセプト)**:

旧 ShareComposer + PNG export 系は完全廃案。 新方針:
- URL は `allmarks.app/s/<6文字>` (= Cloudflare KV、 30 日 expiry)
- 送信側: 軽量 modal + viewport snapshot + URL コピー / X 投稿
- 受信側: 送信者ボード読み取り専用 + 「全部取り込む」 / 「選びながら取り込む」 (= /triage 流用)
- sender's tags は受信者に提案表示、 accept したものだけ取り込み
- フィルタ後全件 (= スクロール先含む)、 上限 100 件

**コスト**: 1 万 MAU で ¥0、 100 万 MAU で約 ¥15,000/月。

**検証**: vitest 852 → **881 PASS** (= +29)、 tsc 0 errors、 既存テスト regression なし

**🔴 user 操作待ち (= Phase 3 以降の前提条件)**:

Cloudflare ダッシュボードで:
1. https://dash.cloudflare.com/ → Workers & Pages → KV
2. `SHARE_KV` namespace 作成 (= 本番用)
3. `SHARE_KV_preview` namespace 作成 (= preview deploy 用)
4. 2 つの ID を [wrangler.toml](../wrangler.toml) の `REPLACE_WITH_KV_NAMESPACE_ID` / `REPLACE_WITH_PREVIEW_KV_NAMESPACE_ID` に貼り付け
...
進め方推奨: **subagent-driven** (= task ごとに fresh subagent dispatch + checkpoint review、 並列性活用)

詳細 narrative: [TODO_COMPLETED.md](./TODO_COMPLETED.md) セッション 83 セクション
