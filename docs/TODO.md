...
### 直近の状態 (セッション 205 — ★N-77(YouTubeサムネがスクロール中に灰色になるバグ)をsystematic-debuggingで根治・未デプロイ)

**session 204で出た9件のブラッシュアップ候補(N-69〜N-77)から「好きに進めてOK」を受けてN-77を選定・着手。** 詳細は[TODO_COMPLETED.md](TODO_COMPLETED.md)。

- **根本原因**: `public/sw.js`のService Workerがサードパーティ画像(i.ytimg.com等)まで無差別にインターセプトしており、スクロールでカードがアンマウントされ画像読み込みがキャンセルされた際に`respondWith(undefined)`経路に落ちてネットワークエラー(=壊れた画像アイコン=灰色)になっていた。
- **修正**: `public/sw.js`にクロスオリジンリクエストの早期returnを追加(`/api/`スキップと同じ書き味)。`CACHE_VERSION`もbump。回帰テスト`tests/sw-fetch-routing.test.ts`新規(TDD、修正前RED確認済)。
- **検証**: tsc0/vitest全301ファイル2514緑/関連e2e(1件は無関係な既知flaky、単体再実行でPASS確認済)/`pnpm build`成功。
- **副産物**: `functions/api/ogp.ts`が上流の非2xxを常に502へ丸めてしまい`gone`判定に絶対到達しない実在バグを発見(N-70の手がかりとして上記N-70の行に追記済、修正は別スコープのため見送り)。
- **★次セッション最優先**: ユーザーに本番デプロイの可否を確認してから`allmarks.app`へデプロイ。OK後は残り8件(N-69〜N-76)から次を選ぶ。
...
- **(N-70) デッドリンクの誤判定** — 実際は生きているリンクなのに、まれにデッドリンク扱いになることがある（ユーザー報告、頻度・再現条件は未確認）。着手時はまず実機再現から（systematic-debugging推奨、憶測で直さない）。**s205調査で副次的に発見した手がかり**: `functions/api/ogp.ts`は上流(元サイト)が404等どんな非2xxを返しても、常に自前で502を返す実装(`if (!res.ok) return jsonResponse(...502)`、専用テストあり=意図的)。クライアント側`lib/board/revalidate.ts`の`defaultFetcher`は`res.status===404||410`の時だけ`gone`と判定するため、**この経路では非tweet URLは理論上`gone`に絶対到達しない**(常に`unknown`扱い＝1時間後に再試行し続けるだけ)。本番`curl`で実証済み: 同一の生きたYouTube URLへの連続リクエストが`502/502/200`と揺れる(YouTube側がCloudflare Workersの共有egress IPを断続的にブロック/レート制限している可能性、`reference_cf_worker_5xx_rebrand`と同種)。→ **N-70が本当に踏んでいるのはtweet側の`checkTweetLiveness`(`lib/board/tweet-liveness.ts`)の「200だがTweetペイロードでなければgone」ヒューリスティックの可能性が高い**(一時的なレート制限/パース失敗が"gone"に化ける)。着手時はまずtweetのURLで再現するかから確認。
...
- **(N-73) デッドリンクにもTRASHのような「まとめてゴミ箱へ」ボタンが欲しい** — 現状TRASH表示にはある一括操作が、デッドリンク表示には無い（推定・実コード未確認）。
