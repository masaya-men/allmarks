...
- **🐛 Grid テーマがリロードで消えるバグ修正（既存・grab無関係）**: 静的プリレンダは default テーマ（`patternType:'none'`）で焼かれ、React 18 が hydration の**属性**不一致を直さないため、保存済み grid が `data-pattern='none'` に固着＝格子消失（Paper は data-theme-id CSS 由来で無傷）。**patternLayer を post-mount フラグでゲート**（サーバー＋初回描画で出さず、mount 後に正しい柄で新規描画）。playwright で grid→`linear-gradient` 描画・default 不変を実測。
- 正本: [spec](superpowers/specs/2026-07-01-board-grab-wiggle-design.md) / [plan](superpowers/plans/2026-07-01-board-grab-wiggle.md)。**次**: ③プレミアムテーマ制作／④K3実装（`docs/private/2026-07-01-k3-unlock-plan.md`）／選択的シェア から相談。詳細 [CURRENT_GOAL.md](CURRENT_GOAL.md)。
...
### session 150 で報告（新規・ユーザー実機メモ — 細かい気づき7件）

- **(N-14) Lightbox 表示中はボードのモーションを止める** — ライトボックスを開いている間、背後のボードのモーション（浮遊/スライドショー巡回/動画再生等）は動かす必要がない。切り替えが重くならない限り無駄な動作は省く。要調査: Lightbox 中に何が動き続けているか（`PlaceholderCard` の ambientOn は既に `!lightbox` gate 済のはず。他の float/動画/背景音波を確認）→ 開いてる間は停止。**quick-medium**。
...
- **(N-16) ボードが空のとき青っぽい「旧モーダル」が出る** — 空ボードの empty-state UI が旧デザイン（青系）のまま。現行トーンに合わせる。要 empty-state コンポーネント特定。**quick-medium**。
- **(N-17) TRASH のときだけ EMPTY 表示を赤で目立たせる** — ゴミ箱ビューで空のとき、気づけるよう EMPTY を赤系に強調（通常フィルタの EMPTY は現状維持）。**quick**。
- **(N-18) 別ウィンドウのタグ付けウィンドウで文字見切れ** — スクショ: 2カラムのタググリッドの右カラムが右端で切れる（"read la…"/"ui insp…"/"youtube…"/"recipes…"/"snippet…"）。拡張のクイックタグ窓 or /save 窓 or PiP のどれかを特定して折り返し/幅を修正。**quick-medium**。
- **(N-19) カードのサイズ変更・場所変更をいつでも default に戻せる** — 一括 or 個別で、リサイズ/移動のカスタムをいつでも既定へリセット。要設計（UI 置き場所・一括/個別・"default" の定義＝自動サイズ＆保存順）。**feature・要ブレスト**。
- **(#4 = 既出 N-05) LP ナビの格納演出ブラッシュアップ** — Features 等を選ぶとメニューから消え、スクロールでその語がヘッダー帯に入ると緑の玉付きでヘッダーに乗り右へ「しゅん」と格納。詳細は IDEAS.md (N-05)。**design・要ブレスト**。
