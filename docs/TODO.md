...
### 直近の状態 (セッション 81 — /triage 装飾削減 + TRASH 機能復活 + editorial 大見出し + 質素ページ削除 + auto-mode + review-mode pre-arm)

**ship 済 (= 本番反映、 session 内 7 deploy)**:

1. **chip strip 装飾削減**: ピル枠 + 色付き丸 + ✓ 全削除、 文字完全白、 armed = 緑文字 + glow のみ。 board FilterPill dropdown の色付き丸も削除、 INBOX dropdown 行も削除 (= 機能本体は backward compat 維持)
2. **YES / NO 白黒化**: 赤緑廃止、 完全白 0.92 + 矢印で意味区別
3. **ARCHIVE → TRASH rename + 機能完全復活**: `useBoardData` を `items` / `deletedItems` の 2 state に分離 (= 隠れていた bug 治癒、 ARCHIVE は dropdown には出るが事実上空だった)、 `persistSoftDelete` 双方向同期、 新規 `emptyTrash` API。 TRASH 表示中はカードが見える + hover で × → ↺ Restore に swap、 chrome に EMPTY TRASH button
4. **カスタム TrashConfirmDialog**: 黒 backdrop blur + monospace editorial panel + 赤ボタン **2 秒長押し** (= rAF 駆動の赤 fill scaleX、 完了前リリースで snap back)
5. **「+ TAG」 新規タグトリガー**: board CardsLayer + TAG と語彙統一、 click で fade + slide animation で underline input field 展開
6. **「TAG THIS.」 巨大 editorial 見出し**: `clamp(34px, 4.2vw, 56px)` monospace、 末尾 period が AllMarks brand 緑 + 3.2s パルス、 chrome 2 行構成に再編 (= 上段 heading + progress + ESC / 下段 chip strip)、 stagger 入場 80ms ずつ
7. **質素ページ削除 + auto-mode redirect**: EntryPicker 全削除、 mode null 時 `useEffect` で 「未分類あれば untagged、 なければ all」 自動判定 + `router.replace`、 board の MANAGE TAGS 動線も簡素化 (= 1 タグ filter は ?mode=tag:X、 それ以外全部 /triage)
8. **review mode pre-arm + armed-only persist**: all / tag:X mode で current カード切替時に `armedTagIds = new Set(current.tags)` 同期、 user が chip 外す / 追加できる、 YES で armed そのまま persist (= union 廃止、 既存タグ削除も自然に動く)。 untagged mode は session 79 「武装維持」 仕様維持
9. **ガラス外 click で離脱**: TriagePage root に `onClick` で `e.target === e.currentTarget` 判定、 `.outerProgress` も `pointer-events: none` 追加 (= 既存 `.outerHeading` + AmbientBackdrop と揃えて装飾系は全部 click 透過)

**user 視点 (= session 後の体験)**:

- `/triage` 開いた瞬間に巨大「**TAG THIS.**」 が editorial decoration として陣取る、 緑 period パルス + chrome stagger 入場 + ガラスの spotlight 緑 halo と呼応
- chip strip がピル枠なしの素テキスト並び、 armed は緑文字 + glow だけで「選択中」 が一目
- `+ TAG` 押すと fade + slide で underline input field がさっとスライドイン
- 未分類カードを連続スワイプで武装維持仕様、 使い切ったら板 MANAGE TAGS 経由で auto「**全部**」 mode、 既存タグ付きカードを review (= カード切替ごとにタグが緑、 外したり追加したり編集可能)
- 板で × 削除 → **TRASH** に積まれる (= 旧 ARCHIVE)、 TRASH 表示中はカード見える + 個別 ↺ Restore + chrome に **EMPTY TRASH** button
- EMPTY TRASH 押すと editorial confirm dialog、 赤ボタンを 2 秒長押しで赤 fill 走り切って永久削除
- /triage の真っ黒余白 (= ガラス外 + ボタン以外) を click で板に戻れる (= ESC と同じ)

**テスト**: vitest **829 PASS** 維持、 tsc 0 errors、 build 25 routes static prerender 全 success、 e2e testid 更新済 (= new-mood-* → new-tag-*)

**設計上の重要発見**:

- **`pointer-events: none` + root onClick + `e.target === e.currentTarget` pattern**: 中央コンテンツ枠外 click で閉じる canonical recipe。 装飾要素 (= heading / progress / ambient) は pointer-events none で click 透過させて root に届ける
- **rAF 駆動の長押し進捗 = state 不要**: `--p` CSS variable inline 書き込み + scaleX transition で smooth fill、 React state 経由だと毎フレーム re-render で jank、 firedRef で完了重複発火防止
- **Soft-delete システムは load 段階で active filter すると半分死ぬ**: useBoardData L220 の `!isDeleted` filter で削除済みを state に入れてなかった = ARCHIVE filter 経由表示 + sidebarCounts 両方常に空。 active / deleted を 2 state に分離が筋
- **review mode pre-arm + armed-only persist の対称設計**: 「カード切替で armed = current.tags」 と「YES で armed そのまま保存」 はペア。 片方だけだと chip 外しても保存時 union で復活して意味が崩れる
- **AllMarks「データ表示は pointer-events: none、 操作可能なものだけ auto」 の徹底**: progress label 等の data 表示はクリック target に見えないので click 透過させる方が UX 整合

**未達 (= 次セッション持ち越し)**:

- **タグ削除専用 UI** = EntryPicker 撤去で削除経路ゼロ、 user が必要と感じたら別 UI で復活
- **ハロ強すぎ件** (= /triage 外周 4 段 bloom halo 0.5x 絞り) = session 80 持ち越し継続
- **チュートリアル** (= 初回 onboarding、 「タグを選んで振り分けるフロー」 の認知問題) — session 81 の `TAG THIS.` 大見出しで部分達成、 完全 onboarding は別途
- **No 含めて巻き戻す undo 拡張**
...
詳細 narrative: [TODO_COMPLETED.md](./TODO_COMPLETED.md) セッション 81 セクション

---

### 旧情報 (2026-05-27 セッション 80 — /triage 全面 polish 集中: 凸レンズ屈折 + 連続スライド + overflow 修正 + スポットライト + 操作系 + Z 取り消し修正)

**ship 済 (= 本番反映)**:

1. **凸レンズ屈折 PNG 生成 + 差し替え**: `scripts/generate-lens-edge-displacement.mjs` で radial r² barrel distortion PNG を生成 (= 中央 R=G=128 灰、 r² で縁にいくほど色振れる)、 `public/displacement/lens-edge.png` に出力、 TriagePage の `feImage href` 差し替え。 中央うっすら拡大 + 縁で強い屈折の連続曲線
2. **`.root` の radial vignette 削除**: 中央 45% 黒の vignette が凸レンズ magnify と二重で暗くしてた、 削除で本来色が透ける
3. **2 枚並走 slider (= continuous slide)**: TriagePage に incoming state、 TriageCard / AmbientBackdrop に `role + enterDirection` prop、 canvasCardHost を `display: grid; grid-template-rows: 1fr; grid-template-columns: 1fr; place-items: center` で 2 枚同 cell スタック、 純粋 translateX で「1 枚の絵が横スクロール」 感
4. **カード overflow 修正**: backup JSON (= 567 ブクマ) を Playwright で IDB 注入してスキャン → sumy/Instagram カードが canvas 455px 超え 613px に膨らむ overflow 検出 → `grid-template-rows: 1fr` 明示で親 100% clamp + child `min-height/width: 0` で flex 子 shrink 許可
5. **mount-flash 修正**: animation-fill-mode `forwards` → `both` で 1 frame flash 排除
6. **ガラス 10% 白オーバーレイ削除**: 透明に変更でガラス越し色が鮮明に
7. **スポットライト効果**: box-shadow を `.canvas::before` から `.canvas` 本体に移動 (= `.canvas { overflow: hidden }` が `::before` の外向き shadow を clip してた)、 9999px spread shadow (= 0.62 黒) で外側を暗く + 4 段 bloom (= 12px / 48px / 140px / 280px 寒色寄り白) で「ガラス自体が光ってる」 効果
8. **操作系 polish**: ヒント文字列簡素化「1-9 タグ ON/OFF · Z 取り消し」 + canvasFooter 0.55 → 0.92 白 + Yes/No を `<button>` 化 (= マウスクリック可、 hover で `brightness(1.30) + scale(1.06) + 外側 4px translate`)
9. **HeuristicTagger 統合 → 即撤去**: 一旦 triage に統合 (= TopTagStrip に suggestedTagIds prop + ✦ マーク + 寒色ハロ)。 user の汎用英単語タグ (= YOUTUBE / TEST / X / CSS / DESIGN) で keyword 部分一致 (0.5 confidence) が誤爆爆発、 user 「精度微妙すぎ」 で撤去判断。 機構 (`lib/tagger/heuristic.ts`) は残置、 将来 ≥0.8 のみで再挑戦可
10. **Z 取り消し 3 バグ修正**:
    - (A) アニメ中 (= 360ms 間) の race: handleUndo に `exitDecision != null` bail 追加
    - (B) state 喪失: handleNo / handleYes(武装ゼロ) の `setLastAction(null)` 削除、 lastAction は最後の Yes-with-tags まで保持
    - (C) index 復元: 単純 `setIndex(i-1)` → undoTargetRef + useEffect on queue で `queue.findIndex(it => it.bookmarkId === target)` で正確位置
...
- **Z 取り消しが本当に機能** (= Playwright 検証済)

**テスト**: 829 PASS 維持、 tsc 0 errors、 build 25 routes 全 success

**設計上の重要発見 (= memory 化候補)**:

- **`overflow: hidden` は子要素 (= `::before`) の外向き box-shadow を clip する、 要素自身の shadow は clip しない** ← ガラス spotlight 効果が出なかった真因
- **animation-fill-mode `forwards` だけだと mount 直後の 1 frame flash が残る、 `both` (= backwards 効果含む) が必要**
- **CSS grid + place-items center + デフォルト auto-rows は親 height 無視で行が content height に拡張する**、 `grid-template-rows: 1fr` で明示 clamp 必須
- **HeuristicTagger の keyword 部分一致 (0.5 confidence) は汎用英単語タグで誤爆爆発**、 substring 比較は semantics 理解しない
- **setTimeout で予約された state 変更は中断必須**: undo 系の race は exitDecision フラグで bail or cancel 機構が必要

**未達 (= 次セッション持ち越し)**:

- **ハロが強すぎ件** (= session 80 で「一旦 OK」 保留): 4 段透明度を 0.5x に絞る可能性
- **チュートリアル** (= 初回 onboarding): 「タグを選んでから振り分けるフロー」 の認知問題、 user 自身が言及
- **No 含めて巻き戻す undo 拡張**: 現状は Yes-with-tags のみ undo 可、 user が言及したら実装
- **Phase D 残り** (= D1 中断再開 / D4 14 言語 mood→tag rename / D5 NewMoodInput→NewTagInput rename)
- **convex bezel α 案** (= session 78 持ち越し): pre-built PNG triplet

詳細 narrative: [TODO_COMPLETED.md](./TODO_COMPLETED.md) セッション 80 セクション

---

### 旧情報 (2026-05-26 セッション 79 — /triage タグ付け model 全面 redesign + 武装タグ multi 選択 + Yes/No swipe + ワールドスライド、 4 方向 swipe 完全撤去)

**ship 済 (= 本番反映、 session 内 2 deploy)**:

1. **chip 配置 3 回試行**: ガラス内 flex 行 → ガラス外 4 strip → ガラス内絶対配置オーバーレイ。 ここで user が根本問題に気づき「4 方向振り分けってもう難しすぎでは?」 と reframe
2. **brainstorming skill 起動 + Visual Companion (localhost:58851)**: 業界の高速タグ付け 5 種比較 (= 数字キー / AI 推奨 / type-to-tag / 2-3 方向 swipe / 4 方向 + co-tag) で **「4 方向は業界に類似なし、 暗記コスト常時走る」** が見える化
3. **user 対案で model 確定**: 「左右だけで Yes/No、 武装タグ multi 選択で複数同時付与」 + 「カードは縮小せず端までスライド、 背景も同期、 ガラスは固定窓 + clip」 = **武装タグ multi 選択 + Yes/No + ワールドスライド** 採用
4. **実装**: TriagePage.tsx 全面書き換え、 TopTagStrip 新規、 DirChip 完全削除、 AmbientBackdrop / TriageCard を Yes/No 用に書き換え (= translate-only)、 Y/N hints 左右に常時表示、 keyboard: → / D = Yes, ← / A / Space = No, 1-9 = 武装トグル, Z = undo, Esc = exit、 ドラッグ X 軸のみで判定 (60px threshold)
5. **ja.json hint string 更新**: 「→/D YES · ←/A NO · 1-9 タグ ON/OFF · Z 取り消し」
6. **memory 新規 `reference_tweet_video_frames_pipeline`**: LoPo 等他プロジェクト移植用に Tweet 動画 → 3 枚サムネ pipeline 全体 (6 ファイル + 4 落とし穴) を保存、 user に LoPo コピペ用 message も提供

**user 視点 (= session 79 後)**:
- /triage で 1 つ以上のタグを click → 緑 + ✓ で武装状態に
- カードに対して → / D / 右ドラッグ で武装タグ全部一括付与 + 次へ
- ← / A / Space / 左ドラッグ で何もせず次へ
- 連続 swipe で武装維持 (= ガンガン振り分け可能)、 途中で武装内容変更も可
- アニメは縮小無し、 カード + 背景が同方向に translateX 360ms、 ガラス clip で消える

**設計上の重要発見 (= memory 化済)**:
- **「武装」 等の創造的シンボルを UI に漏らさない**: 私の脳内ショートハンドが UI 寄りに漏れたのを user が即停止、 memory `feedback_ui_vocabulary` 再強化
- **業界調査は brainstorm 早期に大きいリターン**: 4 方向 swipe の根本問題が競合 5 種比較で 1 ターンで露見
- **user の casual な「素人考えですが」 提案は道筋を切る**: 「左右だけ Yes/No + 武装 multi 選択」 が私の「数字キー primary」 案より遥かに洗練 (= memory `feedback_layman_simple_path` 再強化)
- **動きの仕様は user 直接発言を引用**: 「ガラスの端まで普通の大きさのまま単にスライド、 はみ出た分は見えないで消えていく」 = 仕様文として実装直結

**テスト**: 829 PASS 維持、 tsc 0 errors、 build success、 deploy 2

**未達 (= 次セッション持ち越し)**:
- **ガラス中央の歪み観察**: AmbientBackdrop (= card サムネ blur up) の微妙な色グラデを displacement map (scale=80) が歪めて blob shape として可視化、 user 「今は残しで」 で却下、 後で削減 / 撤去判断
- **snap でカード退場 → 入場が繋がる**: 「continuous slide」 (= 2 枚並走 slider) 未実装、 中規模追加工事
- **サイズ / レイアウト brushup**: 「ガラスの大きさとレイアウトもっとブラッシュアップしたい」 と user 発言、 次で議論
- **convex bezel α 案** (= session 78 持ち越し): pre-built PNG triplet で再挑戦
- **Phase D 5 項目** の一部 (= D1 中断再開、 D4 他 14 言語 mood→tag rename、 D5 NewMoodInput → NewTagInput rename) 持ち越し。 D2 (= しゅっアニメ進化) と D3 (= タグ削除 fx) は model 変更で意味が変わった、 再設計必要

詳細 narrative: [TODO_COMPLETED.md](./TODO_COMPLETED.md) セッション 79 セクション

---

### 旧情報 (2026-05-26 セッション 76 — scroll polish 4 step 完遂、 メータークリック user 評価「許容範囲」、 booklage.pages.dev 反映済)
