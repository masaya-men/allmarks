...
### 直近の状態 (2026-05-26 セッション 78 — /triage Liquid Glass 屈折の真因解明 + 校正グリッド戦略実証 + convex bezel β 案撤回、 全体歪みタイプ屈折で着地)

**ship 済 (= 本番反映、 session 内 10+ deploy)**:

1. **校正グリッド戦略実証**: 蛍光黄緑 SVG pattern + `?grid=1` URL gate で /triage に一時オーバーレイ、 user/Claude 両方が screenshot で屈折を客観評価可能に。 session 終了時に撤去
2. **z-ladder strict 化** (= Desktop Claude 診断採用): `.root` に `isolation: isolate`、 AmbientBackdrop z 1 / CalibrationGrid z 2 / canvas z 3 / strips z 4。 旧 z 0/0/1/2 は同 z + 独立 stacking context で順序戦争
3. **`.canvas` のガラス装飾を `::before` 疑似要素に**: 旧 `.canvas` がガラス装飾 + カードコンテナ兼任で TriageCard が stacking context に閉じ込められていた、 装飾を `::before { z-index: -1 }` に逃がして「ガラス vs カード」 順位確立
4. **🔴 LightningCSS prefix collapse 真因解明** (= session 78 最大成果): CSS の `backdrop-filter` と `-webkit-backdrop-filter` を同 value 両書きで LightningCSS が `-webkit-` only に collapse、 modern Chromium で computed `none` (= 「ガラスが透明な板」)。 非 prefix だけ書いて LightningCSS の自動 prefix に任せる方式で解決
5. **SVG filter 復元**: 既存 `/displacement/glass-001.png` + `feDisplacementMap scale 80`、 シンプル 2-primitive 構造、 メモリ軽量
6. **撤回: β 案 (lib/glass/displacement-map 動的生成)**: 1265×576 パネル × dpr×4 で 7300 万 pixel loop = タブ 2GB、 user 「怖い」 報告 → 即撤回。 校正グリッドも撤去

**user 視点 (= session 78 後)**:
- /triage で grid 線が canvas 内で **波打ち・歪み**、 屈折機能 (= user 評価「ちゃんとしてる!」)
- ただし「**縁に沿って曲がる convex bezel**」 ではなく「**全体歪み**」 タイプ
- メモリ健全 (= 静的 PNG、 ResizeObserver なし、 state なし)

**設計上の重要発見 (= memory 化済)**:
- **LightningCSS prefix collapse on backdrop-filter** (= 新 memory `reference_lightningcss_prefix_collapse_backdrop_filter`): backdrop-filter で prefix 両書きすると `-webkit-` only に collapse、 modern Chromium で computed none、 非 prefix だけ書く
- **displacement-map は大パネルで OOM** (= 新 memory `reference_displacement_map_large_pane_memory`): lib/glass/displacement-map は ~150px 用、 1265×576 + dpr×4 で 2GB タブメモリ
- **stacking context 二段の罠**: 親 .root + 子 .canvas 両方が context を作ると子の z-index が外に出られない、 `isolation: isolate` + `::before` で分離
- **Desktop Claude 診断 pattern**: user 経由で 3 回追加診断 (= z-ladder + LightningCSS + convex bezel 案) 全部正鵠、 次以降も活用
- **校正グリッド戦略 (= session 77 user 提案) は LightningCSS バグも検出**: 視覚効果系のデバッグに universally 効く

**テスト**: 829 PASS 維持、 tsc 0 errors、 build success

**未達 (= 次セッション持ち越し)**:
- **convex bezel** (= 「ガラスの縁に沿って線が曲がる」 Apple Liquid Glass): 別アプローチ α 案 (= pre-built PNG triplet + build script、 メモリ軽量) で別 session
- 残り polish (a)(b)(c)(e)(f) は session 77 から持ち越し継続

詳細 narrative: [TODO_COMPLETED.md](./TODO_COMPLETED.md) セッション 78 セクション
